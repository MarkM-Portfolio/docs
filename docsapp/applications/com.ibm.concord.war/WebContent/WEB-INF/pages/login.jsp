...
.lotusLogin2 div[dir="rtl"] .lotusDescription {
	float:right;
	padding-right:5px;
}
...
.lotusLogin2 .lotusLoginBoxWide div[dir="rtl"] .lotusLoginForm {
...
.lotusui div[dir="rtl"] .lotusRight {
	float:left;
	text-align:left;
}
...
.lotusui .lotusIBMLogo {
	background-image:url("<%= contextPath + staticRootPath %>/images/lc3/lotusSprite-8bit.png");
...
div[dir="rtl"] input.lotusBtn, div[dir="rtl"] button.lotusBtn {
	margin-left: 5px;
}
...
<div class="lotusLoginBox lotusLoginBoxWide">
      <div class="lotusLoginContent" <%=dirAttr%>>
         <div class="lotusDescription">
            
            <h2 id="h2WhatIs"></h2><script>document.getElementById("h2WhatIs").innerHTML = dojo.string.substitute(resourceBundle.login_whatis_symphony, { 'productName' : concord.util.strings.getProdName() });</script>
            <p id="pFindOut"></p><script>document.getElementById("pFindOut").innerHTML = dojo.string.substitute(resourceBundle.login_find_out, ['<a href="https://www.hcltechsw.com/products/connections">hcl.com</a>']);</script>
            <h2 id="h2HaveQuest"></h2><script>document.getElementById("h2HaveQuest").innerHTML = resourceBundle.login_have_question;</script>
            <p><a href="https://help.hcltechsw.com/connections/welcome/index.html" id="hrefTry"></a></p><script>document.getElementById("hrefTry").innerHTML = dojo.string.substitute(resourceBundle.login_try_documentation, { 'productName' : concord.util.strings.getProdName() });</script>
         </div>
         <form class="lotusForm2 lotusLoginForm" role="main" method="post" action="<%=loginUri %>">
            
            <h1 id="h1Welcome"></h1><script>document.getElementById("h1Welcome").innerHTML = dojo.string.substitute(resourceBundle.login_welcome, { 'productName' : concord.util.strings.getProdName() });</script>
            ...
            <span class="lotusRight" aria-hidden="true"><img role="presentation" src="<%= contextPath + staticRootPath %>/images/lc3/blank.gif" alt="HCL" class="lotusIBMLogo"><span class="lotusAltText">HCL</span></span>
            <div class="lotusBtnContainer"><input id="btnLogin" class="lotusBtn lotusBtnSpecial" value="" type="submit"></div><script>document.getElementById("btnLogin").value = resourceBundle.login_button; if(dojo && dojo.isIE){ var sNode = dojo.byId("btnLogin"); if(sNode){dojo.addClass(sNode,"lotusBtn_ie");}}</script>
         </form>
      </div>
</div>
