...
<% 
	...
	request.setAttribute("isDocument", true);
	    		...
	    		if(providersObj != null)
	    		{
	    			for(int j=0; j<providersObj.size();j++)
	    			{
	    				...
	    				if(name != null && "text".equals(name))
	    				{
	    					...
	    					Object trackEnabled = textConfig.get("DocTrackChange");
	    					if(trackEnabled != null)
	    					{
	    						String track = trackEnabled.toString();
	    						docTrackEnabled = Boolean.valueOf(track);
	    					}
...
<script type="text/javascript">
	...
	var g_docTrackEnabled = true;
		if(gIs_cloud) {
			if(g_GatekeeperFeatures && g_GatekeeperFeatures.DocTrackChange && !g_GatekeeperFeatures.DocTrackChange.enabled)
				g_docTrackEnabled = false;
		} else {
			g_docTrackEnabled = <%=docTrackEnabled%>;
		}
	...
	var g_hasNewFeature = <%= NewFeaturesUtil.hasTextNewFeatures()%>;
</script>

<script type="text/javascript" src="<%=  contextPath + staticRootPath %>/js/concord/concord_text.js"></script>
<link rel="SHORTCUT ICON" href="<%= contextPath + staticRootPath %>/images/ibmdocs_wordprocessing_16.ico" />
...
<div id="mainNode" class="wrapper" >
    ...
    <iframe id='editorFrame' frameborder = "0" allowtransparency="true" tabindex="-1" style="width:100%; height:100%" role="region" title="editor Frame">
	</iframe>
