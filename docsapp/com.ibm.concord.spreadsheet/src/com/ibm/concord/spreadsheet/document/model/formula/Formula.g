/*
NOTE for whom needs to parse .g for Java sources.

This .g file generates 2 Java source file, FormulaLexer.java and FormulaParser.java.
Must	:	
* Put FormulaLexer.java to project com.ibm.concord.spreadsheet.common, in package com.ibm.concord.spreadsheet.common.formulalexer;
* Put FormulaParser.java to project com.ibm.concord.spreadsheet, in package com.ibm.concord.spreadsheet.document.model.formula.

All package declarations are updated to fit sources to their projects and packages.
*/
...
atom returns [String value]
  ...
  | x1 = INT (m=COLON x2 = (INT|NAME))?{
      	if($m != null){
		    ...
		    if(token.getArea() != null || !($x2 != null && $m.text.equals(":")))
		    {
  ...
  | NUMBER{
    ...
    | x=SINGLEQUOT_STRING ((name=NAME (WHITESPACE1? m=COLON WHITESPACE1? (x2=SINGLEQUOT_STRING)? endName=(NAME|INT))?{
            ...
            ReferenceToken token = FormulaUtil.generateToken(text, cell,false);
            if(token.getArea() != null || !($endName != null && $m.text.equals(":")))
            {
...
folded returns [String value]
   : (f = FUNC | f = NAME WHITESPACE1? b = '(') {
	  ...
	  (
          (')'{
			FormulaUtil.setErrPropByFuncName($f.text, cell);
          })
          |(range ')'{
          FormulaUtil.setErrPropByFuncName($f.text, cell);
          })
