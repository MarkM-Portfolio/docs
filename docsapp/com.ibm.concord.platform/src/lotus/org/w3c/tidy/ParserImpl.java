...
public final class ParserImpl
{
    ...
    public static class ParseHTML implements Parser
    {
        ...
        /**
         * @see lotus.org.w3c.tidy.Parser#parse(lotus.org.w3c.tidy.Lexer, lotus.org.w3c.tidy.Node, short)
         */
                ...
                if (!((node.tag.model & Dict.CM_INLINE) != 0))
                {
                    ...
                    if (element.tag == tt.tagLi)
                    {
                        if (node.tag == tt.tagFrame
                           ...
                           /*|| node.tag == tt.tagP*/) //allow p inside li tag
                        {
                            lexer.report.warning(lexer, element, node, Report.DISCARDING_UNEXPECTED);
                            continue;
                        }
                        if ( node.tag == tt.tagOl || node.tag == tt.tagUl)
                        {
                            // \n should not be considered as html content when it's located before <ul> and <ol>
                            Node prev = element.last;
                            if (prev != null && prev.type == Node.TEXT_NODE && prev.textarray != null && prev.start <= prev.end)
                            {
                                while ( prev.start < prev.end && prev.textarray[prev.end-1] == '\n' )
                                {
                                    prev.end--;
                                }
