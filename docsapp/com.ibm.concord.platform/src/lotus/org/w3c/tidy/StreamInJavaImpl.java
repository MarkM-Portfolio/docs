 ...
 * @version $Revision: 1.4 $ ($Author: jiaymin $)
 */
public class StreamInJavaImpl implements StreamIn
{
    ...
    /**
     * @see lotus.org.w3c.tidy.StreamIn#readCharFromStream()
     */
    public int readCharFromStream()
    {
        ...
        try
        {
            ...
            //the trailing surrogate pair is already resolved or 
            // invalid character so discard it
            if (c >= 0xDC00 && c <= 0xDFFF) //low-range surrogate pair
            	c = reader.read();
            
            if (c >= 0xD800 && c < 0xDC00) { // high-range surrogate pair
            	int c1 = reader.read();
            	//the surrogate pair will be resolved to a non-BMP character
            	if(c1 >= 0xDC00 && c1 <= 0xDFFF)
            		c = 0x10000 + ((c - 0xD800) << 10) + (c1 - 0xDC00); //such as SIP characters
            	else
            		//discard the leading surrogate pair 
            		//because it's invalid character without the trailing surrogate pair.
            		c = c1; 
            }
    ...
    /**
     * @see lotus.org.w3c.tidy.StreamIn#readChar()
     */
    public int readChar()
    {
        int c;
    ...
    /**
     * @see lotus.org.w3c.tidy.StreamIn#ungetChar(int)
     */
    ...
    /**
     * @see lotus.org.w3c.tidy.StreamIn#isEndOfStream()
     */
    ...
    /**
     * @see lotus.org.w3c.tidy.StreamIn#setLexer(lotus.org.w3c.tidy.Lexer)
     */
