...
public class PPrint
{
    ...
    private void printScriptStyle(Out fout, short mode, int indent, Lexer lexer, Node node)
    {
        ...
        //ignore the cdata here with configuration.ignoreCDATA
        if (lexer.configuration.xHTML && node.content != null && !lexer.configuration.ignoreCDATA)
        {
    ...
    public void printTree(Out fout, short mode, int indent, Lexer lexer, Node node)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                	boolean emptyTD =( (TidyUtils.toBoolean(node.tag.model & Dict.CM_ROW)) && node.getAttrByName("id")==null);
                	if (emptyTD){
                		//ignore empty td
                		//#11537,we should return here if we don't know how to deal with this empty td
                		lexer.report.error(lexer, null, node, Report.UNKNOWN_ELEMENT);
                		return;
                	}
                    printTag(lexer, fout, mode, indent, node);
