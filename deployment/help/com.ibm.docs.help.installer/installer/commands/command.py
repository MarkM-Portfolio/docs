...
def _readCfg(cmd, cmd_instance):
  ...
  if cmd.has_key("cfg") and cmd["cfg"]:
...
def exec_commands(cmd_list, reverse=False, force=False):
  roll_cmds = []
  erro_cmds = []
  order_list = []
  ...
  for cmd in order_list:
    ...
    try:
      ...
      if cmd.has_key("isAtomCmd") and cmd["isAtomCmd"] == False:
        roll_cmds.append(cmd)
      if not force:
        break
