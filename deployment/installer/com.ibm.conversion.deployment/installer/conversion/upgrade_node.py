...
def upgrade():
  print("os.path.isdir(build_path)")
  try:
...
if __name__ == "__main__":
  ...
  try:
    ...
    if not os.path.isdir(build_path):
      print(('%s Error build path (%s) is invalid, upgrade exit now.' % (datetime.datetime.now(),build_path)))
      raise Exception("Installation Failed")
    print("os.path.isdir(build_path)")
    ...
    result = upgrade()
    try:
      shutil.rmtree(CFG.get_temp_dir(), True)
    except:
      pass  
  finally:
