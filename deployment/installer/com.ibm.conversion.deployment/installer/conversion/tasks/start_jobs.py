...
def undo(argv):
  ...
  for host in target_list:    
    ...
    cfg_file_on_dmgr = 'temp/JobManager/'+'/'.join( [directory_name_on_dmgr, web_node_name, web_server_name, plug_cfg_file] )
    if AdminConfig.existsDocument(cfg_file_on_dmgr):
    ...
    if dmgr_node_host_name!= host:
      job_status = distribute_to_nodes([host], cfg_file_on_dmgr, dest_cfg_file_on_webserver)
    else:
      ...
      if wsadminlib.getNodePlatformOS(dmgrnodename) == 'windows':
        ...
        work_dir = 'C:\\'
      #elif wsadminlib.getNodePlatformOS(dmgrnodename) == 'linux':
      else:
        cfg_file_on_dmgr = cfg_file_on_dmgr.replace('\\','//')
