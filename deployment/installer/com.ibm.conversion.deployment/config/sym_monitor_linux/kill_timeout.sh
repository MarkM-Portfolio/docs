...
if [ $# -ne 3 ];then
	echo
	echo "Usage:"
	echo "	kill_timeout.sh [repoPath] [maxConversionCount] [maxTimeToLive]"
	echo "	repoPath -  repositoryPath in conversion-config.json."
	...
	echo
	echo "For example:"
	echo "	./kill_timeout.sh /root/repo 1000 120"
	echo
	exit 1
fi
...
#			echo conversion time is: $convTime seconds
			if [ $convTime -gt $maxLiveTime ];then
				needKill="yes"
#				echo "need to kill soffice because conversion is greater than 120 seconds"
			fi
		fi
