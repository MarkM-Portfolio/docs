...
def fullsyncall():
    ...
    for nodename in nodenames:
        ...
        if not nodeIsUnmanaged( nodename ):
            sop(m,"Sync config to node %s" % nodename)
            Sync0 = AdminControl.completeObjectName( "type=ConfigRepository,process=nodeagent,node=%s,*" % nodename )
            Sync1 = AdminControl.completeObjectName( "type=NodeSync,process=dmgr,node=%s,*" % nodename )
            if Sync0:
                rc = AdminControl.invoke( Sync0, 'refreshRepositoryEpoch' )
                if rc != 'true':  # failed
...
def createSharedLibraryWithScopeID(scopeid, libname, jarfile):
    """Creates a shared library on the specified cell with the given name and jarfile"""
    #m = "createSharedLibrary:"
    #sop(m,"Entry. Create shared library. libname=%s jarfile=%s" % (repr(libname), repr(jarfile) ))
    cellname = getCellName();
    #sop(m,"/Cell:%s%s"%(cellname,scopeid))
    result = AdminConfig.create('Library', AdminConfig.getid("/Cell:%s%s"%(cellname,scopeid)), [['name', libname], ['classPath', jarfile]])
    #sop(m,"Exit. result=%s" % ( repr(result), ))
