...
LC35_OSGIExt_Name = "Viewer_LC35_OSGIExtension.zip"
OSGI_BLD_NAME_PRE = "com.ibm.concord.viewer.lcfiles.extension.provision_"
EXT_PLUGIN_BACKUP_FOLDER = "extension_plugins"

EXT_EAR_PRE_NAME = "com.ibm.concord.viewer.lcfiles.ext_"
EAR_RENAME = "com.ibm.concord.viewer.lcfiles.ext.ear"
...
class InstallPlugin(command.Command):
  ...
  def __init__(self):
    ...
    self.backup_dir = CFG.get_temp_dir() + os.sep+ EXT_PLUGIN_BACKUP_FOLDER
  ...
  def do(self):
    log.info("Install Plugin for HCL Viewer Ext Server Started")
    ...
    log.info("Install Plugin for HCL Viewer Ext Server completed")
    return True

  def undo(self):
    log.info("Start to uninstall Plugin for IBM Viewer Server")
    ...
    log.info("Uninstall Plugin for IBM Viewer Server completed")
    return True
  ...
  def remove_bdl(self):
    ...
    for bdl in os.listdir(self.osgi_dir):
      if bdl.find(OSGI_BLD_NAME_PRE) > -1:
        ...
        bld_num += 1
    if bld_num > 0:
      log.info("%s OSGI bundles removed from Viewer Ext  server %s" % \
        ( str(bld_num), self.osgi_dir))
    else:
      log.info("NO OSGI bundle found from directory %s" % self.osgi_dir)
  ...
  def install_bdl(self):
    ...
    for f in os.listdir(self.ext_bld_dir):
      if f.find(LC35_OSGIExt_Name) > -1:
        ...
        break
    if not bdl_zip:
      raise Exception("%sxxx not found from Viewer Ext build dir %s" %\
  ...
  def osgi_do(self):
    log.info("Install OSGI bundle for HCL Viewer Ext Server Started")
    try:
      self.remove_bdl()
    except:
      log.info("Unable to remove old OSGI bundle")
    self.install_bdl()

    log.info("Install OSGI bundle for HCL Viewer Ext Server completed")

  def osgi_undo(self):
    log.info("Start to uninstall OSGI bundle for IBM Viewer Server")

    self.remove_bdl()

    log.info("Uninstall OSGI bundle for IBM Viewer Server completed")
  ...
  def config_ear(self):
    ...
    for ear_file in os.listdir(self.ext_bld_dir):
      if ear_file.find(EXT_EAR_PRE_NAME) > -1:
        bld_ear_name = ear_file
    if not bld_ear_name:
      raise Exception("Didn't find Viewer EXT EAR in %s" % self.ext_bld_dir)
  ...
  def _make_global_js(self, js_src):
    ...
    for j in js_src.split("\n"):
      if ( (j.strip().find("glb_viewer_url_wnd_open =") > -1) and \
                CFG.get_context_root()):
        j = "".join(["glb_viewer_url_wnd_open =\"", CFG.get_context_root(), "\";"])
  ...
  def ear_do(self):
    ...
    if not call_wsadmin(args):
      return False
    log.info("Install EAR for HCL Viewer Ext Server completed")

  def ear_undo(self):
    log.info("Start to uninstall EAR for HCL Viewer Ext Server")

    args = CFG.get_was_cmd_line()
    # wasadmin command line arguments
    args.extend(["-f",  "./icext/tasks/undo_" + __name__.split(".")[1]+ ".py"])

    args.extend([CFG.get_unstall_app_name()]) #IBMConversion
    #log.info(args)

    if not call_wsadmin(args):
      return False
    log.info("Uninstall EAR for HCL Viewer Ext Server completed")
  ...
  def do_upgrade(self):
    log.info("Upgrading Connections plugins")
    ...
    if not os.path.exists(bdl_zip):
      log.info("No need to upgrade Viewer Extension plugins")
      return True
    else:
      self.updater = FileInstall(bdl_zip, self.osgi_dir, self.backup_dir)
      succ = self.updater.upgrade_zip_file()
      if not succ:
        log.info("Failed to upgrade Viewer Extension required plugins")
        return False

    log.info("Successfully upgrade Viewer Extension required plugins for WebSphere")
    return True
  def undo_upgrade(self):
    log.info("Rolling back the upgrade for Connections plugin")

    succ = True
    if not self.updater:
      log.info("No need to upgrade the plugins")
    else:
      succ = self.updater.undo_upgrade_files()
      if not succ:
        log.info("Failed to rollback the required plugins upgrade")
        return False
    log.info("Successfully rollback the plugins")
    return succ
...
class Map2WebServer(command.Command):
  ...
  def do(self):
    if CFG.get_icext_mode() == "IS_JAR":
      log.info("HCL Connections 3.5+ detected, skip mapping webserver...")
      return True
    elif CFG.get_icext_mode() == "IS_EAR":
      log.info("Map Viewer Ext EAR application to WebServer...")
      ...
      if not succ:
        return False
      log.info("Map Viewer Ext EAR application to WebServer completed")
      return True

  def undo(self):
    if CFG.get_icext_mode() == "IS_JAR":
      log.info("HCL Connections 3.5+ detected, skip undo mapping webserver...")
      return True
    elif CFG.get_icext_mode() == "IS_EAR":
