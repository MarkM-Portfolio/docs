...
class CreateUninstaller(command.Command):
  ...
  def do(self):
    ...
    try:
      shutil.copy(CFG.get_cfg_path(), CFG.get_install_root()+"/cfg.properties")
  ...
  def do_upgrade(self):
    ...
    try:
      ...
      created = CFG.create_new_config(CFG.get_cfg_path(), \
      	os.path.join(CFG.get_build_dir(), "installer", "cfg.properties"), \
      	new_cfg_path, True, False, False)
      
      if not created:
        shutil.copy(CFG.get_cfg_path(), installer_dest)
 
      #update cfg.properties in install root.
      install_cfg_path = os.path.join(CFG.get_install_root(), os.path.basename(CFG.get_cfg_path()))
      if os.path.exists(install_cfg_path):
        os.remove(install_cfg_path)
      shutil.copy(new_cfg_path, CFG.get_install_root())
  ...
  def _lnx_create_installer(self, dest, cfgf, pf):
    ...
    ins_content.append("python3 icext/install%s.py " % self.node_suffix)
  ...
  def _w32_create_installer(self, dest, cfgf, pf):
    ...
    ins_content.append("python icext\install%s.py " % self.node_suffix)
    ins_content.append('"')
    ins_content.append(cfgf)
    ins_content.append('"')
    ins_content.append(" ")
  ...
  def _create_install_uninstall_script(self, installer_dest, install_type="build"):
    ...
    if install_type == "build":
      ...
      pf_path = os.path.abspath(CFG.get_product_dir())
      #if os.name == "nt":
