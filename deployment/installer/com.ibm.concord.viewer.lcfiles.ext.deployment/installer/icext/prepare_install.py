...
class PrepareInstall(command.Command):
  ...
  def verify_scope_name(self):
    logging.info("Verifying WebSphere WebSphere scope_name...")
    args = CFG.get_was_cmd_line()
    args.extend(["-f",  "./icext/tasks/" + "verify_scope_name.py"])
    args.extend([CFG.get_scope_type()])
    args.extend([CFG.get_scope_name()])
    args.extend([self.filesApp])
    succ, ws_out = call_wsadmin(args)
    #logging.info(ws_out)
    if not succ:
      return False
    logging.info("Successfully verify WebSphere WebSphere scope_name")
    return True
...
#	+ "  Also make sure the username and password you entered are correct.")
  ...
  def do_upgrade(self):
    ...
    if not check_version(old_version, new_version):
      logging.error("This build is version (" + new_version + "). It cannot be used to upgrade the installed version (" + old_version + ").\n" \
...
#	+ "  Also make sure the username and password you entered are correct.")
    ...
    logging.info("Viewer Extension is installed " + CFG.get_icext_jar_location())
    logging.info("Viewer Daemon Install root is " + CFG.get_daemon_location())
