...
def getJvmPropertyValue(nodename,servername,propertyname):
    """Get a particular JVM property for the named server.
Some useful examples:
        'maximumHeapSize': 512 ,
        'initialHeapSize':512,
        'verboseModeGarbageCollection':"true",
    ...
    """
    jvm = getServerJvm(nodename,servername)
    children = getFilteredTypeList('Property', [['name', propertyname]], jvm)
    if len(children) > 0:
      return getObjectAttribute( children[0], 'value' )
    else:
      return ''
