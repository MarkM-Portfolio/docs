...
class SetWCProperty(command.Command):
  ...
  def do(self):
    ...
    succ = self.call_task("set_webcontainer_property.py", [self.scope, self.scope_name, self.target_scope, self.name, self.value])
    log.info("WebContainer property set completed")
    return succ   
  
  def undo(self):
    log.info("Removing WebContainer property %s ..." % (self.name))
    succ = self.call_task("undo_set_webcontainer_property.py", [self.scope, self.scope_name, self.target_scope, self.name])
    return succ