...
#print "$Id: wsadminlib.py,v 1.3 2011/04/13 05:29:55 zhaocdl Exp $"
...
def checkProxyVirtualHostAction( action ):
    ...
    if 'name' not in list(action.keys()) or emptyString(action['name']):
        raise m + "ERROR action dictionary is missing key 'name'. action=%s" % ( action )

    if 'type' not in list(action.keys()) or emptyString(action['type']):
        raise m + "ERROR action dictionary is missing key 'type'. action=%s" % ( action )
    type = action['type']

    if "HTTPRequestHeaderAction" == type:
        if 'headerModifyAction' not in list(action.keys()) or emptyString(action['headerModifyAction']):
        ...
        if headerModifyAction == 'REMOVE':
            if 'headerName' not in list(action.keys()) or emptyString(action['headerName']):
                raise m + "ERROR: missing key in action.  action=%s" % ( action )
        elif headerModifyAction == 'SET':
            if 'headerName' not in list(action.keys()) or emptyString(action['headerName']) or \
               'headerValue' not in list(action.keys()) or emptyString(action['headerValue']):
                raise m + "ERROR: missing key in action.  action=%s" % ( action )
        elif headerModifyAction == 'EDIT':
            if 'headerName' not in list(action.keys()) or emptyString(action['headerName']) or \
               'headerValueExpression' not in list(action.keys()) or emptyString(action['headerValueExpression']) or \
               'headerValue' not in list(action.keys()) or emptyString(action['headerValue']):
                raise m + "ERROR: missing key in action.  action=%s" % ( action )
        ...
        if 'compressionType' not in list(action.keys()) or emptyString(action['compressionType']):
            raise m + "ERROR: missing key in action.  action=%s" % ( action )
    elif "RoutingAction" == type:
        if 'routeType' not in list(action.keys()) or emptyString(action['routeType']):
            raise m + "ERROR: missing key in action.  action=%s" % ( action )
...
def createProxyVirtualHostSettings( pvh_id, pvh_settings, ):
    ...
    lepp = None
    if 'local_error_page_policy' in list(pvh_settings.keys()):
    ...
    # This is because we use it in fritz, but it is not recognized by the proxy.
    cepp = None
    if 'custom_error_page_policy' in list(pvh_settings.keys()):
    ...
    if None != lepp:
        ...
        # This is because we use it in fritz, but it is not recognized by the proxy.
        error_mappings = None
        if 'error_mappings' in list(lepp.keys()):
...
def setDmzProxySecurityLevels( nodename, proxyname, dmz_security_levels ):
    ...
    if not 'scope' in list(dmz_security_levels.keys()):
    ...
    # Get overall level.
    if not 'overall_level' in list(dmz_security_levels.keys()):
    ...
    if not 'individual_levels' in list(dmz_security_levels.keys()):
...
def setCEASettings( nodename, servername, settingsdict, clustername=None ):
    ...
    for (settingname,settingvalue) in list(settingsdict.items()):
...
# https://www.ibm.com/support/knowledgecenter/SSEQTP_8.5.5/com.ibm.websphere.base.doc/ae/rxml_atservermanagement.html
...
def setMimeType(virtualHostName, newExtension, newMimeType):
    m = "setMimeType:"
...
def setChannelCustomProperty(nodename, servername, name, value, channelType, endPointName = None, channelName = None):
    ...
    for p in properties:
        ...
        if pname == name:
            ...
            return
    # Does not exist, create and set
    return AdminConfig.create('Property', foundChannel, [['name', name],['value', value]])
...
def setCustomProperty(object_id, name, value):
    ...
    https://www.ibm.com/support/knowledgecenter/SSEQTP_8.5.5/com.ibm.websphere.base.iseries.doc/ae/txml_mailcustom.html

    https://www.ibm.com/support/knowledgecenter/no/SSEQTP_9.0.5/com.ibm.websphere.base.doc/ae/txml_envcustom.html
    """
...
def addOrSetCustomPropertyToDictionary(newName, newValue, dict):
    ...
    if 'custom_properties' in list(dict.keys()):
...
def addAttributeToChannelList(channellist, channelType, channelName, attributename, attributevalue):
    ...
    for old_attribute_dict in channellist:
        ...
        if 'channelType' in list(old_attribute_dict.keys()) and 'channelName' in list(old_attribute_dict.keys())and 'attributename' in list(old_attribute_dict.keys()):
...
def createSharedLibrary(libname, jarfile, nativePath = None):
    ...
    if nativePath != None:
    	result = AdminConfig.create('Library', cell_id, [['name', libname], ['classPath', jarfile], ['nativePath', nativePath]])
    else:
...
def createSIBus(clusterName, nodeName, serverName, SIBusName, SIBusDesc, scope, busSecurity=""):
    ...
    if((len(bus) == 1) or (len(bus) == 0)):
        params = ["-bus", SIBusName, "-description", SIBusDesc]
        if(not(busSecurity == "")):
...
def deleteSIBJMSActivationSpec(jmsASName, clusterName, serverName):
    ...
    if(clusterName == "none"):
        ...
        ra = AdminConfig.getid('/ServerCluster:%s/J2CResourceAdapter:SIB JMS Resource Adapter' % clusterName)
    #endElse
...
def createJAAS(auth_alias, auth_username, auth_password,auth_desc=""):
    ...
    if auth_desc == "":
    	auth_desc = "authentication information"
...
#endDef
def modifySessionSecurityIntegration(nodename,servername,enabled):
    server_id = getServerByNodeAndName(nodename,servername)
    sessionmanager = AdminConfig.list('SessionManager', server_id)
    AdminConfig.modify(sessionmanager, [['enableSecurityIntegration',enabled]])
...
#endDef
def getCellVariable(varname):
    """Return the value of a variable for the cell -- or None if no fsuch variable or not set"""
    vmaps = _splitlines(AdminConfig.list('VariableMap', getCellId()))
    if 0 < len(vmaps):  # Tolerate nodes with no such maps, for example, IHS nodes.
        map_id = vmaps[-1] # get last one
        entries = AdminConfig.showAttribute(map_id, 'entries')
        # this is a string '[(entry) (entry)]'
