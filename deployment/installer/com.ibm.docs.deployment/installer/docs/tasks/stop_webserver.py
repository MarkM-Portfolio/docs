      ...
      if webserver_name == web_server_name:
        ...
        if mBean:
          ...
          if status == 'RUNNING':#'STOPPED':
            #ret = AdminControl.invoke(mBean,'stop','[%s %s %s]' % (cellname,web_node_name,web_server_name),'[java.lang.String java.lang.String java.lang.String]')
            wsadminlib.stopWebServer(web_node_name,web_server_name)
            stopped = "True"
        break
    else:
      mBean = AdminControl.queryNames('WebSphere:type=WebServer,*')
