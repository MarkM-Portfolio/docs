...
class AddBus(command.Command):
  """This command will add bus for journal"""

  def __init__(self):
    self.SIBusName = 'IBMDocsBus'
  ...
  def do(self):
    ...
    log.info("Journal:start to add bus")
    ...
    if clusters:
      ...
      shared_data_dir = CFG.get_shared_data_dir()
      if shared_data_dir.endswith(':'):
        shared_data_dir += '\\'
      store_path = os.path.join(shared_data_dir,'dataStore')
      
      args.extend([store_path])

    succ, ws_out = call_wsadmin(args)	         
    if not succ:
      return False
    log.info("Journal:complete to add bus")
    return True
    
  def undo(self):
    log.info("Journal:Start to remove bus")
    ...
    log.info("Journal:remove bus completed")
    return True
