...
from common import command, CFG, get_upgrade_type
...
class CreateUninstaller(command.Command):
  ...
  def do_upgrade(self):
    ...
    try:
      ...
      #copy cfg.properties from the specified path to installer    
  ...
  def _lnx_create_installer(self, dest, cfgf, pf):
    ...
    ins_content.append("python3 docs/install%s.py " % self.node_suffix)
  ...
  def _w32_create_installer(self, dest, cfgf, pf):
    ...
    ins_content.append("python docs\install%s.py " % self.node_suffix)
    ins_content.append('"-configFile" ')
    ins_content.append('"')
    ins_content.append(cfgf)
    ins_content.append('"')
  ...
  def _create_install_uninstall_script(self, installer_dest, install_type="build"):
    ...
    if install_type == "build":
      #create installer
      cfg_path = os.path.abspath(CFG.get_install_root() + os.sep + os.path.basename(OPTIONS["-configFile"]))
