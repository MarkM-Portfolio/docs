...
from common import command, call_wsadmin, ZipCompat, CFG, FileInstall
...
class InstallPlugin(command.Command):
  ...
  def remove_bdl(self):
    if not os.path.exists(self.osgi_dir):
  ...
  def do_upgrade(self):
    log.info("Upgrading HCL Docs Extension plugins")
    ...
    if not os.path.exists(bdl_zip):
      log.info("No need to upgrade HCL Docs Extension plugins")
      return True
    else:
      self.updater = FileInstall(bdl_zip, self.osgi_dir, self.backup_dir)
      succ = self.updater.upgrade_zip_file()
      if not succ:
        log.info("Failed to upgrade HCL Docs Extension required plugins")
        return False

    log.info("Successfully upgrade HCL Docs Extension required plugins for WebSphere")
    return True

  def undo_upgrade(self):
    log.info("HCL Docs Extension plugins for WebSphere rollback...")
    ...
    if not self.updater:
      ...
      if not succ:
        log.info("Failed to rollback HCL Docs Extension required plugins upgrade")
        return False
    log.info("Successfully rollback HCL Docs Extension plugins for WebSphere")
    return succ
