dojo.provide("writer.view.Tab");
dojo.require("writer.view.Run");
dojo.declare("writer.view.Tab",[writer.view.Run],
{
	MIN_TAB : 1,
	w:0,
	h:0,
	...
	_generateText: function(){
		var text = dojo.isIE?"&#8203;":"\ufeff";//zero-width char
		...
		if (leaderText)
		{
			text = "";
			var computedStyle = this.getComputedStyle();
			var size = common.MeasureText.measureTextRun(leaderText, computedStyle);
			...
			if (dojo.isIE <= 9 || dojo.isSafari) {					
				var totalSize = common.MeasureText.measureTextRun(text, computedStyle);
				while(totalSize.w < this.w && totalSize.w + size.w < this.w){
					...
					totalSize = common.MeasureText.measureTextRun(text, computedStyle);
				}
			}
		}
	...
	render:function(){
		if(!this.domNode ||this.dirty){
			...
			if(leaderText || !dojo.isIE)	// 38413, not set style to empty tab
