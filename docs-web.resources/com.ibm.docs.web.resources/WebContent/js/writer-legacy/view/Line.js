dojo.provide("writer.view.Line");
dojo.require("concord.util.BidiUtils");

writer.view.Line=function(w,h,para){
	this.parent = para;
	this.init(w, h);
};
writer.view.Line.prototype={
	...
	init:function(w,h){
		...
		this.h = h||0;
		this.pureH = h||0;
		this.container = new common.SubContainer(this.parent.container);
		this.initAlignment();
		this.bidiMap = null;		
	...
	initListSymbol:function(listSymbol, body, space, isAlignRight){
		var isInTableCell = writer.util.ViewTools.isCell(body);
		if (isInTableCell)
			space = null;
		...
		var layoutSymbol = function(t){
			...
			if(t.getViewType() == "text.ImageView")
			{
				...
				var size = common.MeasureText.measureTextRun("Q", computedStyle);
				tabH = size.h;
				t.calcSize(size.h);
			}
		...
		if(paraProperty && specialType=="hanging"){
			var hangingValue = common.tools.toPxValue(paraProperty.getIndentSpecialValue() +"pt");
...
//				else
	...
	isFirstLine:function(){
		...
		var preParentView = views.prev(this.parent);
	...
	initProperty:function(){
		...
		if(paraProperty){
			...
			var indLeft = Math.ceil( common.tools.toPxValue(paraProperty.getIndentLeft() + "pt") );
			this.indentLeft = indLeft;	// this.indentLeft is used for calculate Tab position.
			this.paddingLeft = indLeft;
			this.w -= indLeft;
			var indRight = common.tools.toPxValue(paraProperty.getIndentRight() + "pt");
			this.w -= indRight;
			this.rightIndent = indRight;
			var specialType  = paraProperty.getIndentSpecialType();
			var specialIndentVal = Math.ceil( common.tools.toPxValue(paraProperty.getIndentSpecialValue() + "pt") );
			...
			if((!pLine && isFirstLine)|| (pLine && pLine.isFirstPrePageBreakLine()))
			{
				...
				// First line
				var spaceBefore = common.tools.toPxValue(paraProperty.getSpaceBefore() + "pt");
				if(spaceBefore > 0){
					...
					if (preParaProperty)
					{
						...
						if(preParaProperty.styleId == paraProperty.styleId && isContextualSpacing && isPreContextualSpacing){
							...
							/*
							  ...
							  */
							var preSpaceAfter = common.tools.toPxValue(preParaProperty.getSpaceAfter() + "pt");
							if (preSpaceAfter > 0)
							{
...
//				this.fixHeight=common.tools.toPxValue( lineSpacingValue);
			}
			else if(lineSpaceType=="atLeast"){
				var lineSpacingValue = common.tools.toPxValue(paraProperty.getLineSpaceValue());
				if(!isNaN(lineSpacingValue))
					this.h = lineSpacingValue;
			}
			else {
				this.heightRatios = 1;
			}
	...
	setPaddingBottom: function(line, paraProperty){		
		var isContextualSpacing = paraProperty.isContextualSpacing();
		var spaceAfter = common.tools.toPxValue(paraProperty.getSpaceAfter() + "pt");
		if(spaceAfter>0 && !line.next()){
			...
			else if(writer.util.ModelTools.isNotes( line.parent.model.parent )){	// Defect 49230
				line.paddingBottom += spaceAfter;
			}
	...
	getHeight:function(){
		if(this.heightRatios>0){
			//return this.h*this.heightRatios;
			return Math.ceil(this.h + this.pureH * (this.heightRatios - 1));
	...
	getPadding:function(){
		...
		if(paraProperty && this.isBidiLine() && (align=='right' || 
			 (align=='justified' && this._isLastLine())))
		...
		if(this.alignPaddingLeft < 0)
			this.alignPaddingLeft = 0;

		/* offset line to left on 'width' of would to be append carriage, carriage node should appear on left of RTL line */
		if(this.isRtlDir && !this._isTocLine() && pe.scene.isCarriageReturn() && this._isLastLine() && writer.util.ViewTools.isLastViewOfModel(this.parent))
			this.alignPaddingLeft -= common.MeasureText.measureTextRun("\u21b2", "font-size:9pt;font-family:Arial").w;
...
//		}
	...
	_PostProcess: function()
	{
		...
		var isInCell = writer.util.ViewTools.isCell( this.parent.parent );	// defect 48933
...
//			this._PostProcess();
	...
	updateHeight: function(){		
		this.domNode && dojo.style(this.domNode,'height',this.getHeight()+"px");
	},
	
	calcScriptLineHeight: function(){		
		var parentNode = common.MeasureText.textMeasure;
		
		var oldW = dojo.style(parentNode).width;
		dojo.style(common.MeasureText.textMeasureFrame, "width", "1000000px");
		dojo.style(parentNode, "width", "1000000px");	// Mobile fix.
		...
		oldW && dojo.style(parentNode, "width", oldW);
		return h;
	},
...
//		return dojo.query(".carriageNode", this.domNode).length > 0 ? true: false;
	...
	appendCarriage: function(){
		if (!this.domNode || (this.isRtlDir && this._isTocLine()))
			return;
...
//		var carriageNode = dojo.create("div", {"class":"carriageNode", innerHTML: "\u21b2" },	this.domNode );
		...
		if(this.h < writer.util.ViewTools.getCarriageHeight()){
			styleStr += " height:"+this.h+"px;";
			if (this.h < 15)
				styleStr += "font-size:7pt";
		}
		carriageNode.innerHTML = "\u21b2";
		carriageNode.className = "carriageNode";
	...
	/* assemble line text */
	_getText: function() {
		var text = "", run = this.container.getFirst();
		while(run){
			if(run.getViewType() == 'text.Run')	
				text += run.getText();
			else if(run.len == 1) {
	...
	_measureCharWidths: function(run, charWidths) {
		var runNext, runPrev, widthsArray, text = run.getText(), len = text.length, shift = 0,
			cssStr = run.getCSSStr(), textMeasure = common.MeasureText.textMeasure;
		if(BidiUtils.isArabicText(text)) {
			...
			widthsArray = common.MeasureText._getCharArraySize(text, cssStr);

		} else {
			widthsArray = common.MeasureText.measureCharsWidth(text, cssStr);
		}
	...
	initBidiMap: function(isRenderStage) {
		...
		var text = isRenderStage ? this.parent.model.text : this._getText();
		if(isRenderStage) {
			if(BidiUtils.hasBidiChar(text))
				this.bidiMap = {'visualCharWidths': true};		
			return;
		}
		/* initialize logical<->visual map and array of character widths */
		if(BidiUtils.hasBidiChar(text) || this.isRtlDir) {
			...
			var logicalToVisualMap = this.bidiMap.visualfromLogical,
				inRunIdx, visualIdx, inLineIdx = 0,run = this.container.getFirst();
			while(run){
				if(run.getViewType() == 'text.Run') {
					var maxIdx = 0, minIdx = this.bidiMap.levels.length, charWidths = [];
					...
					var charWidth = Math.round(run.getWidth() * BidiUtils.RATIO);
					if(this.bidiMap.levels[inLineIdx] % 2 != 0)
						charWidth *= -1;
	...
	render:function(parentNode){
		if(!this.domNode||this.dirty || parentNode){
			...
			var run = this.container.getFirst(), backgroundColor, unicodeBidi;
			...
			if( run ){
				...
				if(BidiUtils.isBidiOn() && this._isTocLine())
					unicodeBidi = 'embed';
			}
			var bookmarkRuns = [];
			...
			while(run){	
				...
				if( backgroundColor && !dojo.style(runNode, "backgroundColor") ){
					dojo.style(runNode, "backgroundColor", backgroundColor );
				}
				newDOM.appendChild(runNode);
				run = this.container.next(run);				
				if(unicodeBidi && run && run.getViewType() == "text.Tab")
			...
			if(this._isLastLine())
			{
				...
				if (writer.util.ViewTools.isLastViewOfModel(this.parent))
				{
					// section node
					if (paraView.directProperty.getSectId())
					{
						var sectWidth = 50;
						var sectNode = dojo.create("div", {
							"class": 'sectionbreak',
							"style": "width:"+sectWidth+"px;"
						});
						this.domNode.appendChild(sectNode);
					}
					this.appendCarriage();
				}
			}
	...
	_renderListSymbol:function(dom, isCalcHeight){
		if(this.listSymbol){
			var run = this.container.getFirst(), backgroundColor;
			if( run ){
				var txtProp = run.model.paragraph.getMergedTextProperty();
				backgroundColor = txtProp.getBackgroundColor && txtProp.getBackgroundColor();
			}
			this.listSymbol.forEach(function(l){
				...
				dojo.addClass(runNode,"listSymbol");
				...
				if(fallbackNode && fallbackNode.style && fallbackNode.style["fontFamily"]){	// No fallbackNode for image list
					if (concord.util.browser.isMobile() || 
							(dojo.isMac && fallbackNode.style["fontFamily"].toLowerCase() == "symbol")) {
						concord.util.mobileUtil.fallbackListSymbol(fallbackNode);
					}
				}
				if( backgroundColor && !dojo.style(runNode, "background-color") ){
					dojo.style(runNode, "background-color", backgroundColor );
				}
	...
	updateListDOM:function(){
		...
		while(firstNode&&dojo.hasClass(firstNode,"listSymbol")){
...
//		}	
	...
	isPagebreakLine: function(){
		...
		return content && content.getViewType()=="text.PageBreak"; 
	},
	...
	layoutBorderView:function(view, remainingText){
		...
		{	
			...
			if(preView && preView.model.getBorder && preView.model.getBorder()){
				...
				if(BidiUtils.isMergeBordersBidi(view, preView, remainingText, this.isRtlDir)) {
...
//	},
	...
	alignitem:function(bCalcHeight){
		...
		this.container.forEach(function(run){
			...
			{
				console.error("The run layout result was wrong: " + dojo.toJson(run.model.toJson()));
	...
	getChildPosition:function(idx, placeholderArg, bNeedLogicalPos, asRunInLine){
		...
		if(!run){
			return {'x':this.w+this.getLeft(asRunInLine),'y':this.getTop(asRunInLine)};
		}else{
			return {'x':run.getLeft(asRunInLine),'y':run.getTop(asRunInLine)};
		}
	},
	...
	_getVisualOffset: function(lineLogicalIdx, isSegmentStart, logicalToVisualMap) {
		var lineVisualIdx = logicalToVisualMap ? logicalToVisualMap[lineLogicalIdx] : lineLogicalIdx,
			offset = 0, charWidths = this.bidiMap.visualCharWidths;
		...
		if((isSegmentStart && charWidths[lineVisualIdx] < 0)
			|| (!isSegmentStart && charWidths[lineVisualIdx] > 0)) {
				offset += Math.abs(charWidths[lineVisualIdx]);
				offset += this.bidiMap.bordersWidths[lineVisualIdx] || 0;
		}
		return (offset/BidiUtils.RATIO);
	},
	...
	getSelectionBoundaries: function(startLogicalIdx, endLogicalIdx) {
		var lineOffset = this.container.getFirst().getLeft(), carriageWidth = 0, selBoundaries = [];

		if(this.isRtlDir) {
			var carriageNode = dojo.query(".carriageNode", this.domNode);
		...
		if((!this.isRtlDir && (isStartCharBidi || isEndCharBidi))
			|| (this.isRtlDir && (!isStartCharBidi || !isEndCharBidi))) {
	...
	getElementPathVisual:function(x, y, h, path, options ){
		...
		x = x * BidiUtils.RATIO;
		var logIdx, visIdx = 0, offset = 0, len = this.bidiMap.visualCharWidths.length,
		...
		/* None Bidi character on the left and Bidi character on the right */
			logIdx = 1 + ((x > offset) ? this.bidiMap.logicalFromVisual[visIdx]
					: this.bidiMap.logicalFromVisual[visIdx-1]);
		}else if((this.isRtlDir && visIdx == len-1 && charWidth == 0 && this.bidiMap.visualCharWidths[visIdx-1] > 0)
				|| (!this.isRtlDir && visIdx == 0 && this.bidiMap.visualCharWidths[visIdx] < 0)){
		...
		var run = this.container.getFirst(), len = run.len;
		...
		path.push(run);
		var ret = {"delX":(offset-x)/BidiUtils.RATIO,"delY":run._getDOMOffsetTop()-y,"index":logIdx,"h":run.getContentHeight()};
		path.push(ret);
		return true;
	},
	...
	getElementPath:function(x,y,path,options,fromException){
		...
		if(!tarRun){
			...
			while(tarRun && (tarRun.isAnchor || tarRun.getViewType() == "bookMark")) {
				tarRun = this.container.prev(tarRun);
			}
...
common.tools.extend(writer.view.Line.prototype,new writer.view.AbstractView());