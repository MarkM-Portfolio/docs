dojo.provide("writer.view.SpaceBlock");

writer.view.SpaceBlock = function(){
};

writer.view.SpaceBlock.prototype = {
	_DEBUG:		false,
	bodySpace:	null,
	...
	occupy: function(space, notAffectSpace){
		...
		while (body)
		{
			if (body != this)
			{
				...
				var newSpace = new common.Space(space.w, space.h, x, y, space.owner);
	...
	getSpaceElementPath:function(x,y,path, options){
		...
		var anchoredObj = this.bodySpace.getOccupiedObject(x, y);
	...
	{
		if (this._spaceONode)
			dojo.destroy(this._spaceONode);
	
		this._spaceONode = dojo.create("div", {"class": "BodyOcuppied"}, this.domNode);
	...
	{
		if (this._spaceCNode)
			dojo.destroy(this._spaceCNode);
	
		this._spaceCNode = dojo.create("div", {"class": "BodyContained"}, this.domNode);
