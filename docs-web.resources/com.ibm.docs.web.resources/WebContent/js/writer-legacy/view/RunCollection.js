dojo.provide("writer.view.RunCollection");
...
dojo.declare("writer.view.RunCollection", null, {


	//--------------------------------------------------
	// members
	//--------------------------------------------------
	isRunCollection: function() { return true; },
	...
	{
		var tagPos = { index: -1, runView: null, textIndex: -1 };
		...
		for (var i = 0; i < this._collection.length; ++i)
		{
			...
			else
			{
				...
				if (writer.util.ViewTools.isInlineDrawingObj(run))
				{
					if (0 == i && (1 == forceFit || 2 == forceFit || 3 == forceFit))
						tagPos.textIndex = 1;
					else
						tagPos.textIndex = 0;
				}
	...
	_getSplitPos: function(w, forceFit)
	{
		...
		else if (writer.util.ViewTools.isInlineDrawingObj(tagPos.runView))
		{
		...
		else
		{
			...
			var pos = concord.i18n.LineBreak.findLineBreak(txt,txt.length);
			if(pos > 0)
				return tagPos;
			else
			{
...
//			return tagPos;
	...
	_splitAt: function(i, indexInRun)
	{
		...
		if (!newRun)
		{
			...
			var newCllct = new writer.view.RunCollection(newArray);
			this.init();
			newCllct.init();
			return newCllct;
		}
