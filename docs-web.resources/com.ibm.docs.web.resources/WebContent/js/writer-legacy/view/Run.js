...
//			prev = parentSibling && parentSibling.container.getLast();
	...
	_getBorderSize:function()
	{
		...
		if(border){
			...
			var bdrSize = Math.ceil(common.tools.toPxValue(borderSize));
	...
	getChildPosition:function(idx, placeholderArg, bNeedLogicalPos){
		...
		if(idx == 0 && !(line && line.isBidiLine())){
			var prev  = writer.util.RangeTools.getPrevRun( this );
			if (prev && prev.parent == line && writer.util.ViewTools.isTextRun(prev))
			{			
		...
		var x = (!bNeedLogicalPos && line && line.isBidiLine()) ?
			line.container.getFirst().getLeft() + this._logicalIndexToOffset(idx, line)
				: this.getLeft() + this._indexToOffset(idx);

		var y = this.getTop();
		var isItalic = this.getComputedStyle()["font-style"] == "italic";	
		return {'x':Math.ceil(x),'y':y, "italic":isItalic, "bColor": writer.util.HelperTools.getCursorColor(bColor, bCalcCursoColor), "h" : this.getHeight()};
	},
	/**
	...
	{
		return (this.len == 0 || this.getViewType() == "text.LineBreak");
	},
	getHeight:function(){
		...
		if(this._isDummyRun())
		{
			...
			if(sibling && !sibling._inCalcHeight && sibling.getParent() == this.getParent() && !vTools.isAnchor(sibling) && !vTools.isImage(sibling))
			{
				...
				return this.getParent().h;//fix issue 41076, not use method getHeight
		}
		return this.h;
	},
	...
	getLastBreakPoint: function(){
		...
		var brkPos = concord.i18n.LineBreak.findLineBreak(txtOriginal, txtOriginal.length>this.len?this.len:txtOriginal.length);
		if (brkPos <= 0)
			return -1;
		return brkPos;
	},
	...
	preMeasure:function()
	{
		common.MeasureText.preMeasureText(this.getText(), this.getCSSStr());
	},
	...
	layout:function(line){
		...
		if(this.model.isListRun && text.length == 1)
		{
			...
			if(concord.util.browser.isMobile() || (dojo.isMac && computedStyle["font-family"] && computedStyle["font-family"].toLowerCase() == symbolFont))
			{
				var fontfamily = computedStyle["font-family"].toLowerCase();
				var fallback = concord.util.mobileUtil.getFallbackFont(text, fontfamily);
				text = fallback.newText;
				cssStr = cssStr.replace(computedStyle["font-family"], fallback.newFont);
			}
		}	
			
		if(text=="" || !text || this.getComputedStyle['display']=="none"){
			size = common.MeasureText.measureTextRun("Q", cssStr);
			this.w = 0;
			this.h = size.h;
			this._emptyRun = true;
		}else{
			size = common.MeasureText.measureTextRun(text, cssStr);
			this.w = size.w;
			this.h = size.h;
			if(this.model.parent.parent.isWaterMarker)
			{
				...
				this.mw = this.w;
				var size20 = common.MeasureText.measureTextRun(text, cssStr);
				this.mw_20 = size20.w;
				this.mh_20 = size20.h;
				
				var parentModelW = common.tools.toPxValue(this.model.parent.parent.width);
				var parentModelH = common.tools.toPxValue(this.model.parent.parent.height);
				...
				size20 = common.MeasureText._getRunSize(text, cssStr);
	...
	{
		this._appendLeftMargin = 5- this.w;
		this.w = 5;
	},
...
//	},
	...
	{
		// Defect 45521 
		if(this.justifyCnt > 0 && (dojo.isSafari || dojo.isIE <= 9)){
			var innerHTML = "";
			
			var justifyChar = '\u00a0';
			var pos = 0, wordIdx = 0, word;
			...
			return common.MeasureText.measureHTMLWidth(innerHTML, style);
		}
		else
		{
			var tm = common.MeasureText.measureTextRun(text, style);
			var width = tm.w;
	...
	_offsetToIndex: function(offset, forceFit) {
		...
		if (index >= 0) {
			...
			var measureTools = common.MeasureText;
...
//				} else {
					...
					var brkPos = concord.i18n.LineBreak.findLineBreak(txtOriginal, currentIndex);
					if (brkPos <= 0)
					{
						if (forceFit == 1)
						{
...
//		var sizechar = common.MeasureText.measureTextRun(sub, this.getComputedStyle())
...
//			var tm = common.MeasureText.measureTextRun(text, this.getCSSStr(),byWord);
//			return tm.w + this.leftBorderWidth;
	...
	_logicalIndexToOffset: function(logicalIndex, line) {
		var i, offset = 0, lineVisualIdx, charWidths = line.bidiMap.visualCharWidths,
			lineLogicalIdx = logicalIndex, run = this.previous();
		while(run){
		...
		offset = offset/BidiUtils.RATIO;
		if(line.isRtlDir) {
			var carriageNode = dojo.query(".carriageNode", line.domNode);
...
//				return false;
	...
	splitByIndex: function(index) {
		...
		this.markRelayout();
		var newRun = new writer.view.Run(this.model,this.getOwnerId(), this.start+index,this.len-index);
		this._sizeIndexCache = {};
	...
	isWordBreak: function(text, index) {
		return common.tools.isWordBreak(text, index);
	},
...
//	},
	_checkAndSetSpellcheckMisword: function() {
		...
		if (typeof this.model.text == 'undefined' && dojo.isArray(miswords) && miswords.length > 0) {
			for (var i = 0; i < miswords.length; i++) {
				...
				if (!(this.start >= (misword_start + misword_len)) &&
					!((this.start + this.len) < (misword_start + 1))) {
					var cls = dojo.attr(this.domNode, "class");
					dojo.attr(this.domNode, "class", cls + " misspellWord");
					dojo.attr(this.domNode, "misword", miswords[i].word);
				}
			}
		}
	},
	_calculateLeftMargin:function(){
		...
		if (writer.util.ViewTools.isBookMark(this))
			return this._leftMargin;
	...
	_wrapSpanForSuperSub:function(){
		...
		if(verticalAlign=="super"||verticalAlign=="sub"){
			...
			if(dojo.isFF)
				dojo.style(wrapSpan,"verticalAlign",'baseline');				
			else
				dojo.style(wrapSpan,"vertical-align",'baseline');
			...
			if(indicator)
			{
				...
				if(indicator.length > 0 )
					dojo.removeClass(this.domNode, indicator);
			}
	...
	_calcJustifyWidth:function(){
		var justifyChar = '\u00a0';
		var size = common.MeasureText.measureTextRun(justifyChar, this.getCSSStr());
	...
	_createChildrenDomImpl:function(parentNode){
		...
		var scdata = this.model.paragraph.scdata;
		var miswords = scdata.misWords;
		var text = this.getText(), start = this.start, len = this.len;
		...
		for (var i = 0; len > 0 && i < miswords.length; i++) {
			...
			dojo.attr(node, "misword", miswords[i].word);
	...
	_createChildrenDom:function(parentNode){
		...
		// skip symbol list text run
		if ( hasMisWord && !(typeof this.model.text == 'undefined' && dojo.isArray(miswords) && miswords.length > 0) )
			hasMisWord = false;
		...
		if(this.justifyCnt > 0)
		{
			...
			var misWordIndex = {"idx" : 0};
			var start = 0, pos, alignmentNode;
			var subText;
	...
	handleLinkField: function()
	{
		...
		var linkObj = writer.util.ModelTools.getLink(this.model.parent);
		if(!linkObj && p &&  p.modelType == writer.MODELTYPE.FIELD && p.isPageRef())
			linkObj = p;
		else if( !linkObj && writer.util.ModelTools.isImage(this.model) && this.model.src && this.model.src!="")
			linkObj = this.model;
		var fieldObj = writer.util.ModelTools.getField(p);
		if( linkObj || fieldObj || writer.util.ModelTools.hasComments(this.model))
			dojo.publish( writer.EVENT.DOMCREATED,[this.model, this.domNode, this, {"linkObj":linkObj, "fieldObj":fieldObj} ] );
	},
...
//		this._checkAndSetSpellcheckMisword();
	...
	_updateLeftMarginDom: function()
	{
		...
		if(leftMargin!=oldV){
			if(leftMargin > 0){
				dojo.style(this.domNode,{"marginLeft":leftMargin+"px"});
			}else{
				var line = writer.util.ViewTools.getLine(this);
				...
				if(this == lstSmb)
					...
					dojo.style(this.domNode,{"marginLeft":null});
			}
	...
	getComputedStyle:function(){
		if(!this._cacheComputedStyle){
			this._cacheComputedStyle =  this.model.getComputedStyle();
			this._cacheComputedStyleFontFallbacked = false;
		}			
		if (this._cacheComputedStyle && this._cacheComputedStyle["font-family"] && !this._cacheComputedStyleFontFallbacked)
		{
			...
			var fontFamily = writer.util.FontTools.fallbackFonts(orig);
			if (fontFamily != orig)
			{
				this._cacheComputedStyle = dojo.clone(this._cacheComputedStyle);
	...
	getCSSStr: function(){
		...
		{	
			var computedStyle = this.getComputedStyle();
			this._cacheCSSStr = common.MeasureText.getCSSStr(computedStyle);
		}
...
//			style['background-color'] = computedStyle['background-color'];
		...
		if(computedStyle['color'] && (computedStyle['color']=='autoColor' || computedStyle['color']=='auto'))
			style['color'] = 'black';
		if( computedStyle['color'] && this.model.getStyleId() == "Hyperlink" &&  writer.util.ModelTools.isInToc( this.model ))
		{
		...
		if(!backgroundColor){
			// fix issue 43354
			var textboxParent =  writer.util.ViewTools.getTextBox(this);
			if(textboxParent)
				backgroundColor = textboxParent.model.bgColor;
		}
		if(backgroundColor && writer.util.HelperTools.isNeedChangeColor(backgroundColor))
			style['color'] = 'white';	
		
		var str = "";
		for(var n in style){
			if(n == "t" || n == "bdr" || n == "styleId" || !style[n]) continue;	// Ignore t:rPr; bdr, styleId
			if(n == "font-family" && this._cacheComputedStyleFontFallbacked)
				str += n+":"+writer.util.FontTools.fallbackFonts(style[n])+";";
...
//			str += "background-color:rgba(255,255,0,0.5);";
	...
	getElementPath:function(x,y,h,path,options){
		...
		var run={"delX":fixedX-x,"delY":this._getDOMOffsetTop()-y,"index":index,"h":this.getContentHeight()};
		path.push(run);
		
	},
	_getDOMOffsetTop:function(){
		var page = writer.util.ViewTools.getPage( this );
		if( !this._offsetTop && ( !this.domNode || !page || !page.domNode ) )
		...
		if(this._offsetTop==undefined)
		{
			if(this._emptyRun){
				...
				if(dojo.isIE && !writer.util.ViewTools.isTextBox(this))	// Defect 44280 for text box.
				{
					var style = this.getStyle();
					var verticalAlign = style['vertical-align'];
					if(verticalAlign=="super"||verticalAlign=="sub")
...
writer.model.Model.prototype.viewConstructors[writer.MODELTYPE.TEXT]=writer.view.Run;
common.tools.extend(writer.view.Run.prototype,new writer.view.AbstractView());
