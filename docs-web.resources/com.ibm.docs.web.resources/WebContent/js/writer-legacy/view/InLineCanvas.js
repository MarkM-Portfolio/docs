dojo.provide("writer.view.InLineCanvas");
dojo.require("writer.view.ImageView");
dojo.require("concord.util.browser");
dojo.require("writer.view.selection.CanvasResizerView");

dojo.declare("writer.view.InLineCanvas", [writer.view.ImageView, writer.view.AbstractCanvas], {
	className: "InLineCanvas",
	
	_createResizer: function()
	{
		this._resizerView = new writer.view.selection.CanvasResizerView(this);
	},
	...
	render: function(parentChange)
	{
		if(!this.domNode ||this.dirty){
			...
			var style = "position:absolute;left:0px;bottom:0px;"
	...
	getElementPath:function(x,y,lineHeight,path,options){
		...
		else if(concord.util.browser.isMobile())
		{
			var run={"index":0,"isInside":true};
			path.push(run);
		}
	}
});

writer.model.Model.prototype.viewConstructors[writer.MODELTYPE.CANVAS] = writer.view.InLineCanvas;
