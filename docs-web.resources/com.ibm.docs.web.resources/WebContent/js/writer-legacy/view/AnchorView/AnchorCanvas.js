dojo.provide("writer.view.AnchorView.AnchorCanvas");
dojo.require("writer.view.selection.AnchorCanvasResizerView");

dojo.declare("writer.view.AnchorView.AnchorCanvas", [ writer.view.AnchorView.AnchorImageView, writer.view.AbstractCanvas], {
	className: "AnchorCanvas",

	_createResizer: function()
	{
		this._resizerView = new writer.view.selection.AnchorCanvasResizerView(this);
	},
		...
		if(!this.domNode ||this.dirty){
			...
			this.borderNode = dojo.create("div", {
				...
				"style": "position:absolute;left:" + this.padLeft + "px;bottom:" + this.padBottom + "px;"
					+ "width:" + this.iw + "px; height:" + this.ih + "px;background-color:rgba(1,1,1,0)"
				},this.domNode);
			...
			if (this._maskLeft > 0 || this._maskRight > 0 || this._maskTop > 0 || this._maskBottom > 0)
				maskHidden = "hidden";
			this.maskNode = dojo.create("div",{"class":"canvas_mask", "style": "position:absolute;overflow:" + maskHidden + ";" +
			...
			// canvas node
			var style = "position:absolute;left:"+(-this._maskLeft)+"px;bottom:"+(-this._maskBottom)+"px;"
			...
			this.canvasNode = dojo.create("div", {
				"class":"canvas_node",
				"style" : style
			}, this.maskNode);
