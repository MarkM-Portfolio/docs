dojo.provide("writer.view.selection.ResizerView");
...
dojo.declare("writer.view.selection.ResizerView",
null, {
	...
	// static variable
	_static: {selectedModelID: null}, // this static variable contains current selected model id
	
	// tools
	_getPxValue: common.tools.toPxValue,
	...
	_isInCorrectEditMode: function()
	{
		...
		if (shell)
		{
			...
			var header = writer.util.ViewTools.getHeader(this._ownerView);
			var footer = writer.util.ViewTools.getFooter(this._ownerView);
			if (editorMode == EDITMODE.HEADER_FOOTER_MODE)
			{
				if (!header && !footer)
					return false;
			}
			else if (editorMode == EDITMODE.EDITOR_MODE)
			{
				if (header || footer)
					return false;
			}
	...
	_onIndicatorDown: function(event) {
		...
		var mainNode = concord.util.browser.getEditAreaDocument();
		
		this._docConnects.push( dojo.connect(mainNode, "onmousemove", this, "_onIndicatorMove") );
		this._docConnects.push( dojo.connect(mainNode, "onmouseup", this, "_onIndicatorUp") );
	...
	_onIndicatorUp: function(event) {
		...
		{
			dojo.disconnect(this._docConnects[i]);
		}
	...
	_drawResizer: function(left, bottom, width, height) {
		...
		*/
		...
		if(!node)
			return;
		dojo.style(node, "left", (left - this._RESIZER_BORDER_SIZE) + "px");
		dojo.style(node, "bottom", (bottom + height) + "px");
		
		// top-right
		node = this._domNodes2;
		dojo.style(node, "left", (left + width) + "px");
		dojo.style(node, "bottom", (bottom + height) + "px");
		
		// bottom-left
		node = this._domNodes5;
		dojo.style(node, "left", (left - this._RESIZER_BORDER_SIZE) + "px");
		dojo.style(node, "bottom", (bottom - this._RESIZER_BORDER_SIZE) + "px");
		...
		dojo.style(node, "left", (left - this._RESIZER_BORDER_SIZE) + "px");
		dojo.style(node, "bottom", (bottom + (height - this._RESIZER_BORDER_SIZE) * 0.5 + 0.5) + "px");
		...
		dojo.style(node, "left", (left + (width - this._RESIZER_BORDER_SIZE) * 0.5 + 0.5) + "px");
		dojo.style(node, "bottom", (bottom + height) + "px");
	...
	_getInc: function(offsetX, offsetY) {
		...
		// lock aspect
		if ((0 == this._moveNodeIndex || 2 == this._moveNodeIndex || 5 == this._moveNodeIndex || 7 == this._moveNodeIndex)
			&& this._ownerView.isAspectLocked && this._ownerView.isAspectLocked())
		{
			var viewSize = this._getViewSize();
	...
	_displayAnchor: function(isDisplay)
	{
		...
		{
			this._anchorIndicatorDom = dojo.create("div", {	"class": anchorClass });
	...
	_addIndicator: function()
	{
		...
		for (var i = 0; i < 8; ++i) {
			...
			this._connects.push( dojo.connect(node, "onmousedown", this, "_onIndicatorDown") );
	...
	select: function()
	{
		...
		this._listeners.push( dojo.subscribe(writer.EVENT.REMOVE_SELECT, this, this.unSelect));
	...
	unSelect: function(ranges, beforeUpdate) {
		...
		for (var i = 0; i < this._listeners.length; i++) {
			dojo.unsubscribe(this._listeners[i]);
		}
