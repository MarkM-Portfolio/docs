 ...
 * 
 */
dojo.provide("writer.plugins.ContextMenu");
dojo.require("writer.plugins.Plugin");
dojo.require("writer.ui.menu.Menu");
dojo.require("concord.util.BidiUtils");
dojo.declare("writer.plugins.ContextMenu",
...
{		
	...
	_addInfoItems2Menu : function(itemobjs, parent) {
		...
		var lastGroup = -1,newMenuItem = null, subWidget = null, itemobj,i;
		...
		for ( i = 0; i < itemobjs.length; i++) {
			...
			if (groupCfg[itemobj.group] !== lastGroup && lastGroup !== -1) {
				...
				if(!nextSprt){//the menu separator need to be created
					var newSprt = new dijit.MenuSeparator();
			...
			if(itemobj.subWidget){//with sub items, handle the sub items
				if(isCreated){
					...
					switch(itemobj.subWidget){
						case "menu":{						
							subWidget = new writer.ui.menu.Menu({dir: dirAttr});
							break;
						}
						case "liststyle":{					//list style widget.
							subWidget = new writer.ui.widget.ListStyle();
							break;
						}
						case "tables":{
							break;
						...
						case "color":{
							subWidget = new writer.ui.widget.ColorPalette(itemobj.args);
							break;
						}
						default:subWidget=itemobj.subWidget;break;
					}
					this._menuItems[itemName].sub = subWidget; 
					itemobj.popup = subWidget;
					itemobj.dir = dirAttr;
					newMenuItem = new dijit.PopupMenuItem(itemobj);
				}
				...
				if(!isCreated){
					...
					if (itemobj.isCheckedMenu)
						newMenuItem = new dijit.CheckedMenuItem(itemobj);
					else
						newMenuItem = new dijit.MenuItem(itemobj);
				}
			}
	...
	_resetCtxMenu : function(){
		for(var i in this._menuItems){
			...
			menuItem.menu && dojo.style(menuItem.menu.domNode, "display", "none");
		}
		for(var j in this._menuSeparators){
			var menuSprt = this._menuSeparators[j];
			dojo.style(menuSprt.domNode, "display", "none");
		}
	},
	/**
	...
	_setMenuItemDisplay : function(menu){
		dojo.style(menu.domNode, "display", "");
	}
	,
	/**
	...
	removeMenuItems : function(type) {		
		var contextMenu = dijit.byId('ctx');		
		for(var i in this._menuItems){
	...
	show : function(target, selection, x, y) {
		var listenerItems=[], listeners = this.listeners, closelisteners = this.closeListeners, menuId='ctx';
		var contextMenu = dijit.byId(menuId);
		
		//create only when initializing
		if(!contextMenu){
			contextMenu = new writer.ui.menu.Menu({
			id : menuId,
			dir: BidiUtils.isGuiRtl() ? 'rtl' : ''
			});
		}
		dijit.popup.moveOffScreen(contextMenu);
		...
		listenerItems.sort(function(a,b){
			...
			var orderA =  orders[a.itemInfo.order], orderB = orders[b.itemInfo.order],
				groupA =  groups[a.itemInfo.group], groupB = groups[b.itemInfo.group];
		...
		dojo.publish(writer.EVENT.CANCELRESIZE);
		contextMenu.focus();
		setTimeout(function(){
