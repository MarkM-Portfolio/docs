...
dojo.provide("writer.plugins.Comments");
dojo.require("writer.plugins.Plugin");
dojo.declare( "writer.plugins.Comments",
[writer.plugins.Plugin], {	  
	...
	addContextMenu: function(){
		...
		if(ctx && ctx.addListener) ctx.addListener(function(target,selection){
			...
			if(bAdd)
			{
				ret.toggleCommentsCmd = {disabled:false};
				pe.lotusEditor.getCommand('toggleCommentsCmd').setState(writer.TRISTATE_OFF);
				return ret;
			}else{
				pe.lotusEditor.getCommand('toggleCommentsCmd').setState(writer.TRISTATE_DISABLED);
				return;
			}
