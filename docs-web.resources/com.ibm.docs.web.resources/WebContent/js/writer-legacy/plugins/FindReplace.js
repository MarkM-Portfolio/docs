dojo.provide("writer.plugins.FindReplace");
dojo.require("dojo.fx");
dojo.require("writer.plugins.Plugin");
dojo.require("writer.ui.dialog.NewFindReplaceDlg");
 ...
 * 
 */
dojo.declare( "writer.plugins.FindReplace",
				...
				var kmpPartMatchArray = function(targetStr){
					...
					var kmpFinder = function(para,sourceStr,targetStr){
					     ...
					     for(i=0,j=sourceStr.length;i<=j-n;i++){
					         ...
					         if (m == n){
					        	 ...
					        	 while(sourceArray[start])
					        	{
					        	  ...
					        	  results.push({'start':start,'end':start+targetLength});
					         }
					     }
					     delete sourceArray;
					     var newResults = [],result = null;
					     for(var k = 0; k < results.length; k++)
					     {
					    	 result = results[k];
					    	 if(newResults.length == 0 || (result.start  >= newResults[newResults.length-1].end ))
					    		 newResults.push({'start':result.start,'end':result.end});
					 ...
					 var normalizeAllParas = function(){
						 var para, idx = 0, doc = pe.lotusEditor.document ;
						 ...
						 if(doc.modelType == writer.MODELTYPE.HEADERFOOTER)
						 {
					 ...
					 var getModelsFromPara = function(para,targetStr,matchCase,matchWord,view){
						 var sourceStr = para.text;
						 para.clearMatchModels();
						 if(!matchCase)
						 {
							 sourceStr = sourceStr.toLowerCase();
							 // fix 45950
							 if(dojo.isChrome && sourceStr.length != para.text.length){// do matchCase find
								 sourceStr = para.text;
							 }else{
						 ...
						 var isBreak = common.tools.isWordBreak;
						 // Find match words
						 ...
						 for(var i=0; i < results.length; i++)
						 {
							 ...
							 if(!matchWord || ((result.start == 0 || isBreak(sourceStr, result.start-1)) && (sourceStr.length == result.end || isBreak(sourceStr, result.end))))
						 	 {
								 findResult.push({'para':para,'start':result.start,'end':result.end, 'view':view});//normalize run range by focus start index
					 ...
					 var modelToRange = function(model){
						 var start = {"obj": model.para, "index": model.start };
						 var end = {"obj": model.para, "index": model.end };
						 return new writer.core.Range( start, end, model.view );
					 };
					 var modelsToRanges = function(models){
						 var ranges = [];
						 var model = null, start, end, curPageNum;
						 if(finder.views)
						 {
					 ...
					 var handleReplaceFinished = function(){
						 ...
						 {
							 dojo.publish(writer.EVENT.SHOWTOTALNUM, [finder.focusIndex, finder.matchBeforeFocus, finder.matchModels.length]);
							 if(finder.replacedMatchModels.length > 0)
							 {
...
//								 models = models.concat(getModelsFromPara(objs.paras[j],targetStr,matchCase,matchWord,objs.view));
						 ...
						 return {"models":models, "matchBeforeFocus":matchBeforeFocus};
					 };
					 ...
					 var finder = {
								...
								find : function( targetStr, matchCase, matchWord, highlightAll , scroll )
								{
									...
									if(this.state == KMP_NOTFOUND)
									...
									else if(this.state == KMP_NOSTARTTOMATCH )
								 	{
										...
										dojo.publish(writer.EVENT.SHOWTOTALNUM, [this.focusIndex, this.matchBeforeFocus, this.matchModels.length]);
								...
								alert : function(msg){
									var callback = function(){
										dojo.publish(writer.EVENT.FOCUSBACKFINDDLG);
									};
									concord.util.dialogs.alert( msg, callback,{"editorFocusNotback": true});
								},
								/**
								...
								replace : function( newStr, matchCase, matchWord )
								{
										...
										// request spell check
										dojo.publish(writer.EVENT.REQUESTSPELLCHECKONPARA, [currModel.para, newStr, currModel.start, true]);
									...
									try{
										...
										while(this.matchModels.length > 0)
										{
											try{
												...
												// request spell check
												if (shiftCount == 0) {
													dojo.publish(writer.EVENT.REQUESTSPELLCHECKONPARA, [para, newStr, startindex, true]);
												}
										...
										var info = dojo.string.substitute(nls.replaceSuccessMsg, { 'replacedCount' : totalCount });
										this.alert(info);
										dojo.publish(writer.EVENT.SHOWTOTALNUM, [this.focusIndex, this.matchBeforeFocus, this.matchModels.length]);
										dojo.publish(writer.EVENT.GROUPCHANGE_END);
									}
				...
				var local_finder = {
						...
						resize_hdl : function(){
							...
							if (!this.show){
								dojo.style(editFrame,"height","100%");
							}
							else{
								var mainHeight = dojo.style("mainNode","height");
								var height = dojo.marginBox(dojo.byId("lotus_editor_floating_finder")).h;
							    var editHeight = mainHeight - height;
								dojo.style(editFrame,"height",editHeight + "px");
							}
						...
						onshow_hdl : function()
						{
							this.show = true;
							this.resize_hdl();
							dojo.fx.wipeIn({ node: dojo.byId("lotus_editor_floating_finder") }).play();
							this.editAreaChangedHandler = dojo.subscribe(writer.EVENT.EDITAREACHANGED, this, this.findForEditAreaChanged),
						...
						onhide_hdl : function()
						{
							...
							{
								dojo.unsubscribe(this.editAreaChangedHandler);
						...
						navigate_hdl : function(direction)
						{
							...
							{
								finder.find(this.input.value, dijit.byId("D_t_MatchCase").attr("checked"),dijit.byId("D_t_MatchWord").attr("checked"),true, true);
								if (finder.state == KMP_NOTFOUND){
									finder.alert(nls.notFoundMsg);
									return;
								}
...
//								return;
						...
						find_hdl : function(findForCheckBox)
						{
							...
							{
								dojo.publish(writer.EVENT.SHOWTOTALNUM, [finder.focusIndex, finder.matchBeforeFocus, -1]);// clear the totol num text
								return;
							}
							else
							...
							if (pe.lotusEditor.findTimer)
								clearTimeout(pe.lotusEditor.findTimer);
							pe.lotusEditor.findTimer = setTimeout(dojo.hitch(finder, finder.find,this.input.value,
									dijit.byId("D_t_MatchCase").attr("checked"),dijit.byId("D_t_MatchWord").attr("checked"),true,
									true),findTimeLapse);
						},
						...
						replace_hdl : function()
						{
							...
							try{
								...
								if(this.isDefaultReplaceValue)
									replaceWord = "";
								finder.replace( replaceWord, dijit.byId("D_t_MatchCase").attr("checked"),dijit.byId("D_t_MatchWord").attr("checked"));
						...
						replaceall_hdl : function()
						{
							...
							}else
							{
								...
								if(this.isDefaultReplaceValue)
									replaceWord = "";
								finder.replaceAll( replaceWord, dijit.byId("D_t_MatchCase").attr("checked"),dijit.byId("D_t_MatchWord").attr("checked"));
							}
						}
					};
				this.editor.addCommand( 'find',
						{ exec : function(){
							writer.ui.dialog.NewFindReplaceDlg.show("find",local_finder);
						  }
						} ,writer.CTRL + 70); // Ctrl + F
							
					this.editor.addCommand( 'replace',
							{ exec : function()
								{
								writer.ui.dialog.NewFindReplaceDlg.show("replace",local_finder);
								}
							},writer.CTRL + 82 );//Ctrl + R
		}
});