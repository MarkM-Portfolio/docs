...
dojo.provide("writer.plugins.SpellCheck");
dojo.require("writer.plugins.Plugin");
dojo.require("concord.spellcheck.scaytservice");

dojo.declare("writer.plugins.SpellCheck", [ writer.plugins.Plugin ], {
	init : function() {
		...
		if (typeof spellcheckerManager != 'undefined') {
			dojo.mixin(spellcheckerManager, {
				...
				removeMiswordsInRange : function(misWords, range_start, range_end ){
					var ret = false;
					if (!dojo.isArray(misWords) || misWords.length == 0) return ret;
					if (range_start >= range_end) return ret;
					var delidx_start = 0, delidx_end = 0;
				...
				mergeTwoMiswordsArray: function (miswords, miswords2) {
					if (!dojo.isArray(miswords2)) return;
					var i = 0;
		...
		dojo.subscribe(writer.EVENT.LOAD_READY, checker, checker.onLoadReady);

		return;
	}
});

dojo.declare("concord.spellcheck.writerscaytservice", [concord.spellcheck.scaytservice], {
	...
	init : function(editor) {
		...
		this._scParaArray = [];
		this.nls = dojo.i18n.getLocalization("concord.spellcheck", "spellcheckUI");
	...
	onLoadReady : function () {
		...
		if(ctx && ctx.addListener){
			ctx.addListener(dojo.hitch(this, "onContextMenu"));
			ctx.addCloseListener(dojo.hitch(this, "onContextMenuClose"));
		}
	...
	_collectAllPara : function (para, data) {
		   if (para.text && para.text.length /*&& para.text != "\r"*/) {
	...
	doSpellCheckOnDocument : function() {
		...
		var data = {para_array:[]};
		this._traverseDocumentParagraph(dojo.hitch(this,this._collectAllPara), data);
		
		this._scParaArray = this._scParaArray.concat(data.para_array);
		
		if (this._scDocumentTimer == null)
			this._scDocumentTimer = setInterval(dojo.hitch(this,this._checkInterval), this._CHECK_INTERVAL);
	},
				...
				if (para && para.text.length > 0) {
					var sc_start = (para.scdata && para.scdata.scrange_start > 0) ? 
				        ...
				        sc_end = (para.scdata && para.scdata.scrange_end > 0 && 
				        		  para.scdata.scrange_end < para.text.length) ? 
				        			para.scdata.scrange_end: para.text.length;
				        sc_start = spellcheckerManager.getPreviousSep(para.text, sc_start);
				        sc_end = spellcheckerManager.getNextSep(para.text, sc_end);
				    var thislen = sc_end - sc_start;

					if (thislen >= this._REQUEST_TEXTLEN && len_s > 0)
						break;
					else {
						this._scParaArray.shift(); array_len--;
						if (!para.scdata) para.scdata = {};

						if (thislen > 0) {
							if (!sc_text) {
								len_s += thislen;
								sc_text += para.text.substring(sc_start, sc_end);
							} else {
								len_s += thislen + 1;
								sc_text += '\r' + para.text.substring(sc_start, sc_end);
							}
							para.scdata.checking = {};
							para.scdata.checking.start = sc_start;
							para.scdata.checking.end = sc_end;
							para.scdata.checking.state = 0;
							sc_paras.push({para:para, range:para.scdata.checking});
						}
					}
				} else if (para) {
					para.scdata= {};  // mark this paragraph as checked
	...
	_traverseAParagraph : function(para, cb, data) {
		...
		while (child) {
			if (child.modelType == writer.MODELTYPE.TXBX || child.modelType == writer.MODELTYPE.FLTXBX ||
				child.modelType == writer.MODELTYPE.SQTXBX || child.modelType == writer.MODELTYPE.TBTXBX ||
				child.modelType == writer.MODELTYPE.RFOOTNOTE || child.modelType == writer.MODELTYPE.RENDNOTE) {
				var grandchild = child.firstChild();
				while (grandchild) {
					if (grandchild.modelType == writer.MODELTYPE.PARAGRAPH)
	...
	_traverseTableParagraph : function (table, cb, data) {
		table.rows.forEach(dojo.hitch(this, function(row) {
			row.cells.forEach(dojo.hitch(this, function(cell){
				cell.container.forEach(dojo.hitch(this, function(obj){
	...
	_traverseDocumentParagraph : function (cb, data) {
		...
		while (child) {
		   if (child.modelType == writer.MODELTYPE.PARAGRAPH) {
			   this._traverseAParagraph(child, cb, data);
		   } else if (child.modelType == writer.MODELTYPE.TABLE) {
			...
			if (rels._relations) {
				for (var key in rels._relations) {
					if (rels._relations.hasOwnProperty(key)) {
						...
						if (relobj.modelType == writer.MODELTYPE.HEADERFOOTER) {
							var relchild = relobj.firstChild();
							while (relchild) {
							   if (relchild.modelType == writer.MODELTYPE.PARAGRAPH) {
				...
				dojo.forEach(props, function (prop) {
					...
					if (notes) {
						dojo.forEach(notes, function(note){
							...
							while (notechild) {
							   if (notechild.modelType == writer.MODELTYPE.PARAGRAPH) {
	...
	doSpellcheckOnNewParagraph : function (para) {
		...
		if (para.parent && para.parent.modelType == writer.MODELTYPE.TOC)
			return;
	...
	doSpellcheckOnParagraphRange: function(para, text, index, bforce) {
		...
		if (para.modelType != writer.MODELTYPE.PARAGRAPH) return;
		if (para.scdata && text != null && (typeof text != 'undefined') && 
				para.text && para.text.length) {
			...
			if (!this.needBatchCheck())
				setTimeout(dojo.hitch(this, this.checkText, para), this._SHORT_CHECK_INTERVAL);
			else
				this._enqueueARequest(para, !this.needBatchCheck());
		}
	},
	...
	_enqueueARequest : function (para, btriggertimer) {
		...
		if (this._scDocumentTimer == null && (typeof btriggertimer == 'undefined' || btriggertimer == true))
			this._scDocumentTimer = setInterval(dojo.hitch(this,this._checkInterval), this._LONG_CHECK_INTERVAL);
	},
	...
	checkText : function(para)
	{
		...
		if(strText && strText.length) {
			...
			para.scdata.checking.end = sc_end;
			para.scdata.checking.state = 0;
			paras.push({para:para, range:para.scdata.checking});
...
//				context : scRequest,
				...
				load : function (data){
								...
								if (dojo.isString(wArray)){
									if(wArray.lengh && !spellcheckerManager.isSkipWord(wArray)) {
										...
										if (!spellcheckerManager.isWordBoundaryChar(code)) {
											var misword = {word: wArray, index: bIndex};
									...
									for(var k = 0; k<wArray.length; k++)
									{
										...
										if(aWord.length && !spellcheckerManager.isSkipWord(aWord))
										{
											...
											if (!spellcheckerManager.isWordBoundaryChar(code)) {
												var misword = {word: aWord, index: bIndex};
												newmisWords.push(misword);
					...
					for (var it = 0; it < arlen; it++) {
						...
						for (; checked_misw_index < miswords_len; checked_misw_index++) {
							...
							var hint = para.byIndex(w_start, false);
							// dont mark the word when it is link text
							if (hint && hint.modelType == writer.MODELTYPE.LINK && w_start >= hint.start &&
	...
	connectEvents: function() {
		if (this.subscriptionEventhandlers.length == 0){
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.REQUESTSPELLCHECKONPARA, this, this.doSpellcheckOnParagraphRange));
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.CREATEDNEWPARA, this, this.doSpellcheckOnNewParagraph));
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.GROUPCHANGE_START, this, this.doBatchCheckStart));
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.GROUPCHANGE_END, this, this.doBatchCheckEnd));
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.LEFTMOUSEDOWN, this, dojo.hitch(this, "onMouseDown")));
			this.subscriptionEventhandlers.push(dojo.subscribe(writer.EVENT.CURSORCOMMAND,this, dojo.hitch(this, "onCursorCommand")));
		}
	},

	disconnectEvents: function() {
		dojo.forEach(this.subscriptionEventhandlers, dojo.unsubscribe);
	...
	onMouseDown : function(EditorWindow) {
		...
		if (endmodel) {
			...
			if (para && para.scdata && typeof para.scdata.scrange_start != 'undefined' && para.scdata.scrange_start >= 0) {
				...
				setTimeout(dojo.hitch(this, this.checkText, para), this._CHECK_INTERVAL);
			}
		}
	},
	...
	onContextMenu: function (target, selection, x, y) {
		...
		var cmds = {
			sc_ignore : {
				...
				order : 'spellcheck',
				name  : 'sc_ignore',
				onClick : dojo.hitch(scsug_context, this.onSuggestionIgnoreWord),
				type : "scsug_ctxmenu"
			},
			sc_skipall : {
				...
				order : 'spellcheck',
				name  : 'sc_skipall',
				onClick : dojo.hitch(scsug_context, this.onSuggestionSkipall),
				type : "scsug_ctxmenu"
			}
		};
		var ret = {sc_ignore: false, sc_skipall : false};
		...
		if (!suggestions || suggestions.length == 0) {
			...
			this.__correctall_submenu = new dijit.Menu();
			ret[cmds_correctall.name] = {subWidget: this.__correctall_submenu};
			
			var scsug_context0 = null;
			suggestions.forEach(dojo.hitch(this, function (sugg) {
				if (sugg.length > 0 ){
					scsug_context0 = {
							sc_inst : this,
							parapos : parapos,
					...
					var cmds_suggword_templ = {
							...
							order : 'spellcheck',
							name  : name,
							onClick: dojo.hitch(scsug_context0, this.onSuggestionWord),
							type : "scsug_ctxmenu"				
						};
					...
					this.__correctall_submenu.addChild(new dijit.MenuItem({
					    id: newId,
					    label: sugg,
					    onClick : dojo.hitch(scsug_context0, this.onCorrectAll)
					}));
				}
			} ));
		}
	...
	onCorrectAll : function () {
		...
		var csr_pos = {index : this.misword.index + this.misword.word.length, obj : this.parapos.obj};
		...
		var amisword = {index : this.misword.index, word: this.misword.word};
		...
		this.sc_inst._traverseDocumentParagraph(function(para, data) {
			...
			for (var i = para.scdata.misWords.length; i > 0; i-- ) {
				if (para.scdata.misWords[i-1].word == amisword.word) {
					var sindex = para.scdata.misWords[i-1].index;
					var start = {index:sindex+1, obj: para};
					var end = {index:sindex + amisword.word.length, obj: para};
					var range = new writer.core.Range(start, end, csr_rootview);
					if (range.getEndView()) {
						sel.destroy();
						...
						range = new writer.core.Range({obj:para, index:sindex}, {obj:para, index:sindex+1}, csr_rootview);
						sel.select(range.getStartView(), range.getEndView());
						shell.deleteText();
	...
	onSuggestionWord : function(data) {
		...
		if (para)
			setTimeout(dojo.hitch(this.sc_inst, this.sc_inst.correctOneMisWord, para, this.misword, this.sugg, this.rootview), 0);
		// need change focus back to editor explicitly.
		pe.lotusEditor.getShell().focus();
	},
	...
	onSuggestionIgnoreWord : function () {
		...
		var para = this.parapos.obj;
		if (para)
			setTimeout(dojo.hitch(this.sc_inst, this.sc_inst.ignoreOneMisWord, para, this.misword), 0);
	...
	onSuggestionSkipall : function () {
		...
		setTimeout(dojo.hitch(this.sc_inst, this.sc_inst.ignoreOneMisWordForAllPara, this.misword), 0);
	},
 ...
 */	
	_markMisspellByRanges : function(para, miswords) {
		...
		while (run && run.modelType != writer.MODELTYPE.TEXT)
			run = para.container.next(run);
		for (var mis_idx =  0; mis_idx < miswords.length; mis_idx++) {
			if (!run) return ret;
			var trynextrun = 1;
			var misword_start = miswords[mis_idx].index;
			var misword_len = miswords[mis_idx].word.length;
	...
	_markMisspellForRequest : function(para) {
		...
		var scdata = para.scdata;
		if (!scdata || !dojo.isArray(scdata.misWords) ||
	...
	correctOneMisWord : function (para, misword, suggword, rootview) {
		...
		var sindex = misword.index;
		var start = {obj:para, index:sindex+1};
		var end = {obj:para, index:sindex + misword.word.length};
		var range = new writer.core.Range(start, end, rootview);
		if (range.getEndView()) {
			WRITER.MSG.beginRecord();
			try{
			...
			range = new writer.core.Range({obj:para, index:sindex}, {obj:para, index:sindex+1}, rootview);
			sel.select(range.getStartView(), range.getEndView());
			shell.deleteText();
			
			var topos = {obj:para, index:sindex+suggword.length}; 
	...
	ignoreOneMisWordForAllPara : function (amisword) {
		var data = {count:1, misword: amisword};		
		var that = this;
...
// message handle
	subscribeToEvents: function(editor){
		dojo.subscribe("concordsc::scaytEnabledInfo", this, dojo.hitch(this,this.handleSpellCheckEnabled));
		dojo.subscribe("concordsc::scaytFinishedInfo", this, dojo.hitch(this,this.handleSpellCheckFinished));
		dojo.subscribe("concordsc::autoScaytEnabledInfo", this, dojo.hitch(this,this.handleAutoSpellCheckEnabled));
	},
