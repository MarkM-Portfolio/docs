...
/*                                                                   */
/* ***************************************************************** */
dojo.provide("writer.plugins.list");
dojo.require("writer.plugins.Plugin");
dojo.require("writer.plugins.ListTemplate");
dojo.require("writer.model.numberingDefinition");
dojo.require("writer.util.ListTools");
dojo.require("writer.ui.widget.SetNumberingValueDlg");
dojo.requireLocalization("concord.widgets","menubar");
dojo.requireLocalization("writer","lang");

dojo.declare("writer.plugins.list", [writer.plugins.Plugin], {
...
//					return;
	...
	{
		var template = new writer.plugins.ListTemplate();
		...
		for(var numType in this.defaultNumberings)
		{
			...
			this._cacheDefaultsDefiniton[numType] = new writer.model.numberingDefinition(numDefJson);
		}
	...
	_initTemplate: function()
	{
		...
		this.templateNumLevel = this.listTemplate.templateNumLevel;
	...
	_addContextMenu: function()
	{
		...
		if(ctx && ctx.addListener) ctx.addListener(function(target,selection){
			...
			{
				var firstPara = para.list.getParagraph(0);
				if(para.isBullet())
					return {};
				else{
					setNumberingValCmd.setState(writer.TRISTATE_ON);
					return {"setNumberingVal": true}; 
				} 	
			}		
			else 
			{
				...
				if(para.isBullet())
				{
					setNumberingValCmd.setState(writer.TRISTATE_DISABLED);
					if(firstPara == para)
					{
						separateListCmd.setState(writer.TRISTATE_DISABLED);
						joinListCmd.setState(disableStatus ? writer.TRISTATE_DISABLED : writer.TRISTATE_ON);
						return {"joinList": disableStatus}; // First item
					}
					else
					{
						separateListCmd.setState(disableStatus ? writer.TRISTATE_DISABLED : writer.TRISTATE_ON);
						joinListCmd.setState(writer.TRISTATE_DISABLED);
						return {"separateList": disableStatus }; // Other item
					}
...
//					return { "separateList": disableStatus };
				...
				else
				{
					...
					if(firstPara == para)
					{
						restartListCmd.setState(writer.TRISTATE_DISABLED);
						continueListCmd.setState(disableStatus ? writer.TRISTATE_DISABLED : writer.TRISTATE_ON);
						return {"continueList": disableStatus, "setNumberingVal": true}; // First item
					}
					else
					{
						restartListCmd.setState(disableStatus ? writer.TRISTATE_DISABLED : writer.TRISTATE_ON);
						continueListCmd.setState(writer.TRISTATE_DISABLED);
						return {"restartList": disableStatus, "setNumberingVal": true }; // Other item
					}
				}		
			}
		});
	...
	_getSiblingList : function(para, isPrevous){
		...
		while(sibling)
		{
			if(sibling.modelType == writer.MODELTYPE.PARAGRAPH)
			{
				if(sibling.isList())
				{
	...
	{
		return writer.util.ListTools.createList(numJson, msgs, abstractNum );
	},
	...
	_isEmptyParaInTable:function(para)
	{
		...
		if( writer.util.ModelTools.isCell(cell) && cell.firstChild() == para && cell.lastChild() == para )
		{
			return true;
		}
...
//				msg && msgs.push(msg);
			...
			else if(ignoreEmptyPara && para.getLength() == 0 && !this._isEmptyParaInTable(para)){
	...
	{
		var listId = null, listLevel;
		for(var i = 0; i < paras.length; i++)
		{
			var para = paras[i];
			if(!para.isList()) 
			{
				if(ignoreEmptyPara && para.getLength() == 0)
					continue;
				else
					return false;
			}
	...
	setListCmd: function(arg, isNumbering)
	{
		...
		else	// Click from list on/off Button
		{
			...
			else
			{
				...
				var isSameList = true, selectionIsBullet = false ,numId = null;
					...
					if(siblingListPara){
						...
						if( !sameSibling && ((siblingIsBullet && !isNumbering) || (!siblingIsBullet && isNumbering)) )
						{
							...
							for(var i = 0; i < selectedParagraphs.length; i++)
							{
								...
								if(ignoreEmptyPara && para.getLength() == 0 && !this._isEmptyParaInTable(para)){
									// Ignore empty paragraph which not in table.
									continue;
								}
	...
	setMultiLevelList: function(mulType)
	{
		...
		else
		{
			...
			var abstractNum = new writer.model.abstractNum(numJson);
			var numId = null;
			if(isHeadingStyle)
			{
	...
	restartList: function(forceRestart){
		...
		var msg = WRITER.MSG.createMsg( WRITER.MSGTYPE.List, [WRITER.MSG.createAddListAct( numId,absId,numJson )],WRITER.MSGCATEGORY.List );
		msgs.push(msg);
		
		// remove below
		var listParas = para.list.getParagraphs().concat([]);
...
//				msg && msgs.push(msg);
	...
	_getLastParagraphInCell: function(cell)
	{
		...
		if(lastChild.modelType == writer.MODELTYPE.PARAGRAPH)
			return lastChild;
		else if(lastChild.modelType == writer.MODELTYPE.TABLE)
		{
	...
	{
		if(!para || writer.util.ModelTools.isTextBox(para) || writer.util.ModelTools.isDocument(para))
			return null;
		var preSib = para.previous();
		if(preSib) 
		{
			if(preSib.modelType == writer.MODELTYPE.PARAGRAPH)
				return preSib;
			else if(preSib.modelType == writer.MODELTYPE.TABLE)
			{
				var lastRow = preSib.lastChild();
				var lastCell = lastRow.lastChild();
				preSib = this._getLastParagraphInCell(lastCell);
			}
			else if(preSib.modelType == writer.MODELTYPE.ROW)
			{
				var lastCell = preSib.lastChild();
				preSib = this._getLastParagraphInCell(lastCell);
			}
			else if(preSib.modelType == writer.MODELTYPE.CELL)
			{
		...
		if(preSib && preSib.modelType == writer.MODELTYPE.PARAGRAPH)
			return preSib;
		else
...
//			msg && msgs.push(msg);
	...
	setNumberingVal: function() {
		...
		var dialog = new writer.ui.widget.SetNumberingValueDlg(pe.lotusEditor, nls.SET_NUMBERING_VALUE, null, null, {"para": para, "callback": callback});
//		dialog.setInitParamter(para, callback);
	...
	getFreeAbstractNumId: function() {
		return writer.util.ListTools.getFreeAbstractNumId();
	},
	getFreeNumId : function() {
		return writer.util.ListTools.getFreeNumId();
	},
	...
	updateCmdStatus: function()
	{
		...
		var bulletState = writer.TRISTATE_OFF, numberingState = writer.TRISTATE_OFF,multiLevelState = writer.TRISTATE_OFF;
		...
		if(paras.length > 0 && this._isSameList(paras, ignoreEmptyPara))
		{
			if(this._isSameListLevel(paras, ignoreEmptyPara))
			{
				for(var i = 0; i < paras.length; i++){
					...
					if(para.isBullet())
						bulletState = writer.TRISTATE_ON;
					else
						numberingState = writer.TRISTATE_ON;
					break;
				}
			}
			else
		...
		if(bInToc || writer.util.ModelTools.isInSmartart()) {
			bulletState = writer.TRISTATE_DISABLED;
			numberingState = writer.TRISTATE_DISABLED;
			multiLevelState = writer.TRISTATE_DISABLED;
		}
		...
		if(concord.util.browser.isMobile())
		{
			...
			var bulletIndex = bulletState == writer.TRISTATE_ON ? defaultBulletsIndex[this.getListType()] : 0;
			var numberIndex = numberingState == writer.TRISTATE_ON ? defaultNumberIndex[this.getListType(true)] : 0;
	...
	_addSelectionChange: function()
	{
		...
		dojo.subscribe(writer.EVENT.SELECTION_CHANGE, this, this.updateCmdStatus);
	}
});