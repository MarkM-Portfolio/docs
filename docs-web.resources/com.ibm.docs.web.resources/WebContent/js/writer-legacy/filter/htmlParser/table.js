dojo.provide("writer.filter.htmlParser.table");
dojo.require("writer.filter.htmlParser.element");

dojo.declare("writer.filter.htmlParser.table", [writer.filter.htmlParser.JsonWriter],{
	toJson : function()
	{
	...
	containsShape: function(){
		if( writer.filter.htmlParser.isPasteFromWord ){
	...
	getColSpanCount: function(){
		...
		var  table = this.element, maxCount = 0, tbody,  count ;
	...
	getTableRows: function( tbody, grid ){
		var trs = [],cell,colspan, filledCells = [];
		var maxCount = this.getColSpanCount(), count;
	...
	getTablePropertyJson: function()
	{
		...
		var ret = {}, element = this.element, bPasteFromWord = writer.filter.htmlParser.isPasteFromWord;
		var border = ( bPasteFromWord ) ? {} : element.getBorder(),
		defaultBorder =  {"width":"1px","style":"solid","color":"#000000"};
