dojo.provide("writer.filter.htmlParser.element");
dojo.require("writer.filter.htmlParser.fragment");
 ...
 * @example
 */
writer.filter.htmlParser.element = function( name, attributes )
{
	/**
	 * The element name.
	 * @type String
...
writer.filter.htmlParser.JsonWriter = function( element ){
...
(function()
{
	...
	writer.filter.htmlParser.element.prototype =
	{
		/**
		 ...
		 * @example
		 */
		type : writer.filter.NODE_ELEMENT,
		 ...
		 * @example
		 */
		add : writer.filter.htmlParser.fragment.prototype.add,
		...
		clone : function()
		{
			return new writer.filter.htmlParser.element( this.name, this.attributes );
		},
		...
		filter: function( filter ){
			...
			if( filter ){
				...
				if ( element.type != writer.filter.NODE_ELEMENT )
				{
					return element;
				}
		...
		createJsonWriter: function( tagName, element ){
			...
			case 'h5':
			case 'h6':
				return new writer.filter.htmlParser.paragraph(element);
				break;
			case 'li':
				return new writer.filter.htmlParser.listitem(element);
				break;
			case 'img':
				return new writer.filter.htmlParser.image(element);
				break;
			case 'ol':
			case 'ul':
				return new writer.filter.htmlParser.list( element);
				break;
			case 'table':
				return new writer.filter.htmlParser.table(element);
				break;
			case 'tr':
				return new writer.filter.htmlParser.row(element);
				break;
			case 'td':
			case 'th':
				return new writer.filter.htmlParser.cell(element);
				break;
//			case 'span':
//			case 'em':
//			case 'u':
...
//				break;
			...
			case "script":
				return null;
			case "div":
				if( writer.filter.htmlParser.isPasteFromWord && (/shape/).test( element.attributes[ 'class' ]||""))
						return null;
				else
					return new writer.filter.htmlParser.JsonWriter(element );
			default:
				if( !element._.isBlockLike )
					return new writer.filter.htmlParser.textportion(element);
				else
					return new writer.filter.htmlParser.JsonWriter(element );
				break;
			}
		...
		addStylesFromAttibutes: function( styles ){
			...
			var attsMap = { "color":"color", "face":"font-family","align":"text-align"};
		...
		cssStyleToJson: function( cssStyle ){
			...
			for( var s in rules ){
				...
				if( s == "background"){
					...
					rules['background-color'] = this.toColorValue(v);
					delete rules[s];
				}
			}
			return rules;
	 ...
	 */
	writer.filter.htmlParser.JsonWriter.prototype = 
	{
		toJson : function( )
		{
		...
		fixBlockChildren: function( element ){
			...
			var children = [], createP;
			for ( var i = 0 ; i < element.children.length ; i++ ){
				if( !element.children[i]._.isBlockLike ||  element.children[i].name == "br" ){
					if( !createP ) createP = new writer.filter.htmlParser.element( "p" );
		...
		writeBlockContentsJson: function( element, root ){
			...
			if( createP ){
				...
				if( root )
					jsonObj = {"c":jsonObj.c, "fmt":jsonObj.fmt};
				retVal.push( jsonObj );
			}
			return retVal;
		}
