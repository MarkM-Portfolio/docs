dojo.provide("writer.filter.htmlParser");

writer.filter.htmlParser = function()
{
	this._ =
	{
...
(function()
{
	...
	writer.filter.htmlParser.prototype =
	{
		/**
		 ...
		 * 		tag doesn't.
		 * @example
		 * var parser = new writer.filter.htmlParser();
		...
		parse : function( html )
		{
			...
			while ( ( parts = this._.htmlPartsRegex.exec( html ) ) )
			{
				...
				if ( ( tagName = parts[ 1 ] ) )
				{
					...
					if ( cdata && writer.filter.dtd.$cdata[ tagName ] )
					{
						// Send the CDATA data.
