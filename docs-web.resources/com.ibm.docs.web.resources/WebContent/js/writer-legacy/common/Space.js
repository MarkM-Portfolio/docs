dojo.provide("writer.common.Space");
common.Space=function(w,h,x,y,spaceOwner){
	this._occupied= null;
	this.init(w,h,x,y,spaceOwner);
};
common.Space.prototype= {
	...
	clone: function() {
		var newSpace = new common.Space(this.w, this.h, this.x, this.y, this.owner);
		for (var i in this._occupied) {
			var o = this._occupied[i];
			var rect = new common.Space(o.w,o.h,o.x,o.y);
	...
	occupy: function(w,h,x,y) {
		var newSpace = new common.Space(w,h,x,y);
	 ...
	 */
	getOccupiedObject : function(x, y){
		var subSpace;
		
		for(var i = 0; i < this._children.length; ++i)
		{
			subSpace = this._children[i];
			if( subSpace.owner && (x > subSpace.x && x < subSpace.x + subSpace.w) && (y > subSpace.y && y < subSpace.y + subSpace.h) )
			{
				var p = writer.util.ViewTools.getParagraph(subSpace.owner);
	...
	release: function(w,h,x,y) {
		var index = this.indexOfOccupied({x:x,y:y,w:w,h:h});

		if (index >= 0) {
			this._occupied.splice(i, 1);
		}
	},
	...
	indexOfOccupied: function(rect) {
		...
		for (var i in this._occupied) {
			var o = this._occupied[i];
			if (this._numberEqual(o.x,rect.x)&&this._numberEqual(o.y,rect.y)
					&&this._numberEqual(o.w,rect.w)&&this._numberEqual(o.h,rect.h)) {
				index = i;
				break;
			}
		}
	...
	subSpace: function(subParam) {
		...
		if (subParam && subParam.x > 0 && (subParam.x < spaceWidth || spaceWidth < 0)) {
			...
			newSpace = new common.Space( newSpaceWidth, spaceHeight);
			spaceWidth = newSpaceWidth;
			for (var i in rects.right) {
				var rect = dojo.clone(rects.right[i]);
		...
		if (subParam && subParam.h > 0 && subParam.h < newSpace.h) {
			...
			newSpace = new common.Space(this.getWidth(),subParam.h);

			for (var i in rects.above) {
				var rect = dojo.clone(rects.above[i]);
				newSpace.occupy(rect);
			}
			for (var i in rects.cross) {
				var rect = dojo.clone(rects.cross[i]);
	...
	mallocSpace:function(x,y,w,h){
		...
		var ret = new common.Space(w, h, x, y);
		for(var i=0;i< this._occupied.length;i++){
			var o = this._occupied[i];
	...
	_getIntersection:function(space){
		...
		return new common.Space(w, h, x-this.x, y-this.y);
	},
	...
	getNextSpaceY: function(y,h){
		...
		var tempSpace = new common.Space(this.w, h, 0, y);
		for (var i in this._occupied) {
			var rect = this._occupied[i];
			if (rect.w > 0 && rect.h > 0) {
	...
	getNextSpaceX : function(x, height){
		var tempSpace = new common.Space(this.w -x, this.h, x, this.y);
		var nextX = x;
		for (var i in this._occupiedSortX) {
