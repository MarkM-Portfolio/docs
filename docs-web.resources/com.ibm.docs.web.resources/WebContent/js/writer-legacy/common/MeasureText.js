dojo.provide("writer.common.MeasureText");
//var G_FRE_CHARS = [];
common.MeasureText = {
	_fontMeasureCache: {},
	_charMeasureCache: {},
...
//	context2d: null,
	...
	init: function(){
		if(_beforeHandler)
		{
			dojo.unsubscribe(_beforeHandler);
			_beforeHandler = null;
		}	
		...
		var data =
			...
			'</html>';
		this.textMeasureFrame =  dojo.byId("measureFrame");
		var doc = this.textMeasureFrame.contentWindow.document; 
		doc.open();
		doc.write( data );
		doc.close();
		
		this.textMeasure = dojo.byId("textMeasure", doc);
		//this.test();
		
		dojo.subscribe(writer.EVENT.PREMEASURE, this, this._preMeasureImpl);
...
//		window.wordLen = [];
	...
	getCSSStr: function(font)
	{
		...
		if(verticalAlign=="super"||verticalAlign=="sub"){
			var fontsize = common.tools.transformFontSize(font);
		}
	...
	_enlargeFrame: function()
	{
		...
		dojo.style(this.textMeasureFrame, "width", "1000000px");
		this._enlarged = true;
		
		var that = this;
		setTimeout(function(){
			that._enlarged = false;
			dojo.style(that.textMeasureFrame, "width", "100px");
		},200);
	},
	...
	_preMeasureImpl: function()
	{
		...
		for(var styleStr in this._preMeasureCache)
		{
			...
			for(key in words)
			{
				...
				if(dojo.isIE && (word.indexOf('\u3000') == 0 || word.lastIndexOf('\u3000') == word.length - 1)){	// Defect 50250
...
//			span += '<div style="display:inline-block;' + css + '">' + words[i] +'</div>';
	...
	measureTextRun_byWord: function(text, font, byWord)
	{
		...
		words = (concord.util.browser.isMobile()||dojo.isSafari || dojo.isIE <= 9) ? [text] : text.split(splitChar);
		var dupWordCnt = {}, noCacheWords = [], word, cacheWidth, keyWord; 
		for(var i = 0; i < words.length; i++)
		{
			word = words[i];
			if(word.length == 0)
	...
	_getCharArraySize: function(text, css)
	{
		...
		{
			rect = child.getBoundingClientRect();
			sizeArray.push({"w":rect.width, "h":rect.height});
			child = child.nextSibling;
		}	
		return sizeArray;
	},
	...
	measureTextRun_byChar: function(text, font, byWord)
	{
		...
		var cssStr = typeof(font) == "string" ? font : this.getCSSStr(font);
		var cache = this._charCache[cssStr] = this._charCache[cssStr] || {};
		var dupTextCnt = {}, noCacheText = "", ch, cacheObj;
		for(var i = 0; i < text.length; i++)
		{
			ch = text[i];
			cacheObj = cache[ch];
...
var _beforeHandler =  dojo.subscribe(writer.EVENT.BEFORE_LOAD, common.MeasureText, common.MeasureText.init);
