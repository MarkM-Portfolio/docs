dojo.provide("writer.common.Container");
common.Container = function(owner){
	this.owner = owner;
	if (!dojo.isChrome) {
		this.first= null;
		this.last = null;
...
//			}
		...
		return {"next":null,"pre":null,"content":obj};
	},
//	getById:function(id){
//		var refId = "a"+id;
//		if(this.nodes[refId]){
...
//		}
	...
	divide:function(obj){
		...
		if(this.first==node){
			...
			var newLink = new common.Container(this.owner);
			newLink.first = node;
	...
	next:function(obj){
		if(obj.refId)obj = this.nodes[obj.refId];
		if(obj&&obj.next){
	...
	sort:function(subContainer){
		var t  = new common.Container(subContainer.owner);
	...
	*/
	sortByFunc: function(comp)
	{
		...
		var sortFunc = function(node1, node2) { return comp(node1.content, node2.content); };
		var arr = [];
		var node = this.first;
		while (node) {
			arr.push(node);
...
common.Container.prototype = {
	...
	divide: function(item, createParam) {
		...
		if (i==0) {
			...
			var newContainer = new common.Container(this.owner);
