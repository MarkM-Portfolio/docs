...
dojo.provide("writer.collaboration.TaskHandler");
dojo.require("concord.task.AbstractTaskHandler");
//dojo.require("concord.text.MsgUtil");
dojo.require("concord.task.CachedTask");
dojo.declare("writer.collaboration.TaskHandler", [concord.task.AbstractTaskHandler], {
	...
	selectionChange:function(range){
		...
		var taskId = null;
		if(para) {
			if(writer.util.HelperTools.canTaskCreate())
			...
			if(taskId!=this.curTaskId) {
				...
				var taskBean = this.getTaskBeanById(writer.util.HelperTools.getSelectedTaskId());
	...
	isTaskTop:function(viewNode){
		if(viewNode.model.isTask()) {
			var pre_obj = writer.util.ModelTools.getPrev(viewNode.model,writer.util.ModelTools.isParaOrTable);
	...
	isTaskBottom:function(viewNode){
		if(viewNode.model.isTask()) {
			var next_obj = writer.util.ModelTools.getNext(viewNode.model,writer.util.ModelTools.isParaOrTable);
	...
	createTaskAction:function(viewNode){
		if(viewNode.model.getTaskId() != this.curTaskId)
			return;
		var taskActionNode = dojo.create("div", {
	...
	createTaskTitle:function(viewNode){
		if(viewNode.model.getTaskId() != this.curTaskId)
			return;
		var taskTitleNode = dojo.create("span", {
			"class":"task_title"
		});
	...
	_taskRender : function(viewNode) {
		if (this.getShow()) {
			if (viewNode.model.isTask()) {
				...
				viewNode.taskNode.left = this.createTaskNode(viewNode,writer.TASK_FLAG_LEFT, bold_size,
							height, -bold_size - x_offsize-border_left_w,-border_top_w);;
				viewNode.taskNode.right = this.createTaskNode(viewNode,writer.TASK_FLAG_RIGHT, bold_size,
							height, viewNode.parent.getWidth() + x_offsize-border_right_w,-border_top_w);
				if (this.isTaskTop(viewNode)) {
					var next_view = writer.util.RangeTools.getNextPara(viewNode);
					...
					viewNode.taskNode.top = this.createTaskNode(viewNode,writer.TASK_FLAG_TOP, viewNode.parent.getWidth()
							+ 2 * x_offsize, bold_size, -x_offsize-border_left_w,-border_top_w);
					this.createTaskAction(viewNode);
					this.createTaskTitle(viewNode);
				}
				if (this.isTaskBottom(viewNode)) {
					viewNode.taskNode.bottom = this.createTaskNode(viewNode,writer.TASK_FLAG_BOTTOM, viewNode.parent
							.getWidth()
							+ 2 * x_offsize, bold_size, -x_offsize-border_left_w,height-border_top_w-bold_size);
					var pre_view = writer.util.RangeTools.getPrevPara(viewNode);
	...
	postTaskCreate: function(taskBean, context, info){
		var msgs = [], msg;
		var taskObjs = writer.util.HelperTools.getSelectedParaOrTable();
		for(var i = 0; i < taskObjs.length; i++) {
			msg = taskObjs[i].setTask(taskBean.getId());
			msg && msgs.push(msg);
		}
		var updateAct = WRITER.MSG.createTaskAct(WRITER.ACTTYPE.TaskUpdate,taskBean.getId());
		var updateMsg = WRITER.MSG.createMsg( WRITER.MSGTYPE.Task, [updateAct] );
	...
	setCommandState : function(commandList)
    {
    	...
    	for(var i=0;i<commandList.length;i++){
    		...
    		command.setState(commandList[i].disableCondition?writer.TRISTATE_DISABLED:writer.TRISTATE_OFF);
    	}
    },
	...
	setTaskSelected: function(taskId){
		if (taskId != this.curTaskId){
			...
			if (this.curTaskId){
				taskEvent = [{taskId:this.curTaskId , selected:false}];
    				concord.util.events.publish(concord.util.events.taskEvents_eventName_taskSelected,taskEvent);
			}
...
//			cmdList.push(LB_COM.createNodeInsertCmd(blockId))
	...
	destroyActionButton:function(taskId){
		var actionBtn = dijit.byId(taskId+'_Action');
		if (actionBtn)
	...
	createActionButton:function(taskId,disable){
		...
		actionBtn.openDropDown = function(){
			...
			if(actionBtn.dropDown){
				...
				//adjust the position
				dojo.style(menuFrame, "left", x + dojo.byId('editorFrame').offsetLeft + "px");
				dojo.style(menuFrame, "top", y + dojo.byId("editorFrame").offsetTop + "px");
				return retval;
			};
		};
	...
	preLoadTask: function(){
		...
		dojo.subscribe(concord.util.events.taskEvents_eventName_taskLoaded, this, loadFinished);
	},
...
//					pe.scene.showWarningMessage(this.nls.cannotSupportSocialEdit, 30000);            		
	...
	enableTaskCmds: function(bEnable){
    	 ...
    	 if(bEnable && this.bServiceReady){	
    	    ...
    	    selectedTask = this.getTaskBeanById(writer.util.HelperTools.getSelectedTaskId());
    	 }
...
//		dojo.removeAttr(body, "doc_id");	
	...
	getSelectedTask: function(selection){
		return this.getTaskBeanById(writer.util.HelperTools.getSelectedTaskId());
		try{
			if (!selection){
	...
	publishUpdateTaskMsg: function(taskId, cachedTask){
		...
		var msgs = [];
		var updateAct = WRITER.MSG.createTaskAct(WRITER.ACTTYPE.TaskUpdate,taskId);
		var updateMsg = WRITER.MSG.createMsg( WRITER.MSGTYPE.Task, [updateAct] );
		updateMsg && msgs.push(updateMsg);
	//	msg.msg.disableRedo = true;
	//	msg.rMsg.disableUndo = true;
	...
	publishDeleteTaskMsg: function(taskId){
		...
		var msgs = [];
		var updateAct = WRITER.MSG.createTaskAct(WRITER.ACTTYPE.TaskDelete,taskBean.getId());
		var updateMsg = WRITER.MSG.createMsg( WRITER.MSGTYPE.Task, [updateAct] );
		updateMsg && msgs.push(updateMsg);
	//	msg.msg.disableRedo = true;
	//	msg.rMsg.disableUndo = true;
	...
	processMessage : function ( act) {
		switch (act.t) {
				case WRITER.ACTTYPE.TaskDelete:
					var taskId = act.tsk;
					var selectedTask = this.getSelectedTask();
					...
					break;
				case WRITER.ACTTYPE.TaskUpdate:
					if ( act.tsk != null ) {
...
writer.TASK_FLAG_TOP = 0x01;
writer.TASK_FLAG_BOTTOM = 0x02;
writer.TASK_FLAG_LEFT = 0x04;
writer.TASK_FLAG_RIGHT = 0x08;
