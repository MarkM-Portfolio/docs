dojo.provide("writer.util.RangeTools");
dojo.require("writer.util.ViewTools");
dojo.require("writer.core.Range");
writer.util.RangeTools = {
	RUNGUARD: function( item ){
		return writer.util.ViewTools.isRun( item ) &&  writer.util.ViewTools.isVisible(item) ;
	},
	PARAGUARD: function( item ){
		return writer.util.ViewTools.isParagraph( item ) &&  writer.util.ViewTools.isVisible(item) ;
	},
	TABLEGUARD: function( item ){
		return writer.util.ViewTools.isTable( item ) &&  writer.util.ViewTools.isVisible(item) ;
	},
	TextBoxGuard: function(item)
	{
		return writer.util.ViewTools.isTextBox(item);
	},
	TXBXorCANVASGuard: function(item)
	{
		return writer.util.ViewTools.isTextBox(item) ||
			writer.util.ViewTools.isCanvas(item);
	},
	RUNDrawingobjGuard: function(item)
	{
		return ( writer.util.ViewTools.isRun(item) ||
			writer.util.ViewTools.isDrawingObj(item));
	},
	...
	{
		return writer.util.ViewTools.isDrawingObj(item);
	},
	view: writer.util.ViewTools,
	...
	getNextPosition: function( pos, bShift){
		...
		//index
			...
			// end of run 
				if( next = this.getNextRun( obj, this.TextBoxGuard, this.TXBXorCANVASGuard ) ){
					...
					// end or in same line or same paragraph
						...
						if(!bShift || writer.util.ModelTools.isEmptyParagraph(para) || samepara)
			...
			next = obj.next();
			if( !next && (writer.util.ViewTools.isCell(obj)|| writer.util.ViewTools.isRow(obj)|| writer.util.ViewTools.isTable(obj))){
				var parent = obj;
				...
				if(writer.util.ViewTools.isRow(nextParent)){
	...
	getPrevPosition: function( pos, bShift){
		...
		//index
			...
			prev = obj.previous();
			if( !prev && (writer.util.ViewTools.isCell(obj)|| writer.util.ViewTools.isRow(obj)|| writer.util.ViewTools.isTable(obj))){
				var parent = obj;
				...
				if(writer.util.ViewTools.isRow(prevParent)){
	...
	compare: function( startView, endView ){
		...
		if( body1 == body2 ){
			var startRow = this.view.getRow(startView.obj),endRow = this.view.getRow(endView.obj);
	...
	optimize: function( range,isHighlight ){
		if(!isHighlight && this.isNeedSwap( range )){
			return new writer.core.Range( range.getEndView(), range.getStartView() ) ;
		}else
			return range;
	},
//	/*
...
//	},
	...
	ifContainOnlyOneObject: function(range, isObjFunc) {
		...
		if (startView.obj == endView.obj && 0 == startView.index && endView.obj.len == endView.index
				&& isObjFunc(startView.obj)) {
	...
	ifContainOnlyOneObjectEx: function(range, isModelFunc)
	{
		...
		*/
		var getOnlyOneSpecModel = function(range, isObjFunc)
		{
			...
			if (startModel.obj == endModel.obj && 0 == startModel.index && endModel.obj.length == endModel.index
				&& isObjFunc(endModel.obj))
			{
	...
	ifContainOnlyOneImage: function(range) {
		return this.ifContainOnlyOneObject(range, writer.util.ViewTools.isImage)
			|| this.ifContainOnlyOneObjectEx(range, writer.util.ModelTools.isImage);
	},
	...
	ifContainOnlyOneTextBox: function(range) {
		return this.ifContainOnlyOneObject(range, writer.util.ViewTools.isTextBox)
			|| this.ifContainOnlyOneObjectEx(range, writer.util.ModelTools.isTextBox);
	},
	...
	ifContainOnlyOneCanvas: function(range) {
		return this.ifContainOnlyOneObject(range, writer.util.ViewTools.isCanvas)
			|| this.ifContainOnlyOneObjectEx(range, writer.util.ModelTools.isCanvas);
	},
	...
	ifContainOnlyOneDrawingObj: function(range) {
		return this.ifContainOnlyOneObject(range, writer.util.ViewTools.isDrawingObj)
			|| this.ifContainOnlyOneObjectEx(range, writer.util.ModelTools.isDrawingObj);
	},
	
	getStyleOperationRange: function(range){
		if( writer.util.RangeTools.ifContainOnlyOneTextBox(range)){
			paras = range.startModel.obj.getParagraphs();
			if(paras.length > 0)// Select shape without paragraph.
				return new writer.core.Range({"obj": paras[0], "index":0}, {"obj": paras[paras.length - 1],"index": paras[paras.length - 1].getLength()} );				
		}
		return range;
	},
	...
	toViewPosition: function( obj, index, rootView ){
		var MODEL = writer.util.ModelTools;
		if( (MODEL.isRun(obj) || MODEL.isAnchor(obj) || MODEL.isInlineObject(obj))
			&& !MODEL.inCanvas(obj) ) {
...
//			console.error("view has not layouted");
		...
		if( obj.modelType == 'paragraph'){
			...
			// search run view
			var tools = writer.util.RangeTools;
			...
			if( run ){
				...
				index -= run.start;
				if (index < 0) index = 0;
				return writer.util.RangeTools.makeViewPosition(run, index);
			}else{
			...
			if( !viewObj )
				return { "obj": viewers.getLast(), "index": index };
			else
				return {"obj": viewObj, "index": start };
		}
	},
	...
	selectToEditStart:function(range,model){
		...
		if( writer.util.ModelTools.isParagraph(model))
			...
			p =  writer.util.ModelTools.getFirstChild( model, writer.util.ModelTools.isParagraph,true );
		if(p){
			range.setStart(p,0);
		}
	},
	selectToEditEnd:function(range,model){
		...
		if( writer.util.ModelTools.isParagraph(model))
			...
			p =  writer.util.ModelTools.getLastChild( model, writer.util.ModelTools.isParagraph,true);
		if(p){
			range.setEnd(p,writer.util.ModelTools.getLength( p ));
		}
	},
	canDo:function(ranges,noMessage){
		...
		for (var i=ranges.length-1;i>-1;i--){
			...
			var startNote = writer.util.ModelTools.getParent(ranges[i].getStartModel().obj,writer.util.ModelTools.isNotes);
			var endNote   = writer.util.ModelTools.getParent(ranges[i].getEndModel().obj,writer.util.ModelTools.isNotes);
			if((startNote||endNote) && startNote!=endNote || rootModel!=ranges[i].rootView.model ){
				if(!noMessage){
					var nls = dojo.i18n.getLocalization("writer","lang");
					if(startNote.modelType == writer.MODELTYPE.FOOTNOTE){
						var str = nls.NOTE_INVALIDACTION_FOOTNOTE;
					}else{
						var str = nls.NOTE_INVALIDACTION_ENDNOTE;
					}
	...
	getFirstParaInRanges:function(ranges){
		var range = ranges[0];
	  	...
	  	if(ranges.length>1){	  		
	  		...
	  		var path1 = writer.util.ModelTools.getParagraphPath(start.obj);
	  		var path2 = writer.util.ModelTools.getParagraphPath(lastStartPara.obj);
	  		var compareResult = common.tools.arrayCompare(path1, path2);
	  		if(compareResult>0){
	  			start = lastStartPara;
	  		}
	  	}
