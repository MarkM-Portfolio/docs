dojo.provide("writer.model.table.Cell");
dojo.require("writer.model.prop.CellProperty");
dojo.require("writer.model.update.Block");
writer.model.table.Cell=function(json,row,table){
	this.parent = row;
	this.table = table;
	this.init(json);
};
writer.model.table.Cell.prototype={
	modelType : writer.MODELTYPE.CELL,
	init:function(json){
		this.id = json.id;
		this.rowSpan =1;
		this.colSpan =1;
//		this.container = new common.Container(this);
		this.cellProperty = new writer.model.prop.CellProperty(json.tcPr,this);
		if(json.tcPr){
			if(this.cellProperty.tcW){
				this.w = Math.round(common.tools.toPxValue(this.cellProperty.tcW.w,this.cellProperty.tcW.type));	
			}
			this._initColSpan();
		}else{
			this.cellProperty = new writer.model.prop.CellProperty({},this);
		}
...
//		}
	...
	replaceContent: function(paragraphs, bInternal){
		...
		for(var i=0;i< paragraphs.length;i++){
			...
			if(paragraphs[i].t == null)
			{
				...
				jsonData = writer.util.ModelTools.getEmptyParagraphSource();
			}	
			...
			if(paragraphs[i].t == null)
			{
				...
				writer.util.ModelTools.insertInlineObject( richTextJson, m, 0, false );
			}	
		}
		...
		for(var i = oldLen - 1; i >= 0; i--)
		{
			...
			msgs.push( WRITER.MSG.createMsg( WRITER.MSGTYPE.Element, [ WRITER.MSG.createDeleteElementAct( para )] ) );
			this.remove(para);	
		}
	...
	toJson:function(){
		...
		try{
			tc.tcPr.tcW={"w": common.tools.PxToDXA(this.getTable().getColumnWidth(this.getColIdx())),"type":"dxa"};
		}catch(e){
	...
	_getCellPosType:function(){
		var matrix = this.getTable().getTableMatrix();
		var colIdx = this.getColIdx();
		var rowIdx = this.parent.getRowIdx();
		var rowCnt = matrix.length();
		var colCnt = matrix.length2();
		var type ="N";
		if(colIdx==0){
			type = "L";
		}
	...
	changeBorder:function(border,options){
		if(!border)
			return;
		var borderChangeSet = dojo.clone(border);
		var cellProperty = this.getProperty();
		if(cellProperty.tableProperty.getDirection(true) == "rl-tb"){
			var tmp = border.left;
			border.left = border.right;
			border.right = tmp;
		}
		var newBorder = writer.util.TableTools.mergeBorderChangeSet(cellProperty.getBorder(),borderChangeSet);
	...
	getBorder:function(){
		...
		var cellType = this._getCellPosType();
		var border = dojo.clone(cellPerporty.getBorder());
		if(cellPerporty.tableProperty.getDirection(true) == "rl-tb") {
			var tmp = border.right;
			border.right = border.left;
			border.left = tmp;
		}
		if(border.left&&border.left.style=="none"){
			var leftNull = border.left;
			border.left = null;
		}
		if(border.right&&border.right.style=="none"){
			var rightNull = border.right;
			border.right = null;
		}
		if(border.top&&border.top.style=="none"){
			var topNull = border.top;
			border.top = null;
		}
		if(border.bottom&&border.bottom.style=="none"){
			var bottomNull = border.bottom;
			border.bottom = null;
		}
		...
		var tablePropertyHandler   = function(styleBorder){
			...
			if(cellType=="N"){
				...
				border.right = border.right||dojo.clone(styleBorder.v);
				border.top =   border.top||dojo.clone(styleBorder.h);
				border.bottom =   border.bottom||dojo.clone(styleBorder.h);
			}else if(cellType=="R"){
				border.left  = border.left||dojo.clone(styleBorder.v);
...
//				border.left  = border.left||dojo.clone(styleBorder.v);
		...
		var colStyleHandler = function(styleBorder){
			...
			if(cellType=="WN"||cellType=="EN"||cellType=="T"||cellType=="WEN"){
				...
				border.top =   border.top||dojo.clone(styleBorder.top)||dojo.clone(styleBorder.h);
				border.bottom =   border.bottom||dojo.clone(styleBorder.h);
			}
		};
    ...
    emptyClone:function(){
    	...
    	if(firstElement){
    		if(!writer.util.ModelTools.isParagraph(firstElement)){
    			firstElement = writer.util.ModelTools.getFirstChild(firstElement,writer.util.ModelTools.isParagraph,true);
    		}
    		var paraJson = writer.util.ModelTools.getEmptyParagraphSource();
    		if(firstElement){
...
//    	this.colSpan = this.colSpan+ delSpan;
	...
	_insertViewAfter:function(){
		...
		if(allViewrs){
			for(var ownerid in allViewrs){
				...
				view = allViewrs[ownerid].next(view);
				while(view){
					myView = new writer.view.table.CellView(this,ownerid,true);
					myView.container = new common.Container(myView);
	...
	_insertViewBefore:function(){
	 ...
	 isContentEmpty:function(){
		 if(this.container.length()<=1){
			 ...
			 if(c.modelType == writer.MODELTYPE.PARAGRAPH&&c.text.length==0){
				 return true;
			 }
		 }
		 return false;
	 ...
	 markCheckBorder:function(){
		...
		for(var ownerId in allViews){
			...
			var firstView = viewers.getFirst();
			while(firstView){				
				firstView.toCheckBorder(true);
				firstView = viewers.next(firstView);
			}
		}	
	}
};
common.tools.extend(writer.model.table.Cell.prototype, new writer.model.table.TableBase());
