dojo.provide("writer.model.prop.ParagraphProperty");
 ...
 *    			'before':'',
 *    			'after':'',
 *    			'lineSpace':{'type':'relative|absolute','value':''}	
 *    		}
 *  }
 * 
 * 
 * 
 */
writer.model.prop.ParagraphProperty = function(json,paragraph,isListPpr){
//	this.source = json;
...
writer.model.prop.ParagraphProperty.prototype = {
	beforeAutoSpace:"14",
	afterAutoSpace:"14",
	type: writer.model.prop.Property.PARAGRAPH_PROPERTY,
	_defaultVal : "none",
	fromJson:function(json){
		...
		if(json)
		{
			...
			if(json.indent){
				...
				var indValue = json.indent.left&&common.tools.toPtValue(json.indent.left);
				this.indentLeft = indValue == 0 ? indValue : (indValue || this.indentLeft); 
				var indvalueRight = json.indent.right&&common.tools.toPtValue(json.indent.right);
				...
				if(json.indent.hanging){
					...
					this.specialIndentValue = common.tools.toPtValue(json.indent.hanging);
				}else if(json.indent.firstLine){
					this.specialIndent='firstLine';
					this.specialIndentValue = common.tools.toPtValue(json.indent.firstLine);
				}
				
			}
			if (json.tabs){
				this.tabs = new writer.model.prop.TabProperty(json.tabs);
			}
			...
			if(json.space){
				if(ifAutoSpaceingSet(json.space.beforeAutospacing)){
					...
					this.spaceBefore = json.space.before&&common.tools.toPtValue(json.space.before);
					( this.spaceBefore == null )&& ( this.spaceBefore = this._defaultVal );
				}
				if(ifAutoSpaceingSet(json.space.afterAutospacing)){
					this.spaceAfter = this.afterAutoSpace;
				}else{
					this.spaceAfter  = json.space.after&&common.tools.toPtValue(json.space.after);
					( this.spaceAfter == null )&& ( this.spaceAfter = this._defaultVal );
				}
...
//				eval("debugger");
			...
			//fill default numPr for toc heading
				json.numPr = {"numId":{"val":-1},"ilvl":{"val":0}};
			}
			...
			if(json.numPr)
			{
				...
				else if(json.numPr.numId && json.numPr.numId.val)	// Avoid empty numPr
					this.iLvl = 0;
			}
		...
		if(this.paragraph){
			...
			refStyle = pe.lotusEditor.getRefStyle(writer.model.defaultStyle);
	...
	toJson:function(){
		...
		if(this.border && this.border != this._defaultVal)
			json.pBdr = dojo.clone(this.border);
		...
		if(this.sectId && this.sectId != this._defaultVal)
		...
		if(this.pageBreakBefore != this._defaultVal)
			json["pageBreakBefore"] = {"val" : this.pageBreakBefore ? "1" : "0" };

		if(this.keepLines != this._defaultVal)
			json["keepLines"] = {"val" : this.keepLines ? "1" : "0" };

		if(this.widowControl != this._defaultVal)
			json["widowControl"] = {"val" : this.widowControl ? "1" : "0" };
	...
	clone: function()
	{
		return new writer.model.prop.ParagraphProperty(this.toJson());
	},
	/**
	...
	_mergeTabs: function(tabs1, tabs2)
	{
		...
		var newTabsJson = [];
		var tabs1Json = tabs1.getTabsJson(), tabs2Json = tabs2.getTabsJson();
		var pos1, pos2; 
		var i = 0, j = 0;
		
		var unitRex = (/^([\+|-]?[\d|\.]*e?[\+|-]?\d+)(pc|px|pt|em|cm|in|mm|emu)$/i), cTools = common.tools;
		...
		return new writer.model.prop.TabProperty(newTabsJson);
	},
	/**
	...
	merge: function(destParaProperty)
	{
		...
		if(this.tabs != this._defaultVal)
		{
			if(destProperty.tabs == undefined || destProperty.tabs == this._defaultVal)
				destProperty.tabs = new writer.model.prop.TabProperty(this.tabs.toJson());
			else
		...
		destProperty.border = destProperty.border || (this.border ? dojo.clone(this.border) : null);
...
//		return new writer.model.prop.ParagraphProperty(destJson);
	...
	_addToList: function()
	{
		...
		if(numId && numId != -1 && numId != this._defaultVal && pe.lotusEditor.number.isValidNumId(numId) && numLevel != null && numLevel != this._defaultVal)
		{
			...
			if (!curList) {
				...
				if(!curList)
					curList = lists[numId] = new writer.model.list(numId);
			}
			...
			else
			{
				...
				if( writer.util.ModelTools.isNotes(this.paragraph.doc) )
	...
	_getValue: function(key, ignoreList)
	{
		...
		{
			var pContent = this.paragraph && this.paragraph.text;
...
//		if(this.paragraph) // Directly property, get it first
		{
			...
			// Get attribute from list or style.
			// If the list was defined in style, will get from style first.
			// Otherwise will get from list then style.
			var firstProp = ignoreList ? null : this.listParaProperty, secondProp = this.styleParaProperty;
...
//		}
	...
	setBackgroundColor:function( color ){
		if(color != this.backgroundColor ){
			this.changedRecord.push({'type':'backgroundColor','oldValue':this.backgroundColor,'newValue':color});
	...
	setBorder:function(border){
		if(border != this.border){
			this.changedRecord.push({'type':'border','oldValue':this.border,'newValue':border});
			this.border = border;
		}
	},
	...
	setAlign:function(align){
		if((align=="none"||align=="left"||align=="right"||align=="centered"||align=="justified")&&align!=this.align){
			this.changedRecord.push({'type':'align','oldValue':this.align,'newValue':align});
			this.align = align;
		}
	...
	setDirection:function(direction){
		if((direction=="none"||direction=="ltr"||direction=="rtl"||direction=="lr-tb"||direction=="rl-tb")
		   && direction!=this.direction){
			this.changedRecord.push({'type':'direction','oldValue':this.direction,'newValue':direction});
...
//		}
	...
	setIndentLeft:function(indent, isFromMsg){
		if(indent != this._defaultVal)
			indent = common.tools.toPtValue(indent);
		...
		if(this.indentLeft!=indent ){
			...
			this.changedRecord.push({'type':'indent.left','oldValue':this._addUnit(tmp),'newValue':this._addUnit(indent)});
	...
	setIndentRight:function(indent){
		...
		if(this.indentRight!=indent){
			this.changedRecord.push({'type':'indentRight.right','oldValue':this._addUnit(this.indentRight),'newValue':this._addUnit(indent)});
	...
	clearSpecialIndent: function()
	{
		if(this._defaultVal != this.specialIndent){
			this.changedRecord.push({'type':'indent.specialkey','oldValue':this.specialIndent,'newValue':this._defaultVal});
			this.specialIndent = this._defaultVal;
		}
		
		if(this._defaultVal!=this.specialIndentValue){
			this.changedRecord.push({'type':'indent.specialvalue','oldValue':this._addUnit(this.specialIndentValue),'newValue':this._defaultVal});
			this.specialIndentValue = this._defaultVal;
		}
	},
	/**
	...
	setIndentSpecialTypeValue:function(type, value){
		if(value != this._defaultVal)
			value = common.tools.toPtValue(value);
...
//			type = "firstLine";
		...
		if((type!=""||type!="")&&type!=this.specialIndent){
			this.changedRecord.push({'type':'indent.specialkey','oldValue':this.specialIndent,'newValue':type});
...
//	},
	...
	setSpaceBefore:function(before){
		if(before != this._defaultVal)
			before = common.tools.toPtValue(before);
		if(this.spaceBefore!=before){
			this.changedRecord.push({'type':'space.before','oldValue':this._addUnit(this.spaceBefore),'newValue':this._addUnit(before)});
	...
	setSpaceAfter:function(after){
		if(after != this._defaultVal)
			after = common.tools.toPtValue(after);
		if(this.spaceAfter!=after){
			this.changedRecord.push({'type':'space.after','oldValue':this._addUnit(this.spaceAfter),'newValue':this._addUnit(after)});
	...
	setLineSpaceType:function(type){
		if((type=="relative"||type=="absolute"||type=="atLeast"||type=="none")&&type!=this.lineSpace){
			this.changedRecord.push({'type':'linespacing.lineRule','oldValue':this.getLineSpaceType(),'newValue':type});
			this.lineSpace = type;
		}
	},
	...
	setLineSpaceValue:function(value){
		if(this.lineSpaceValue!=value){
			this.changedRecord.push({'type':'linespacing.line','oldValue':this.getLineSpaceValue().toString(),'newValue':value});
	...
	setNumId:function(nId){
		if(nId!=this.numId){
			this.changedRecord.push({'type':'numPr.numId','oldValue':this.numId,'newValue':nId});
			this.numId = nId;
		}
	},
	...
	setNumLevel:function(lvl){
		if(lvl!=this.iLvl){
			this.changedRecord.push({'type':'numPr.ilvl','oldValue':this.iLvl,'newValue':lvl});
			this.iLvl = lvl;
...
//		return this._getValue("tabs") || this._defaultVal;
	...
	setTabs: function(newTabs)
	{
		...
		{
			this.tabs = newTabs;
			this.changedRecord.push({'type':'tabs','oldValue':this.tabs.tabs,'newValue':newTabs.tabs});
		}	
	},
	getSectId: function(){
	...
	setSectId: function(newSectId)
	{
		if(this.sectId != newSectId){
			this.changedRecord.push({'type':'secId','oldValue':this.sectId,'newValue':newSectId});
	...
	getMessage:function(){
		...
		var msgActionTypes = {}, msgActionIndex = [];
		var oPpr, nPpr;
		...
		for(var i = 0; i < msgActionIndex.length; i++)
		{
			...
			oPpr = msgActionTypes[type]["oPpr"];
			nPpr = msgActionTypes[type]["nPpr"];
			acts.push(WRITER.MSG.createSetAttributeAct( this.paragraph,nPpr,oPpr ));
		}
		
		var msg = WRITER.MSG.createMsg( WRITER.MSGTYPE.Attribute, acts );
		this.changedRecord=[];
		return msg;
	},
	...
	addStyle: function(styleId)
	{
		...
		if(styleId != this.styleId){
			var oPpr = {'type':'style','styleId':this.styleId};
			var nPpr = {'type':'style','styleId':styleId};
			...
			var msg = WRITER.MSG.createMsg( WRITER.MSGTYPE.Attribute,  [WRITER.MSG.createSetAttributeAct( this.paragraph,nPpr,oPpr )] );
			msgs.push(msg);
		}
		return msgs;
	},
	...
	clearStyle:function(){
		...
		runs.forEach(function(run){
			var oRunProp = {},nRunProp = {};
			if(run.textProperty)
			{
				var runStyle = run.textProperty.toJson();
				if(!runStyle)
					return;	
				oRunProp = dojo.clone(runStyle);
				for(var i in oRunProp)
				{
					nRunProp[i] = null;					
				}
				run.textProperty.clearInlineStyle();
				tacts.push(WRITER.MSG.createSetTextAttribute(run.start,run.length,run.parent,nRunProp,oRunProp));
			}
		});	
		...
		if(this.paragraph.paraTextProperty)
		{
			...
			var o = {s:oPTextProp,type:'pt'};
			this.paragraph.paraTextProperty.clear();
			var n = {s:{},type:'pt'};
			var pacts = [WRITER.MSG.createSetAttributeAct(this.paragraph,n,o)];
			var pmsg = WRITER.MSG.createMsg(WRITER.MSGTYPE.Attribute,pacts,mc,false);
			pmsg && msgs.push(pmsg);			
	...
	removeStyle: function()
	{
		if(this.styleId && this.styleId != this._defaultVal){
			...
			var oPpr = {'type':'style','styleId':this.styleId};
			var nPpr = {'type':'style','styleId':null};
...
//			delete this.iLvl;
		...
		{
			this.changedRecord.push({'type':'style.styleId','oldValue':this.styleId,'newValue':null});
			this.removeStyle();
		}
...
//			this.setIndentSpecialTypeValue("none", "none");
	...
	setPageBreakBefore: function(value)
	{
		if((value == true || value == false || value == this._defaultVal) && this.pageBreakBefore != value )
		{
			...
			if (value == false)
			{
				this.pageBreakBefore = this._defaultVal	; //this.pageBreakBefore  = 搉one�
			...
			this.changedRecord.push({'type':'pageBreakBefore','oldValue':oldValue,'newValue':this.pageBreakBefore});
		}
	},
	...
	setKeepLines: function(value)
	{
		if((value == true || value == false || value == this._defaultVal) && this.keepLines != value){
			this.changedRecord.push({'type':'keepLines','oldValue':this.keepLines,'newValue':value});
	...
	setWidowControl: function(value)
	{
		if((value == true || value == false ||value == this._defaultVal) && this.widowControl != value){
			this.changedRecord.push({'type': 'widowControl','oldValue': this.widowControl, 'newValue':value});
