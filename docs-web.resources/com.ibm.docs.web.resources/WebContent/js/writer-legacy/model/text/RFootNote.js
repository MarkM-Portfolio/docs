 ...
 * 
 */
dojo.provide("writer.model.text.RFootNote");
dojo.require("writer.model.text.Hint");
dojo.require("writer.model.Model");

dojo.declare("writer.model.text.RFootNote",[writer.model.text.Hint,writer.model.Model],{
	modelType: writer.MODELTYPE.RFOOTNOTE,
	...
	fromJson: function( json ){
		...
		this.textProperty= new writer.model.prop.TextProperty(dojo.clone(json && json.style));
		if(json.an){
			this.an = json.an;
		}
	},
...
//	},
	...
	checkReferedNotes:function(){
		...
		if(this.modelType==writer.MODELTYPE.RFOOTNOTE){
			var n = new writer.model.notes.FootNote(this.an);
		}else{
			var n = new writer.model.notes.EndNote(this.an);
		}
		delete this.an;
		var ret = pe.lotusEditor.relations.notesManager.restoreNotes(this,n,this.modelType==writer.MODELTYPE.RFOOTNOTE);
		if(!ret){
			delete this._referFn ;
		}
	}
