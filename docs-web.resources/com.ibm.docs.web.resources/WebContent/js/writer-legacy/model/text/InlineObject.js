dojo.provide("writer.model.text.InlineObject");
 ...
 */
dojo.declare("writer.model.text.InlineObject",[ writer.model.text.TextRun, writer.model.Hints],
{
...
//	},
	...
	{
		var ret = writer.model.text.Hint.prototype.byIndex.apply( this, [index, bSearchChild, bLeftSide] );

		if( ret )
		{
			var child =  writer.model.Hints.prototype.byIndex.apply( this, [index, bSearchChild, bLeftSide]);
			if( child )
				ret =  child;
			else
				return null;
	...
	{	
		...
		{
			writer.model.Hints.prototype.addTextLength.apply(this, [len,index]);
			this.length += len;
	...
	canInsert: function() {
		if( ( this.isPageNumber && this.isPageNumber())
				|| ( this.isTotalPageNumber && this.isTotalPageNumber()))
			return false;
		return true;
	},
	/**
	...
	insertHints: function(index,hints){
		var prop,l, hint, that = this, oldIndex = index;
	...
	insertHint: function( index, hint ){
		...
		if( index > this.start && index < ( this.start + this.length ) )
		{
			...
			(index == this.start ) ? this.parent.hints.insertBefore(hint, this)
