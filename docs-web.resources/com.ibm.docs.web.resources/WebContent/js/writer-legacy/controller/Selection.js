dojo.provide("writer.controller.Selection");

dojo.require("writer.view.selection.Factory");
...
dojo.declare("writer.controller.Selection",
null, {
	...
	_rangeTools: writer.util.RangeTools,
	...
	constructor: function(createParam) {
		...
		this._cursor = new writer.controller.Cursor({
			shell: this._shell,
		...
		this._selectionFactory = new writer.view.selection.Factory();
		this._view = this._shell.view();
		
		dojo.subscribe(writer.EVENT.PAGECREATED, this, this._onPageRendered);
		
		if (!this._coEditIndicator)
		{
			var _that = this;
			var events = [writer.EVENT.COEDIT_STARTED, writer.EVENT.COEDIT_USER_JOINED];
			dojo.forEach(events, function(event){
				dojo.subscribe(event, _that, function(){
					setTimeout(function(){
	...
	{
		var ranges = this._ranges, params = this._renderParam,sel = this;
		setTimeout(function(){
	...
	_renderSelectionInPage: function( range, page, params )
	{
		...
		var doc = writer.util.ViewTools.getDocument(range.rootView);
		...
		var start = range.getStartView(), end = range.getEndView();
		if(!start)
			return;
		var startPage = writer.util.ViewTools.getPage( start.obj),
		endPage = writer.util.ViewTools.getPage(end.obj),
	...
	_renderSelection: function( startObj, startIndex, endObj, endIndex, params ){
		...
		var sel = this, endLine, startLine,
		...
		if(!(view.isTable(endObj)||view.isRow(endObj)||view.isCell(endObj))){
			...
			else if( endIndex != null && endObj.getContainer() )
		    {
				if( endIndex < endObj.getContainer().length())
				{
				 ...
				 {
					 while( endIndex && view.getParent( startObj, function( parent ){ return parent == endObj;}))
					 {
		...
		var range = new writer.core.Range( {"obj": startObj, "index":startIndex}, {"obj": endObj,"index": endIndex});
		...
		if( view.isDrawingObj(endObj) && ( writer.util.ModelTools.isAnchor( endObj)|| endIndex == 0 ) ){
			endObj = sel._rangeTools.getPrevRun( endObj );
			endIndex = endObj && endObj.getEnd();
		}
		
		while( viewItem && endObj ){
			var select, startLineIdx = null, endLineIdx = null, isBidiLine = viewItem.isBidiLine && viewItem.isBidiLine();
			if( viewItem == startLine ){
				if(isBidiLine)
					startLineIdx = startIndex + (startObj.left == viewItem.paddingLeft ? 0 : startObj.start);
				else
			...
			if(viewItem == endLine ){
				if(isBidiLine)
					endLineIdx = endIndex + (endObj.left == viewItem.paddingLeft ? 0 : endObj.start);
				else
...
//				console.log("!!!!!Anchor");
	...
	beginSelect: function(pos, onBoundary){
		...
		if (onBoundary === true) {
			...
			if (this._viewTools.isTextRun(pos.obj)) {
				var modelpos = {obj: writer.util.ModelTools.getParagraph(pos.obj.model),
						        index:pos.obj.start + pos.index};
				var cbs = writer.util.ModelTools.getCharBoundaries(modelpos);
	...
	destroy: function(){
		...
		if( this._ranges && this._ranges.length > 0 )
		{
			...
			{
				if (!this._coEditIndicator)
					dojo.publish( writer.EVENT.REMOVE_SELECT, [this._ranges] );
			}
			catch(e)
			{
				console.error("error when publish writer.EVENT.REMOVE_SELECT:" + e);
			}
		}
		this._ranges = [];
	},
	...
	_getSelectionMsg: function(force)
	{
		...
		if (force || !this._msgData || !this._compareSelectionData(data, this._msgData))
		{
			...
			this._msgData = data;
			return WRITER.MSG.createSelectionMsg(data);
		}
	...
	_compareSelectionData: function(current, before)
	{
		...
		for (var i = 0; i < current.length; i++)
		{
			var c = current[i];
			var b = before[i];
			if (c.startParaId != b.startParaId || c.endParaId != b.endParaId || c.startParaIndex != b.startParaIndex || c.endParaIndex != b.endParaIndex
					|| c.rId != b.rId || c.pageNumber != b.pageNumber)
				return false;
		}
		return true;
	},
	...
	selectRanges: function( ranges,effect ){
		this.destroy();
		if (!this._coEditIndicator)
			dojo.publish( writer.EVENT.BEFORE_SELECT,[ranges] );
	...
	getHighlightRanges: function(ranges, currentRange){
		...
		else
		{
			...
			var queryRanges,highlights = [],pageNum, tmpPage;
			if(ranges.length > 0)
			{	
				queryRanges = ranges;
	...
	_getRangesForScroll2:function(queryRanges,pageNum,highlights){
		if(queryRanges[0])
		{
			var rangePageNum = writer.util.ViewTools.getPage(queryRanges[0].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
	...
	_getRangesForScroll:function(queryRanges,pageNum){
		var highlights = [];
		var rangePageNum = writer.util.ViewTools.getPage(queryRanges[queryRanges.length - 1].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
		while( rangePageNum >= (pageNum-2) && rangePageNum <= (pageNum+2))
		{
			highlights.push(queryRanges.pop());
			if(queryRanges[queryRanges.length - 1])
				rangePageNum = writer.util.ViewTools.getPage(queryRanges[queryRanges.length - 1].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
			else
				break;
		}
	...
	getAppendRangesForScroll:function(pageNum){
		var rangePageNum = -1,queryRanges = [],highlights = [],index = Math.round(this._highlightWaitingList.length/2),half = index,
		startNum = writer.util.ViewTools.getPage(this._highlightWaitingList[0].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
		while((rangePageNum < 0 || rangePageNum < pageNum - 2 ) || (rangePageNum > pageNum + 2))
		{
			...
			if(range){
				rangePageNum = writer.util.ViewTools.getPage(range.getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
	...
	getAppendRangesForNavigate:function(pageNum){
		var rangePageNum = -1, ranges = [], index = 0;
		while(this._highlightWaitingList.length > 0)
		{
			rangePageNum = writer.util.ViewTools.getPage(this._highlightWaitingList[0].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
			if(rangePageNum == pageNum)
				ranges.push(this._highlightWaitingList.shift());
			else
			{
				rangePageNum = writer.util.ViewTools.getPage(this._highlightWaitingList[this._highlightWaitingList.length - 1].getStartParaPos().obj.getAllViews().rootView.getFirst()).pageNumber;
				if(rangePageNum == pageNum)
	...
	selectRangesBeforeUpdate: function(ranges)
	{
		if( this._ranges && this._ranges.length > 0 )
		{
			try
			{
				...
				if (!this._coEditIndicator)
					dojo.publish( writer.EVENT.REMOVE_SELECT, [this._ranges, beforeUpdate] );
			}
			catch(e)
			{
				console.error("error when publish writer.EVENT.REMOVE_SELECT:" + e);
	...
	select: function( start, end ){
		 this.selectRanges( [new writer.core.Range( start, end )]);
	},
	/*
	 * Select from current start to new pos
	 */
	selectTo: function(pos, onBoundary){
		this._cursor.hide();
		if (onBoundary === true) {
			// make sure the pos is a char boundary
			if (this._viewTools.isTextRun(pos.obj)) {
		...
		var range = new writer.core.Range(this._start,pos);
		if (!this._rangeTools.ifContainOnlyOneDrawingObj(range))
			this.scrollIntoView(false, false, new writer.core.Range( pos, pos ));
	},
	...
	moveLeft: function( bShift, bCtrl, nOffset ){
		...
		if (bCtrl && nOffset == null) {
			var prevwb = writer.util.ModelTools.getPreviousWordPosition(modelpos);
			if (prevwb >= 0)
				nOffset = modelpos.index + modelpos.obj.start - prevwb;
		}
		if(nOffset == null) {
			var prevcb = writer.util.ModelTools.getPrevCharBoundary(modelpos);
			if (prevcb >= 0)
	...
	moveRight: function( bShift, bCtrl, nOffset){
		...
		var modelpos = range._viewToModel(pos);
		if (bCtrl && nOffset == null) {
			var nextwb = writer.util.ModelTools.getNextWordPosition(modelpos);
			if (nextwb >= 0)
				nOffset = nextwb - modelpos.index - modelpos.obj.start;
		}
		if(nOffset == null) {
			var nextcb = writer.util.ModelTools.getNextCharBoundary(modelpos);
			if (nextcb >= 0)
...
//			this.select(pos, pos);
			...
			while(para = iterator.nextParagraph()){
				if (para){
					...
					if(startModel== para && endModel== para){
						text += para.text.substring(startIndex, endIndex);
						break;
					}else if(startModel== para){
						text += para.text.substring(startIndex);
					}else if(endModel== para){
						text += para.text.substring(0, endIndex);
						break;
					}else{
						text += para.text;
					}
	...
	getRunInfo: function(runView, nls, endIndex){
		...
		if(text.trim()!="")
			text = " " + text +" ";
		return {text:text, endIndex:endIndex};
	},
	...
	getCommonInfo: function(runView, nls){
		...
		if(cell && row){
			return  " " + dojo.string.substitute(nls.acc_inTable,[row.getRowIdx()+1, cell.getColIdx()+1]) + " ";
		}
		return "";
	},
	...
	getEditModeText: function(mode, nls){
		if(EDITMODE.HEADER_FOOTER_MODE == mode)
			return nls.ACC_headerFooterMode;
		if(EDITMODE.EDITOR_MODE == mode)
			return nls.ACC_EditorMode;
		if(EDITMODE.FOOTNOTE_MODE == mode)
			return nls.ACC_FootnotesMode;
		if(EDITMODE.ENDNOTE_MODE == mode)
	...
	AnnounceSelection: function(opt, bShift){
		if(dojo.isFF){
			if(writer.util.ViewTools.getCurrSelectedImage()){
				return;
			}
			var text = "";
			...
			if(bShift || opt == "all"){
				...
				if(this._ranges && this._ranges.length > 0 ){
					...
					var modeChange = false, pageChange = false, sectionChange = false, bodyChange = false;
					if(!this.posInfo){
						this.posInfo = {};						
					...
					if(pageChange){
						...
						posText +=  dojo.string.substitute(nls.acc_page,[newPage.pageNumber])  + " ";							
					}
					if(sectionChange){
						this.posInfo.sectionIndex  = newSectionIndex;
						posText +=  dojo.string.substitute(nls.acc_section,[newSectionIndex + 1])  + " ";							
					}
					if(bodyChange){
						...
						while(body){
							if(body == newBody ){
								posText +=  dojo.string.substitute(nls.Acc_column,[index])  + " ";
								break;
							}
							body = body.next(true);
							index ++;
	...
	selectOneWord: function(pos) {
		if (pos.obj && this._viewTools.isRun(pos.obj) && this._viewTools.isLine(pos.obj.getParent())) {
			...
			if (pos.index == 0 && first == pos.obj) {
				var nextboundary = writer.util.ModelTools.getNextWordPosition(modelpos);
				if (nextboundary > 0)
					this.select(pos, {index:nextboundary, obj:modelpos.obj});
			} else {
	...
	getWordRange: function( modelpos ){
		...
		var wordrange = writer.util.ModelTools.getCurrentWordRange(modelpos, range.getRootView());
	...
	_moveToLine: function( line, offset, bShift ){
		...
		if( run ){
			if( bShift ){
				this.selectTo( {'obj':  run, 'index': index } );
			}else{
	...
	nextParagraph: function( bShift ){
		...
		var obj = pos.obj, index = pos.index, nextPos;
		var tools = this._rangeTools, viewTools = this._viewTools;
		...
		function getlastPos( para ){
			if( para ){
				...
				return {"obj": last, "index":  last.len };
			}
		}
		...
		else
		{
			...
			if( last )
			{
				if( index == last.len && obj == last )
					...
					nextPos = {"obj": last, "index": last.len };
			}
		}
	...
	prevParagraph: function( bShift ){
		...
		function getFirstPos( para ){
			...
			return firstRun && {"obj": firstRun, "index": 0 };
		}
		if( viewTools.isRow( obj )||viewTools.isTable(obj)){
	...
	prevAnchorObj: function( bShift ){
		...
		var prevAnchor = writer.util.ModelTools.getPrev( modelpos.obj,  writer.util.ModelTools.isAnchor);
		if( prevAnchor ){
			var drawingStart = {"obj": prevAnchor, "index" : 0};
			var drawingEnd = {"obj": prevAnchor, "index": 1};
	...
	nextAnchorObj: function( bShift ){
		...
		var nextAnchor = writer.util.ModelTools.getNext( modelpos.obj,  writer.util.ModelTools.isAnchor);
		if( nextAnchor ){
			var drawingStart = {"obj": nextAnchor, "index" : 0};
			var drawingEnd = {"obj": nextAnchor, "index": 1};
			this.select(drawingStart, drawingEnd);
		}
	},
	/*
		...
		while( prev ){
			if(inCell){
				...
				if(preCell&& inCell !=preCell){
					var delX = pt.x - tools.getBodyPosition({obj:inCell}).x - inCell.getLeftEdge();
					pt = tools.getBodyPosition({obj:prev});
					pt.x += delX;
				}				
			}
			...
			if (pos) {
					...
					viewTools.isDrawingObj(pos.obj)) && !bShift )
				{
					...
					if (viewTools.isInlineDrawingObj(drawingObj) )
					{
						...
						var drawingStart = {"obj": drawingObj, "index" : 0};
						var drawingEnd = {"obj": drawingObj, "index": 1};
						this.select(drawingStart, drawingEnd); 
						break;
					}
					else
	...
	_checkBoundary: function( start, end, bUp  ){
		var startObj = start.obj,  endObj = end.obj, obj, block;
	...
	_getTextItemByPosition: function( pt, txbx ){
		var path;
		var options = {"notAnchor": !this._viewTools.isDrawingObj( this.getEditView()) }
		if( txbx ){
		...
		{
			var rootView=window.layoutEngine.rootView, pos;
		...
		while( next ){
			if(inCell){
				...
				if(nextCell&&inCell!=nextCell){
					var delX = pt.x - tools.getBodyPosition({obj:inCell}).x - inCell.getLeftEdge();
					pt = tools.getBodyPosition({obj:next});
					pt.x += delX;					
				}			
			}
	...
	lineEnd: function( bShift, bCtrl ){
		...
		if( bCtrl ){
			...
			newPos = {'obj':run, 'index':run.len };
		}
		else{
			var pos = this._getPosition(this._ranges[0], true );
			if( pos ){
				var line = pos.obj.getParent();
				var lastRun = writer.util.ViewTools.last( line ),index = lastRun.len;
				if( lastRun.getViewType && lastRun.getViewType()== 'text.LineBreak' ){
					index--;
				}
				newPos = { 'obj': lastRun, 'index': index};
			}	
		}
		if( bShift )
			this.selectTo(newPos);
	 ...
	 */
	scrollIntoView: function(toTop, toEnd, range, tovwcenter, bcheckonly){
		...
		// if drawing obj?
		var drawingObj = writer.util.ViewTools.getCurrSelectedDrawingObj();
		if (drawingObj)
			return;
		...
		if( pos ){
			var page = writer.util.ViewTools.getPage( pos.obj );
			...
			else if( ( scrollTo  + pos.obj.h ) >= ( scrollTop + viewHeight ) )
			{//in the bottom
...
//		this.select( {'obj':firstRun, 'index': 0 },{'obj':lastRun, 'index':lastRun.len });
		...
		var lastObj = container.getLast(),lastIndex;
		...
		this.select( {'obj':firstObj, 'index': 0 },{'obj':lastObj, 'index': lastIndex });
	},
	...
	isPositionInSelection: function(offsetX, offsetY) {
		for (var i = 0; i < this._selections.length; i++) {
			if (this._selections[i].isPointInSelection && this._selections[i].isPointInSelection({x:offsetX, y:offsetY}))
				return true;
		}
