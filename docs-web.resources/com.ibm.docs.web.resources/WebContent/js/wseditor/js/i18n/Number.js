...
//		},
	...
	{		
		...
		case this.FormatType["CURRENCY"]:
			...
			format[wcs.FORMATCURRENCY] = currency ? currency : this.getLocaleIso();
			break;
		case this.FormatType["DATE"]:
	...
	/*string*/generalFormatNumber: function (/*number*/ number) {
		...
		var numberstr = number.toString().toUpperCase();
		...
		if (parts[1] && parts[1].length == 1) {
			if (parts[0].indexOf('.') == -1 || (parts[0].replace(/^\d+\./, '').length + parseInt(parts[1])) < this.MIN_EXP_NUM) {
				result = websheet.Helper.convertSciNum2General(result);
			} else {
				// convert E+8 to E+08
				parts[1] = '0' + parts[1];
	 ...
	 */
	/*string*/roundFloat: function(/*float*/value, maxDigitNum) {
		...
		if (idx != -1) {
			...
			var bRounded;
			var maxdigitnum = (maxDigitNum == null ? this.MAX_DIGIT_NUM : maxDigitNum)  - idx;
		...
		if(eIdx != -1){
			...
			if ((exp >= 0 && exp < this.MIN_EXP_NUM) || (exp < 0 && (decLen - exp) < this.MIN_EXP_NUM)) {
				return websheet.Helper.convertSciNum2General(str);
			}
		}
		return str;
	},
	...
	/*string*/formatNumber: function(showString, /*format*/format, oldFormat, isEditValue){	    	
	    var wcs = websheet.Constant.Style;
...
//		dojo["requireLocalization"]("dojo.cldr",'number', locale);
		...
		try{
			...
			if(!websheet.Helper.isSciNum(showString) && (options.places != null || options.round != null))
...
//				options.enPattern = true;//the code is en_us pattern, but should use bundle.group and decimal for result
	...
	/*string*/formatFraction: function(showString, /*format*/format){
		...
		if (websheet.Helper.isSciNum(showString)) {
			showString = websheet.Helper.convertSciNum2General(showString);
		}
	...
	/*string*/formatCurrency: function(showString, /*format*/format, /*boolean*/ignoreNormalize){
		...
		if(options.pattern && options.pattern.indexOf("_") >= 0) //not support place holder yet, so remove it
			options.pattern = options.pattern.replace(/_./, ""); // "#,##0.00_- €" ==> "#,##0.00 €"
		if(!ignoreNormalize)
		...
		if(intPlace != undefined && intPlace){
			...
			//negative brackets or currency is in the end, for example "-1€"
	...
	/*string*/formatDate: function(showString, /*format*/format, isEditValue){
		if (!websheet.Helper.checkDateValidSpan(showString)) {
			return isEditValue ? showString.toString() : websheet.Constant.ERRORCODE["1004"];
		}
		...
		try{
			...
			rawDate = new Date(ms);
			var dif=websheet.Helper.getMSDifByTimezoneOffset(websheet.Helper.getTimezoneOffset(rawDate),websheet.Helper.getTimezoneOffset(dtTmp));
			if(dif!=0){
	...
	/*string*/formatTime: function(showString, /*format*/format, isEditValue) {
		if (!websheet.Helper.checkDateValidSpan(showString)) {
			return isEditValue ? showString.toString() : websheet.Constant.ERRORCODE["1004"];
		}
		var rawDate = showString;
	...
	/*string*/formatText:function(showString, format){
		...
		if (typeof showString == "number"){
			if (!websheet.Helper.isSciNum(showString)){
				var parts = showString.toString().split(".");
				if (parts[0].length > this.MIN_EXP_NUM || (parts[0] == "0" && parts[1] && parts[1].length >= this.MIN_EXP_NUM)) {
					showString = showString.toExponential();
				}
			}
	 ...
	 */
	/*string*/format: function(showString, /*format*/ format, type, isEditValue) {
...
//						return showString;
			...
			if(formatType == this.FormatType["BOOLEAN"] && type != constct.BOOLEAN){
				...
				{
				case this.FormatType["NUMBER"]:
					showString = this.formatNumber(showString, newFormat, format, isEditValue);
					break;
				case this.FormatType["CURRENCY"]:
				...
				case this.FormatType["PERCENT"]:
					...
					break;
				case this.FormatType["DATE"]:
					showString = this.formatDate(showString, newFormat, isEditValue);
					break;
				case this.FormatType["TIME"]:
					showString = this.formatTime(showString, newFormat, isEditValue);
					break;
				case this.FormatType["TEXT"]:
				...
				default:
					if (typeof(showString) == "number") {
						try {
							var origShowString = showString;
							showString = this.formatNumber(showString, newFormat, format, isEditValue);
						} catch(e){
							showString = origShowString;
						}
					}
					break;
				}
