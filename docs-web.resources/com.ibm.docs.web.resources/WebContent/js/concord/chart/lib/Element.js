dojo.provide("concord.chart.lib.Element");

dojo.require("dojox.gfx");

dojo.declare("concord.chart.lib.Element", null, {
	//	summary:
	...
	createGroup: function(creator){
		...
		//	returns: concord.chart.lib.Element
		//		A reference to this object for functional chaining.
		if(!creator){ creator = this.chart.surface; }
		if(!this.group){
			this.group = creator.createGroup();
		}
		return this;	//	concord.chart.lib.Element
	},
	purgeGroup: function(){
		//	summary:
	...
	createPath: function(s,path)
	{
		var gfx = dojox.gfx;
		var shape = new gfx.Path();
		var node = null;
		if(dojo.doc.createElementNS){
			node = dojo.doc.createElementNS(gfx.svg.xmlns.svg,gfx.Path.nodeType);
		}else{
			node = dojo.doc.createElement(shape.nodeType);
		}
		shape.rawNode = node;
		shape.setShape(path);
		s.add(shape); 
	...
	createRect: function(s,rect)
	{
		var gfx = dojox.gfx;
		var shape = new gfx.Rect();
		var node = null;
		if(dojo.doc.createElementNS){
			node = dojo.doc.createElementNS(gfx.svg.xmlns.svg,gfx.Rect.nodeType);
		}else{
			node = dojo.doc.createElement(shape.nodeType);
		}
		shape.rawNode = node;
	...
	destroyHtmlElements: function(){
		...
		if(this.htmlElements.length){
			dojo.forEach(this.htmlElements, function(entry){
	...
	getTextWidth: function(s, font){
		var escS = this.escapeXml(s);
		if(typeof font !="string")
			font = this.makeFontString(font);
		return dojox.gfx._base._getTextBox(escS, {font: font}).w || 0;
	},
	
	makeFontString: function(font)
	{
		if(font==null)
			return "normal normal normal 10pt Arial";
		if(typeof font =="string")
			return font;
		return (font.style ||"normal") + " " + (font.variant || "normal") + " " + (font.weight || "normal") + " " + (font.size || "10pt") + " " + (font.family || "Arial"); // Object
	},
	
	escapeXml: function(s){
		if(typeof s != "string")
			return s;
		return s? s.replace(/&/gm, "&amp;").replace(/</gm, "&lt;").replace(/>/gm, "&gt;").replace(/"/gm, "&quot;").replace(/'/gm, "&#39;") : "";
	},
