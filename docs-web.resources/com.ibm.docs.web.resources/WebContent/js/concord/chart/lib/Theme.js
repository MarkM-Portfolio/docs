dojo.provide("concord.chart.lib.Theme");
...
dojo.declare("concord.chart.lib.Theme", null, {
	//	summary:
	...
	//		on a concord.chart.lib.Chart2D object), a Theme simplifies this manual setup by allowing you to
	...
	//
	//		Defining a new theme is relatively easy; see any of the themes in concord.chart.lib.themes for examples
	...
	//	|	concord.chart.lib.themes.Grasslands = new concord.chart.lib.Theme({
	//	|		colors: [ "#70803a", "#dde574", "#788062", "#b1cc5d", "#eff2c2" ]
	//	|	});
	//	|
	//	|	myChart.setTheme(concord.chart.lib.themes.Grasslands);
	...
	constructor: function(kwArgs){
		...
		var def = concord.chart.lib.Theme.defaultTheme;
		...
		if(kwArgs.seriesThemes && kwArgs.seriesThemes.length){
			...
			this.colors = (kwArgs.colors || concord.chart.lib.Theme.defaultColors).slice(0);
		}
		this.markerThemes = null;
		if(kwArgs.markerThemes && kwArgs.markerThemes.length){
			this.markerThemes = kwArgs.markerThemes.slice(0);
		}
		this.markers = kwArgs.markers ? dojo.clone(kwArgs.markers) : dojo.delegate(concord.chart.lib.Theme.defaultMarkers);
	...
	clone: function(){
		...
		//	returns: concord.chart.lib.Theme
		//		The cloned theme; any alterations made will not affect the original.
		var theme = new concord.chart.lib.Theme({
			// theme components
			chart: this.chart,
		...
		return theme;	//	concord.chart.lib.Theme
	},
	...
	post: function(theme, elementType){
		...
		if(!this.noGradConv && fill!=null && this.shapeSpaces[fill.space] && fill.type == "linear"){
			if(elementType == "bar"){
...
/*=====
concord.chart.lib.Theme.__DefineColorArgs = function(num, colors, hue, saturation, low, high, base, generator){
	//	summary:
...
=====*/
dojo.mixin(concord.chart.lib.Theme, {
	defaultMarkers: {
	...
	defineColors: function(kwArgs){
		...
		//		arguments.
		//	kwArgs: concord.chart.lib.Theme.__DefineColorArgs
		//		The arguments object used to define colors.
		//	returns: dojo.Color[]
		//		An array of colors for use in a theme.
		//
		//	example:
		//	|	var colors = concord.chart.lib.Theme.defineColors({
		//	|		base: "#369",
		//	|		generator: "compound"
		//	|	});
		//
	...
	generateHslGradient: function(color, fillPattern, lumFrom, lumTo){
		...
		return concord.chart.lib.Theme.generateGradient(fillPattern, colorFrom, colorTo);	// Object
	}
});
