dojo.provide("concord.chart.lib.plot2d.common");
...
(function(){
	...
	dojo.mixin(concord.chart.lib.plot2d.common, {
		...
		collectSimpleStats: function(series){
			...
			for(var i = 0; i < series.length; ++i){
				...
				for(var j = 0; j < run.data.length; j++){
					...
					{
						stats.hmin = Math.min(stats.hmin, j+1);
						stats.hmax = Math.max(stats.hmax, j+1);
					}
				}
			}
			return stats;
		...
		calculateBarSize: function(/* Number */ availableSize, /* Object */ opt, /* Number? */ clusterSize){
			...
			var gap = opt.gap;
			clusterSize = clusterSize + gap/100;
			var size = availableSize / clusterSize;
		...
		collectStackedStats: function(series){
			...
			if(series.length){
				...
				var acc = new Array(stats.hmax);
				for(var i=0;i<series.length;i++)
				{
					for(var j = 0; j < stats.hmax; ++j)
					{
						var v = series[i].data[j];
						...
						if(i==0)
							...
							acc[j] += v;
						if(stats.vmin==null || acc[j]<stats.vmin)
							stats.vmin = acc[j];
						if(stats.vmax==null || acc[j]>stats.vmax)
							stats.vmax = acc[j];
					}
				}
			}
		...
		collectStackedBarStats: function(series){
			// collect statistics
			var stats = dojo.clone(dc.defaultStats);
			if(series.length){
				// 1st pass: find the maximal length of runs
				...
				for(var i = 0; i < stats.hmax; ++i){
					...
					if(firstNV!=null)
						stats.vmin = Math.min(stats.vmin, nv);
					else
						stats.vmin = Math.min(stats.vmin, firstPV);
					
					if(firstPV!=null)
						stats.vmax = Math.max(stats.vmax, v);
					else
						stats.vmax = Math.max(stats.vmax, firstNV);
				}
			}
			return stats;
		},
