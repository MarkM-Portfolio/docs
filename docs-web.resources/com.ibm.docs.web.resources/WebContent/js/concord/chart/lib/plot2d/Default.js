dojo.provide("concord.chart.lib.plot2d.Default");
...
/*=====
dojo.declare("concord.chart.lib.plot2d.__DefaultCtorArgs", concord.chart.lib.plot2d.__PlotCtorArgs, {
	//	summary:
...
(function(){
	...
	dojo.declare("concord.chart.lib.plot2d.Default", concord.chart.lib.plot2d.Base, {
		defaultParams: {
		...
		render: function(dim, offsets){
			...
			//	returns: concord.chart.lib.plot2d.Default
			...
			if(this.dirty){
				...
				var s = this.group;
				df.forEach(this.series, function(item){ item.cleanGroup(s); });
			}
			...
			for(var i = this.series.length - 1; i >= 0; --i){
                ...
                for(var j = 0; j < run.data.length; j++){
                	var v = run.data[j];
                    if(typeof v == "number" || (v!=null && v.x!=null && v.y!=null))
                    {
                        if(!rseg){
                            rseg = [];
                            startindexes.push(j);
                ...
                for(var seg = 0; seg < rsegments.length; seg++){
					...
					if(this.opt.interpolate){
						var rsegment = rsegments[seg + 1];
						if(rsegment){
							lpoly = lpoly.concat(dojo.map([rsegment[0]], function(v, i){
								return {
									x: ht(i + v.x) + offsets.l,
									y: dim.height - offsets.b - vt(v.y)
								};
							}, this));
						}
					...
					if(stroke && theme.series.shadow && lpoly.length > 1){
						...
						if(markers && theme.marker.shadow){
...
//						}
			...
			this.dirty = false;
			return this;	//	concord.chart.lib.plot2d.Default
		}
	});
})();
