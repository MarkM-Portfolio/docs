dojo.provide("concord.chart.lib.plot2d.Spider");

dojo.experimental("concord.chart.lib.plot2d.Spider");
...
dojo.require("concord.chart.lib.scaler.primitive");
...
(function(){
	...
	dojo.declare("concord.chart.lib.plot2d.Spider", [concord.chart.lib.Element, concord.chart.lib.plot2d._PlotEvents], {
		//	summary:
		...
		clear: function(){
			...
			//	returns: concord.chart.lib.plot2d.Spider
			...
			this.oldSeriePoints = {};
			this.animations = {};
			return this;	//	concord.chart.lib.plot2d.Spider
		},
		...
		addSeries: function(run){
			...
			//	run: concord.chart.lib.Series
			//		The series to be added.
			//	returns: concord.chart.lib.plot2d.Base
			//		A reference to this plot for functional chaining.
			var matched = false;
			...
			return this;	//	concord.chart.lib.plot2d.Base
		},
		getSeriesStats: function(){
			...
			return concord.chart.lib.plot2d.common.collectSimpleStats(this.series);
		},
		...
		initializeScalers: function(dim, stats){
			...
			if(this._hAxis){
				...
				this._hScaler = concord.chart.lib.scaler.primitive.buildScaler(stats.hmin, stats.hmax, dim.width);
			}
			if(this._vAxis){
				...
				this._vScaler = concord.chart.lib.scaler.primitive.buildScaler(stats.vmin, stats.vmax, dim.height);
			}
			return this;	//	concord.chart.lib.plot2d.Base
		},
		render: function(dim, offsets){
			...
			var o = this.opt, ta = t.axis,
				...
				asize = g.normalizedLength(axisFont.size),
			...
			for (var j = labelPoints.length - 1; j >= 0; --j) {
				var point = labelPoints[j],
					fontWidth = this.getTextWidth(this.labelKey[j],axisFont),
			...
			for(var key in this.datas){
				...
				for (var i = 0; i < dv; i++) {
					...
					var fontWidth = this.getTextWidth(text, axisTickFont),
						render = this.opt.htmlLabels && dojox.gfx.renderer != "vml" ? "html" : "gfx";
					if (this.opt.htmlLabels) {
			...
			if(o.element == "spider_poly"){
				...
				if(o.type == "onmouseover"){
					...
					if(dijit && dijit.Tooltip){
						dijit.showTooltip(this.escapeXml(o.tdata.sname)+ "<br/>" + this.escapeXml(o.tdata.key) + "<br/>" + this.escapeXml(o.tdata.data), this.aroundRect, position);
					}
				}else{
