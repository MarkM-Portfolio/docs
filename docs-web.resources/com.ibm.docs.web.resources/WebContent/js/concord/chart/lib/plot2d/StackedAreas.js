dojo.provide("concord.chart.lib.plot2d.StackedAreas");

dojo.require("concord.chart.lib.plot2d.Stacked");

dojo.declare("concord.chart.lib.plot2d.StackedAreas", concord.chart.lib.plot2d.Stacked, {
	//	summary:
	...
	render: function(dim, offsets){
		...
		for(var i = 0; i < this.series.length; ++i){
			...
			if(i>0)
			{
				lastpoly = dojo.map(acc, function(v, i){
					return {
						x: ht(i + 1) + offsets.l,
			...
			if(this.opt.areas){
				...
				else 
				{
					...
					{
						lastpoly = lastpoly.reverse();
						apoly = apoly.concat(lastpoly);
					}
					apoly.push(lpoly[0]);
			...
			if(this.opt.lines){
				...
				if(this.opt.tension){
					...
					run.dyn.stroke = s.createPolyline(apoly).setStroke(theme.series.stroke).getStroke();
				}
			}
			if(this.opt.markers){
