dojo.provide("concord.chart.lib.plot2d.Pie");
...
dojo.require("concord.chart.lib.plot2d._PlotEvents");
...
/*=====
dojo.declare("concord.chart.lib.plot2d.__PieCtorArgs", concord.chart.lib.plot2d.__DefaultCtorArgs, {
	//	summary:
...
(function(){
	var df = dojox.lang.functional, du = dojox.lang.utils,
		...
		da = concord.chart.lib.axis2d.common,
		g = dojox.gfx, m = g.matrix,
		FUDGE_FACTOR = 0.2; // use to overlap fans

	dojo.declare("concord.chart.lib.plot2d.Pie", [concord.chart.lib.Element, concord.chart.lib.plot2d._PlotEvents], {
		//	summary:
		...
		constructor: function(chart, kwArgs){
			...
			this.run = null;
			this.dyn = [];
			this.hAxis = kwArgs.hAxis;
			this.vAxis = kwArgs.vAxis;
		},
		clear: function(){
			//	summary:
			//		Clear out all of the information tied to this plot.
			//	returns: concord.chart.lib.plot2d.Pie
			//		A reference to this plot for functional chaining.
			this.dirty = true;
			this.dyn = [];
			this.run = null;
			return this;	//	concord.chart.lib.plot2d.Pie
		},
		...
		render: function(dim, offsets){
			...
			//	returns: concord.chart.lib.plot2d.Pie
			//		A reference to this plot for functional chaining.
			if(!this.dirty && !this.run.dirty){ return this; }
			this.resetEvents();
			this.dirty = false;
			this._eventSeries = {};
			this.cleanGroup();
			...
			var rx = (dim.width  - offsets.l - offsets.r) / 2 - 4,
				ry = (dim.height - offsets.t - offsets.b) / 2 - 4;
			if(rx<2)
				rx = 2;
			if(ry<2)
				ry = 2;
			...
			if(typeof run[0] == "number"){
				filteredRun = df.map(run, "x ? Math.abs(x) : 0");
				...
				//if(this.opt.labels)
				{
					labels = dojo.map(slices, function(x,i){
						var v = run[i];
						var f = v>=0 ? 1:-1;
						return x > 0 ? this._getLabel(f * x * 100) + "%" : "";
					}, this);
				}
			}else{
				filteredRun = df.map(run, "x ? Math.abs(x.y) : 0");
				if(df.every(filteredRun, "<= 0")){
					return this;
				}
				slices = df.map(filteredRun, "/this", df.foldl(filteredRun, "+", 0));
				//if(this.opt.labels)
				{
					labels = dojo.map(slices, function(x, i){
						...
						var v = run[i];
						var f = v.y>=0 ? 1:-1;
						return "text" in v ? v.text : this._getLabel(f * x * 100) + "%";
					}, this);
				}
			}
			var themes = df.map(run, function(v, i){
				...
				//if(v.color==null)
					//return t.next("slice", [this.opt], true);
			...
			if(this.opt.labels){
				shift = df.foldl1(df.map(labels, function(label, i){
					...
					return this.getTextWidth(label,font);
				}, this), "Math.max(a, b)") / 2;
			...
			dojo.some(slices, function(slice, i){
				...
				if(slice == 0){
					this.dyn.push({fill: themes[i].series.fill, stroke: themes[i].series.stroke});
				  return false;
				}
				...
				if(slice >= 1){
					...
					if(events){
						var dataLabel = this._getLabel(typeof v == "number" ? v: v.y);
						dataLabel += " (" + labels[i] + ")";
						var o = {
							element: "slice",
							index:   i,
			...
			if(this.opt.labels){
				if(this.opt.labelStyle == "default"){
					...
					dojo.some(slices, function(slice, i){
						...
						if(slice >= 1){
							// whole pie
							var v = run[i], elem = da.createText["gfx"](
					...
					var labelHeight = dojox.gfx._base._getTextBox("a",{font:this.makeFontString(taFont)}).h;
					...
					dojo.forEach(labeledSlices, function(slice, i){
						if (!slice.omit) {
							var leftColumn = circle.cx - circle.r * 2,
								...
								labelWidth = this.getTextWidth(labels[i], taFont),
