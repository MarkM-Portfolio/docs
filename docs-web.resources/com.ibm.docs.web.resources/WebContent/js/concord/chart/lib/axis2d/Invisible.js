dojo.provide("concord.chart.lib.axis2d.Invisible");

dojo.require("concord.chart.lib.scaler.linear");
dojo.require("concord.chart.lib.axis2d.common");
dojo.require("concord.chart.lib.axis2d.Base");
...
(function(){
	var dc = concord.chart.lib,
		...
		dcsc = dc.scaler.common,
		merge = du.merge,
		labelGap = 4,			// in pixels
		centerAnchorLimit = 45;	// in degrees

	dojo.declare("concord.chart.lib.axis2d.Invisible", concord.chart.lib.axis2d.Base, {
		//	summary:
		//		The default axis object used in concord.chart.lib.  See concord.chart.lib.Chart2D.addAxis for details.
		//
		...
		*/
		...
		initialized: function(){
			...
			return "scaler" in this && !(this.dirty && this.dependOnData()) && !this.scalerDirty;
		},
		setWindow: function(scale, offset){
			...
			this.scale  = scale;
			this.offset = offset;
			return this.clear();	//	concord.chart.lib.axis2d.Default
		},
		...
		getFont: function()
		{
			...
			return dojo.mixin({},(ta.majorTick && ta.majorTick.font) || (ta.tick && ta.tick.font), this.chart.font, this.opt.font);
		},
		
		getFontColor: function()
		{
			var ta = this.chart.theme.axis;
			return this.opt.fontColor || this.chart.fontColor || (ta.majorTick && ta.majorTick.fontColor) || (ta.tick && ta.tick.fontColor) || "black";
		},
		
		getTitleFont: function()
		{
			var ta = this.chart.theme.axis;
			return dojo.mixin({}, (ta.tick && ta.tick.titleFont), this.chart.font, this.opt.titleFont);
		},
		
		getTitleFontColor: function()
		{
			var ta = this.chart.theme.axis;
			return this.opt.titleFontColor || this.chart.fontColor || (ta.tick && ta.tick.titleFontColor) || "black";
		},
		...
		_groupLabelWidth: function(labels, font, wcLimit){
			...
			if (wcLimit) {
				labels = df.map(labels, function(label){
					...
					if(label.length>wcLimit)
						return label.substring(0, wcLimit) + this.trailingSymbol;
					else
						return label;
				}, this);
			}
			labels = df.map(labels,function(label){return this.escapeXml(label);}, this);
			var s = labels.join("<br>");
			return dojox.gfx._base._getTextBox(s, {font: this.makeFontString(font)}).w || 0;
		},
			...
			if(size){
				if(this.vertical ? rotation != 0 && rotation != 180 : rotation != 90 && rotation != 270){
					...
					if(this.labels){
						...
						labelWidth = this.getTextWidth(t.join(""),taFont);
					}
		...
		/**
		 * Gets adjusted x position of gfx text shape element, to make it
		 * compatible with original x position when label alignment 
		 * changes to value "start".
		 * To be used in IE with svg renderer, where svg
		 * text-anchor attribute with values 'middle' & 'end' 
		 * screwes up mixed Bidi & None-Bidi text.
		 */
		getStartOffset: function(x, label, labelAlign, taFont, maxLabelCharCount) {
			if(labelAlign == "end")
				return (x - this._groupLabelWidth([label], taFont, maxLabelCharCount));
			else if (labelAlign == "middle")
				return (x - this._groupLabelWidth([label], taFont, maxLabelCharCount) / 2);

		},
		/**
		 * format the v using the axis' number format 
		 * @param v
		 */
		formatString: function(v)
		{
			...
			return dcsc.getNumericLabel(v,prec,this.opt);
		}
	});
})();
