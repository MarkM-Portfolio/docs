...
], function(Command, Range, domConstruct, TableTools, Numbering, MeasureText, tools, Document, UpdateManager, Relations, Settings, Styles, Table, UndoManager, ColorPalette, Run, Space, TextArea) {

    describe("writer.test.ut.table.tableView", function() {
        MeasureText.init();
        tools.getDPI();
        ...
        it("render  ", function() {
            ...
            tableView.rows.forEach(function(row) {
                spyOn(row, "render").andCallFake(function() {
                    ...
                    var domNode = domConstruct.create("tr", {
                        "class": "row "
                    });
                    return domNode;
                });
        ...
        it("Table Tools get Rows in Range  ", function() {
            ...
            var rows = TableTools.getRowsInRange(first, last);
            expect(rows[0]).toEqual(first);
            expect(rows[rows.length - 1]).toEqual(last);
            rows = TableTools.getRowsInRange(last, first);
            expect(rows[0]).toEqual(first);
            expect(rows[rows.length - 1]).toEqual(last);
        ...
        it("Table plugins getStateBySel not with Repeat ", function() {
            ...
            var tablePlugins = new Table(edit);
        ...
        it("Table plugins repeatHeader", function() {
            ...
            pe.lotusEditor.addCommand = function(commandName, commandDef) {
                if (commandName == "repeatHeader")
                    pe.lotusEditor._commands[commandName] = new Command(pe.lotusEditor, commandDef, commandName);
            };
            var edit = {
                editor: pe.lotusEditor
            };
