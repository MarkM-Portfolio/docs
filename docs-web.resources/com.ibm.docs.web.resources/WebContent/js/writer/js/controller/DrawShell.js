...
define([
    ...
    "writer/controller/EditWindow"
], function(declare, lang, domGeometry, query, browser, EditWindow) {

    var DrawShell = declare("writer.controller.DrawShell", [], {
        ...
        itemByPoint: function(x, y) {
            var point = this.clientToLogical({
                'x': x,
                'y': y
            });
        ...
        _installRenderEvents: function() {
            ...
            this._editWindow = new EditWindow({
                shell: this
            });
        },
        ...
        screenToClient: function(point) {
            ...
            if (!browser.isMobile() || !this._domRect)
                this._domRect = domGeometry.position(this.domNode());
            var scale = this._editor.getScale();
            //          console.log("screen to client: "+point.y +"/" + this._domRect.y +"/"+scale);
            return (browser.contentInIframe()) ? {
