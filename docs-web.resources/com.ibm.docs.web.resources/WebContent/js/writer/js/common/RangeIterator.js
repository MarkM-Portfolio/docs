...
define([
    ...
    "writer/constants",
    "writer/global"
], function(declare, constants, g) {

    var RangeIterator = declare("writer.common.RangeIterator", null, {
        ...
        constructor: function(range, paragraphLimit) {
            ...
            try {
                ...
                if (!range.isSplited) {
                    ...
                    this._initWalker({
                        obj: this._start.obj,
                        index: this._start.index
                    }, {
                        obj: this._end.obj,
                        index: this._end.index
                    });
                } else {
        ...
        _isRichText: function(model) {
            return (model && model.modelType == constants.MODELTYPE.TEXT);
        },
        ...
        nextBlock: function() {
            return this.nextModel(g.modelTools.isBlock);
        },
        ...
        nextParagraph: function() {
            return this.nextModel(g.modelTools.isParagraph);
        },
        _initWalker: function(start, end) {
            ...
            if ((tools.isRun(start.obj) && !tools.isChildOf(end.obj, start.obj)) || (tools.isDrawingObj(start.obj) && !tools.isChildOf(end.obj, start.obj)) || tools.isParagraph(start.obj)) {
                ...
                if (nextBlock) {
                    ...
                    this._initWalker({
                        obj: startPara.parent,
                        index: startPara.parent.getContainer().indexOf(startPara) + 1
                    }, end);
                } else {
                    this._initWalker({
                        obj: startPara.parent,
                        index: g.modelTools.getLength(startPara.parent)
                    }, end);
                }
            } else {
                if (start.obj == end.obj) {
                    ...
                    for (var i = start.index; i < endIndex; i++) {
                        walkers.push(new _RangeIterator(firstChild));
                    ...
                    for (var i = start.index; i < endIndex && child; i++) {
                        if (child == end.obj || tools.getParent(end.obj, function(m) {
                                return m == child;
                            })) {
                            ...
                            walkers.push(new _RangeIterator(child));
                            child = child.next();
                        }
                    }
                ...
                if (next) {
                    ...
                    this._initWalker({
                        obj: start.obj.parent,
                        index: g.modelTools.getLength(start.obj.parent)
                    }, end);
                }
        ...
        _nextModel: function(walker, guard) {
            ...
            if (!this._currentModel) {
                ...
                if (index == null || index == 0 || index <= tools.getLength(this._currentModel) || (walker.start.obj == walker.end.obj && walker.start.index == walker.end.index)) {
                    ...
                    if (!guard || guard(this._currentModel)) {
                        ...
                        var currRun;
                        if (this._currentModel.modelType == constants.MODELTYPE.TEXT) {
            ...
            if (!walker.end.obj || tools.getParent(walker.end.obj, function(m) {
                    return m == current;
                })) {
        ...
        getRun: function(p, i, isLast) {
            if (!g.modelTools.isParagraph(p) && p.getContainer) {
                if (p.getContainer().length() == 0) // For empty shape, like arrow.
                    return null;
                p = p.getContainer().getByIndex(i);
                p = p && g.modelTools.getFirstChild(p,
                    g.modelTools.isParagraph, true);
                i = 0;
            }
    ...
    var _RangeIterator = declare("writer.common._RangeIterator", writer.common.RangeIterator, {
        _isRI: true,
        constructor: function(model) {
            ...
            this._end = {
                obj: model,
                index: g.modelTools.getLength(model)
            };
            if (!this._start || !this._end) {
                throw "range not right, should have start and end!!";
            }
