...
define([
...
], function(has, dtdModule, htmlParser, elementModule, fragment, fragmentClz, textModule, tools, constants, msgHelper) {

    var fragmentPrototype = fragmentClz.prototype,
        elementPrototype = elementModule.prototype;
     ...
     * @param tagName
     */
    dtdModule.parentOf = function(tagName) {
        var result = {};
        for (var tag in this) {
    ...
    function postProcessList(list) {
        var children = list.children,
            ...
            stylesFilter = pastefromword.filters.stylesFilter;
        ...
        for (var i = 0; i < count; i++) {
            ...
            if (match) {
                if (match[1] == mergeStyle || !mergeStyle) {
                    ...
                    //IE11
                    {
                        pastefromword.filters.stylesFilter(
                            [
    ...
    var pastefromword = {
        utils: {
            ...
            createListBulletMarker: function(bullet, bulletText, style) {
                ...
                var marker = htmlParser.createElementByTag('cke:listbullet', attributes);
                marker.add(new textModule(bulletText));
                return marker;
            },
            ...
            isContainingOnlySpaces: function(element) {
                var text;
                return ((text = element.onlyChild()) && !element.attributes["cke_tab"] && (/^(:?\s|&nbsp;)+$/).test(text.value));
            },
            ...
            resolveList: function(element) {
                ...
                if ((listMarker = element.removeAnyChildWithName('cke:listbullet')) && listMarker.length && (listMarker = listMarker[0])) {
                    ...
                    if (attrs.style) {
                        attrs.style = pastefromword.filters.stylesFilter(
                            [
                                ['line-height']
                            ])(attrs.style, element) || '';
                    }
                    // Inherit attributes from bullet.
                    tools.extend(attrs, listMarker.attributes);
                    return true;
                }
                return false;
            },
            ...
            listDtdParents: dtdModule.parentOf('ol')
        },
        ...
        filters: {
            ...
            flattenList: function(element, level) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if (child.name in dtdModule.$listItem) {
                        ...
                        if (last.name in dtdModule.$list) {
                    ...
                    // Flatten sub list.
                    else if (child.name in dtdModule.$list) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if ('cke:li' == child.name) {
                        ...
                        if (listItemAttrs.style) {
                            var marginleft = null;
                            var hanging = null;
                            listItemAttrs.style = pastefromword.filters.stylesFilter(
                                [
                                    ...
                                    [(/^mso-list$/), null, function(val) {
                                        ...
                                        htmlParser.listTypes[listId] = htmlParser.listTypes[listId] || {};
                                        htmlParser.listTypes[listId][indent] = {};
                                        if (marginleft)
                                            htmlParser.listTypes[listId][indent]["left"] = marginleft;
                                        if (hanging)
                                            htmlParser.listTypes[listId][indent]["hanging"] = hanging;
                        ...
                        if (listItemId == null) {
                            ...
                            if (/^Heading\d$/.test(styleId)) {
                                ...
                                listItemId = listItemAttrs["class"] || msgHelper.getUUID();
                                listItemIndent = 1;
                            }
                            ...
                            if (hanging && hanging.indexOf("-") == 0)
                                hanging = hanging.substring(1);
                            htmlParser.listTypes[listItemId] = htmlParser.listTypes[listItemId] || {};
                            htmlParser.listTypes[listItemId][listItemIndent] = {};
                            if (marginleft)
                                htmlParser.listTypes[listItemId][listItemIndent]["left"] = marginleft;
                            if (hanging)
                                htmlParser.listTypes[listItemId][listItemIndent]["hanging"] = hanging;
                        } else
                            listItemIndent = Number(listItemAttrs['cke:indent']);

                        var data = htmlParser.listTypes[listItemId][listItemIndent];
        ...
        getRules: function(config) {
            var dtd = dtdModule,
                blockLike = tools.extend({}, dtd.$block, dtd.$listItem, dtd.$tableContent),
            ...
            return {
                ...
                root: function(element) { //root node
                ...
                elements: {
                    '^': function(element) {
                        ...
                        if (has("ff") && (applyStyleFilter = filters.applyStyleFilter))
                    ...
                    'p': function(element) {
                        ...
                        if (/MsoListParagraph/.exec(element.attributes['class'])) {
                            var bulletText = element.firstChild(function(node) {
                                return node.type == constants.NODE_TEXT && !containsNothingButSpaces(node.parent);
                            });
                    ...
                    'div': function(element) {
                        ...
                        if (singleChild && singleChild.name == 'table') {
                            var attrs = element.attributes;
                            singleChild.attributes = tools.extend(singleChild.attributes, attrs);
                            attrs.style && singleChild.addStyle(attrs.style);

                            var clearFloatDiv = htmlParser.createElementByTag('div');
                    ...
                    'font': function(element) {
                        ...
                        if ((has("ie") || has("trident")) && notParagraphFont(element))
                            return false;
                        ...
                        {
                            tools.extend(parent.attributes,
                                element.attributes);
                    ...
                    'span': function(element) {
                        ...
                        if (styleText) {
                            ...
                            var fs = '';
                            styleText = styleText.replace(pat_fs, function(all, n1) {
                                if (n1 && n1 == ';') {
                                    fs = all.substring(1);
                                    return ';';
                                } else {
                    ...
                    'a': function(element) {
                        ...
                        if (attrs && !attrs.href && attrs.name)
                            delete element.name;
                        else if (has("webkit") && attrs.href && attrs.href.match(/file:\/\/\/[\S]+#/i))
                    ...
                    'style': stylesFilter(
                        ...
                        [
                            ...
                            [(/^width$/), null, function(value, element) {
                                if (value && value.length > 0 && element.name in {
                                        td: 1,
                                        th: 1
                                    })
                ...
                comment:
                    ...
                    function(value, node) {
                        ...
                        if (has("ff") && imageInfo) {
                            var img = fragment.fromHtml(imageInfo[0]).children[0],
