...
], function(domConstruct, domStyle, ViewTools, Container, SubContainer, tools, Model, FootNote, AbstractView, BlockView, viewUpdateTools) {

    var FootNoteView = function(model, ownerId, splited) {
        this.model = model;
        this.left = 0;
        this.top = 0;
        this.h = 0;
    ...
    FootNoteView.prototype = {
        ...
        split: function(w, h) {
            ...
            if (c == this.container) {
                ...
                var footNoteView = new FootNoteView(this.model, this.getOwnerId(), true);
        ...
        render: function() {
            if (!this.domNode) {
                ...
                viewUpdateTools.updateDOM(this.container, this.domNode);
...
//                    this._container.remove(firstView);
                      this.container._container.remove(firstView);
                    firstView = next;
                }
                while (firstView) {
        ...
        getPages: function() {
            ...
            var doc = ViewTools.getDocument(page);
            return doc.getPages();
        }
    };

    Model.prototype.viewConstructors[FootNote.prototype.modelType] = FootNoteView;
    tools.extend(FootNoteView.prototype, new AbstractView());
    tools.extend(FootNoteView.prototype, new BlockView());
    return FootNoteView;
});
