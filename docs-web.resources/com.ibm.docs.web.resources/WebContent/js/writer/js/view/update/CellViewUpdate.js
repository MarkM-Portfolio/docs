...
define([
...
], function(domAttr, domStyle, Container, Space, tools, ModelTools, SpaceBlock, BlockContainerView, BlockView, viewUpdateTools) {

    var CellViewUpdate = function() {};

    CellViewUpdate.prototype = {
        update: function() {
            ...
            if (this.domNode && this.domNode.childNodes)
                viewUpdateTools.updateDOM(c, this.domNode.childNodes[0]);
            var h1 = this.contentHeight;
            var isRepeatHead = ModelTools.isInDocTable(this.model) && (this.getParent().model.isTblHeaderRepeat() == true);
        ...
        updateCellDOM: function() {
            ...
            if ((this.heightChange() || this.widthChange()) && this.domNode) {
                ...
                domStyle.set(node, {
                    "height": Math.max(this.contentHeight, this._rowContentHeight) + "px",
        ...
        updateCellCSSStyle: function() {
            if (this._changeCSSStyle && this.domNode) {
                domAttr.set(this.domNode, {
                    "class": "cell " + this.getCSSStyle()
                });
                var contentNode = this.domNode.childNodes[0];
                domAttr.set(contentNode, {
        ...
        relayout: function() {
            ...
            that.bodySpace = new Space(that.contentWidth, 1080, 0, 0, that);
            ...
            this.container.forEach(function(c) {
                c.reset(true); // Defect 55822, nested table should reset immediately
                delete c.inserted;
                if (!c.hasLayouted()) {
                    c.layout(that);
                }
        ...
        toCheckBorder: function(toCheck, ignoreNotify) {
            if (toCheck) {
                this._toCheckBorder = true;
                this._borderChange = true;
                this.initEdgeWidth();
                if (this.getTable().isMatrixReady()) {
                    this.getTable().getTableMatrix().checkCellBorderChange(this);
                }
                if(!ignoreNotify)
    ...
    tools.extend(CellViewUpdate.prototype, new BlockView());
    tools.extend(CellViewUpdate.prototype, new BlockContainerView());
    tools.extend(CellViewUpdate.prototype, new SpaceBlock());
    return CellViewUpdate;
});
