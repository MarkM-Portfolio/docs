...
], function(declare, domConstruct, Container, tools, Model, trackChange, Row, TableTools, AbstractView, RowViewUpdate) {

    var RowView = declare("writer.view.table.RowView", null, {
    ...
    RowView.prototype = {
        ...
        render: function() {
            if (!this.domNode || this.isDirtyDOM()) {
                this.domNode = domConstruct.create("tr", {
        ...
        split: function(w, h) {
            ...
            var newRow = new RowView(this.model, this.getOwnerId(), true);
            newRow.w = this.w;
            ...
            for (var i = 0; i < rowMatrix.length; i++) {
                ...
                if (!newCell) {
                    ...
                    TableTools.insertCell(newCell, newRow);
                }
            }
            this.parent.insertRow(newRow, this);
            TableTools.splitHCells(rowMatrix, newRow);
        ...
        merge: function(row) {
            ...
            if (ret) {
                this.markDirtyDOM();
                if (row.cells.isEmpty() && row.model == this.model) {
                    row.deleted = true;
        ...
        splitMergedCellH: function(cell, newRow, w, h) {
            ...
            TableTools.insertCell(newCell, newRow);
            TableTools.splitMergedCell(cell, [newCell]);
        },
        ...
        getElementPath: function(x, y, path, options) {
            ...
            var cell = isTableDirRtl ? rowMatrix[rowMatrix.length - 1] : rowMatrix[0],
                prevCell = null;
        ...
        getChildPosition: function(idx) {
            ...
            if (!cell) {
                ...
                return {
                    'x': cell.getBoxWidth() + this.getLeft(),
                    'y': cell.getTop()
                };
            } else {
                return {
        ...
        getHtmlContent: function() {
        	var content = "<tr>";
            this.cells.forEach(function(cell) {
            	content += cell.getHtmlContent();
            });
            content += "</tr>";
            return content;
        }
    };
    Model.prototype.viewConstructors[Row.prototype.modelType] = RowView;
    tools.extend(RowView.prototype, new RowViewUpdate());
    tools.extend(RowView.prototype, new AbstractView());
    return RowView;
});
