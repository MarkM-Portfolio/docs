...
], function(domConstruct, domStyle, domClass, Container, Space, tools, constants, ViewTools, AbstractView, TextArea) {

    var Body = function (page, space, left, top, section, forColumns, spaceAfter) {
        ...
        this.textArea = new TextArea(this, space.clone(), {
            parentContainer: ViewTools.getDocument(this).getBlockViews(),
            name: "content",
            viewType: "text.content"
        });
        this.forColumns = forColumns;
        this.spaceAfter = spaceAfter;
        if (this.section && this.section.needSep())
            this.needSep = true;
    };
    Body.prototype = {
        left: 0,
         ...
         * @returns
         */
        next: function(isSamePage,isSameSection) {
            var page = this.page;
            var nextbody = page.getNextBody(this);
            if (nextbody) {
                if (isSameSection) {
                    if (nextbody.section == this.section)
                        return nextbody;
                    else
                        return null;
                } else
            ...
            if (isSameSection) {
                if (nextpage.getFirstBody().section == this.section)
        ...
        prevBody: function (isSamePage, isSameSection) {
            var prevbody = this.page.getPreviousBody(this);
            if (!prevbody) {
                if (isSamePage)
                    return null;
                else {
                    var prePage = this.page.previous();
                    if (prePage)
                        prevbody = prePage.getLastBody();
                    else
                        return null;
                }
            }
            ...
            if (isSameSection) {
                if (prevbody.section != this.section)
                    return null;
            }
            return prevbody;
        },
...
//            var container = this.textArea.getContainer();
            if (  !this.page.isFirstContent(view) && view.model.parent && view.model.parent.modelType == constants.MODELTYPE.DOCUMENT && view.model.isPageBreakBefore && view.model.isPageBreakBefore())
                return true;
        ...
        updateBodySpace: function(delT, delH, delW) {
            ...
            if (this.domNode)
            {
                domStyle.set(this.domNode, change);
                if (pe.scene.isNote())
        ...
        setDomZ: function(z_index) {
            if (this.domNode)
                domStyle.set(this.domNode, "zIndex", z_index);
        },
        _appendEndnoteDOM: function() {
            this.endnodeDOM = domConstruct.create("div");
            var endNodeSeparator = domConstruct.create("div", {
                "class": "endnote-separator",
                "style": "border-top:1px solid #333;width:" + Math.min(this.getWidth(), 200) + "px;"
            });
        ...
        _appendFootnoteDOM: function() {
            this.footnodeDOM = domConstruct.create("div");
            var footNodeSeparator = domConstruct.create("div", {
                "class": "footnote-separator",
                "style": "border-top:1px solid #333;width:" + Math.min(this.getWidth(), 200) + "px;"
            });
        ...
        buildSpaceNode: function()
        {
            if (this._spaceNode)
            {
                if (this._spaceNode.parentNode)
                    this._spaceNode.parentNode.removeChild(this._spaceNode);
            ...
            this._spaceNode = domConstruct.create("div", {
                    "class": "bodylineSpace",
                    "style": ("background:gray;position:absolute;left:" + (this.bodySpace.w + this.spaceAfter / 2 - 1) + "px;top:2px;width:" + 1 + "px;height:" + (this.bodySpace.h - 4)  + "px;z-index:-20001")
            });
            
            if (this.domNode)
            {
                this.domNode.appendChild(this._spaceNode);
            }
        },
        ...
        render: function() {
            if (!this.domNode) {
                ...
                if (this.forColumns){
                    domClass.add(pageNode, "forColumns");
                    domStyle.set(pageNode,{
                        overflow: "hidden"
                    });
                }
                if (this.needSep)
                    domClass.add(pageNode, "forColumnsNeedSep");
                ...
                if (this.forColumns && this.needSep && this.section && this.section.id)
                {
                    this.domNode.setAttribute("name", this.section.id);
                }
        ...
        append: function(contentView, footNoteView, _cache) {
            ...
            if (!referredFn || referredFn.length() === 0) {
                ...
                // handle the footnote
                if (ViewTools.isParagraph(contentView)) {
                    ...
                    while (h > 0 && devideAt) {
                        ...
                        if (!devideAt || h <= contentH + devideAt.getBoxHeight()) {
                            ...
                            var fnAtTheSameLine = new Container();
        ...
        _appendEndNotes: function(endnotes) {
            ...
            while (firstReferredNote) {
                if (!firstReferredNote.hasLayouted() || endnoteArea.getWidth() != firstReferredNote.w) {
        ...
        getFirstAppendView: function() {
            ...
            if (fnc && !fnc.isEmpty()) {
                var fns = new Container();
                var cnc = this.textArea.getContainer();
                var fn = fnc.getFirst();
                var refer = fn.getReferer();
                var referPara = refer && ViewTools.getParagraph(refer);
                while (referPara) {
                    while (referPara && !ViewTools.isTextContent(referPara.getParent())) {
                    ...
                    if (!cnc.inRange(referPara)) {
                        ...
                        referPara = refer && ViewTools.getParagraph(refer);
                    } else {
                        break;
                    }
                }
        ...
        getFootNoteArea: function() {
            this.footNodeArea = this.footNodeArea || new TextArea(this, new Space(this.bodySpace.w, 0, 0, 0, this), {
                name: "footnote",
                viewType: "text.footnotes"
            });
            return this.footNodeArea;
        },
        getEndNoteArea: function() {
            this.endNodeArea = this.endNodeArea || new TextArea(this, new Space(this.bodySpace.w, 0, 0, 0, this), {
                name: "endnote",
                viewType: "text.endnotes"
            });
            return this.endNodeArea;
        },
        getLeft: function() {
        ...
        releaseDom: function() {
            ...
            if(this.footnodeDOM){
                domConstruct.destroy(this.footnodeDOM);
            }
            this.endNodeArea && this.endNodeArea.releaseDom();
            if(this.endnodeDOM)
                domConstruct.destroy(this.endnodeDOM);
    ...
    tools.extend(Body.prototype, new AbstractView());
    return Body;
});
