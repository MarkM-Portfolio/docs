...
define([
    ...
    "writer/view/Run",
    "writer/constants",
    "writer/view/selection/AnchorImgResizerView"
], function(declare, domConstruct, domStyle, tools, ModelTools, ViewTools, ImageView, Run, constants, AnchorImgResizerView) {

    /**
     * Abstract view, used by squareImageView, TBImageView or FloatImageView
     */
    var AnchorImageView = declare("writer.view.AnchorView.AnchorImageView", ImageView, {
        ...
        _createResizer: function() {
            ...
            this._resizerView = new AnchorImgResizerView(this);
        },
        ...
        getResizerRect: function() {
        	...
        	rect.w = this.getWholeWidth();
        	rect.h = this.getWholeHeight();
        	return rect;
        },

        ifPointMe: function(x, y) {
            ret = x >= this.marginLeft && x <= this.marginLeft + this.getWholeWidth() && y >= this.marginTop && y <= this.marginTop + this.getWholeHeight()
        ...
        _getNeighbourRun: function() {
            return this._getNextNeighbourRun(false) || this._getPrevNeighbourRun(false) || this._getNextNeighbourRun(true) || this._getPrevNeighbourRun(true);
        },
        ...
        resetValuesForInvsible: function()
        {
            if (!this.isVisibleInTrack()) {
                this.w = this.h = 0;
        ...
        _calculateAbsolutePosH: function(line) {
            ...
            //shape position is mirrored for RTL anchor shape in ODT model
            var isRtlShape = line.parent && line.parent.directProperty && (line.parent.directProperty.getDirection(true) == "rl-tb");

            this.marginLeft = isRtlShape ? textContent.getWidth() - boxWidth - offX : offX;
            if (positionH.relativeFrom == "column" || positionH.relativeFrom == "margin") {
                ...
                if (isRtlShape) {
                    this.marginLeft = offX = pageWidth - boxWidth - offX;
                }
                if (body && body.left) {
                    this.marginLeft = offX - body.left;
                }
            } else if (isRtlShape && positionH.relativeFrom == "character") {
                this.marginLeft = textContent.getWidth() - offX;
            }
        ...
        _calculateAbsolutePosV: function(line) {
            ...
            var isInHeaderFooter = textContent.isHeader || textContent.isFooter;
            var inCell = vTools.isCell(textContent);
            var isInCellInHeaderFooter = false;
            var table = inCell ? textContent.getTable() : null;
            if (table)
            {
                isInCellInHeaderFooter = ModelTools.getParent(table.model, constants.MODELTYPE.HEADERFOOTER) != null;
            }
            ...
            var currentHeader = vTools.getHeader(p);
            var currentFooter = vTools.getFooter(p);
            
            if (isInCellInHeaderFooter)
                body = currentHeader || currentFooter;
            ...
            if (isSupported) {
                ...
                if (currentFooter && body && body.getTop() == 0)
                {
                    // in footer, but the body's top is 0, as the footer is not render completely.
                    // actually footer will re-layout the anchors, but anchor in table does not implement that.... so
                    virtualTopOffset = 0 - (pm.top + pageHeight);
                }
                
                virtualTopOffset -= ((body ? body.contentTop : 0) || 0);
        ...
        releaseBodySpace: function() {
            var body = ViewTools.getTextContent(this);
        ...
        _updateVDOM: function() {
            ...
            domStyle.set(this.domNode, {
                "top": (this.marginTop - this.padTop) + "px"
            });
        },
        ...
        _calMaskMarginV: function(line) {
            ...
            var textContent = vTools.getTextContent(line);
            var isInHeaderFooter = textContent.isHeader || textContent.isFooter;
            var isInHeader = textContent.isHeader;
            var isInFooter = textContent.isFooter;
            var body = !isInHeaderFooter ? vTools.getBody(textContent) : textContent;
            var page = vTools.getPage(line);
            var pageSize = page.section.pageSize;

            if (!vTools.isCell(textContent)) {
                var t = body.contentTop;
                var topToPage = this.marginTop + p.top + body.top;
                if (textContent.isFooter)
                    topToPage += body.contentTop;
        ...
        render: function() {
            ...
            if (!this.domNode || this.dirty) {
               ...
               if (this.isVisibleInTrack()) {
                    ...
                    var imgRelWidth = (borderStyle && this.borderWidth && this.borderWidth > 0)? this.iw - 2 * this.borderWidth : this.iw;
                    ...
                    var imgNode = domConstruct.create("img", {
                        ...
                        "class": "img_pic",
                        "src": srcPath,
                        "style": "position:absolute;left:" + (-this._maskLeft) + "px;bottom:" + (-this._maskBottom) + "px;width:" + imgRelWidth + "px;height:" + this.ih + "px;" + borderStyle
                    }, this.maskNode);
               ...
               {
                   // correct getRealPaddingLeft for invisible anchor.
