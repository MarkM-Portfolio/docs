...
], function(array, declare, constants, ModelTools) {

    var filter = declare("writer.plugins.copyfilter.filter", null, {
        styles: {},
        absNumers: {},
        imgs: {},
        /**
        ...
        filter: function(m, editor) {
            ...
            if (m.modelType == constants.MODELTYPE.TABLE) {
        ...
        filterJson: function(jsonData, editor, parentJson) {
            ...
            if (jsonData) {
                ...
                // BOB TRACK FIXME, Paragraph merge?
                var chs = jsonData.ch;
                if (chs && array.some(chs, function(ch){return ch.t == "del";}))
                	return null;
                ...
                delete jsonData.ch;
                delete jsonData.trPrCh;
                delete jsonData.rPrCh;
                delete jsonData.pPrCh;
                ...
                var children = ModelTools.getJsonChildren(jsonData);
                if (children) {
                    if (jsonData.fmt) {
                        //paragraph or hints 
                        var newChildren = [],
                        ...
                        for (var i = 0; i < children.length; i++) {
                            ...
                            if (child) {
                                ...
                                var text = jsonData.c;
                                jsonData.c = text.substring(0, length - start) + text.substring(length - start + l);
                            }
                        }
            ...
            // hints with no fmt should be delted (e.g. Hyperlink)
            if (jsonData.rt &&  jsonData.fmt && jsonData.fmt.length == 0) {
                return null;
            }
            return jsonData;
        },
        ...
        addNumber: function(numId) {
            if (!this.absNumers[numId]) {
                ...
                if (absNumber) {
                    ...
                    array.forEach(absNumber.numDefintions, function(lvl) {
