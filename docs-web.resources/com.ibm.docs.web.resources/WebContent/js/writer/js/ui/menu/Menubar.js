...
define([
    ...
    "writer/ui/widget/TableTemplatePane",
    "writer/ui/widget/TablePicker",
    "writer/ui/widget/ColorPalette",
    "writer/global"
], function(i18nmenubar, i18ntrack, domStyle, has, dojoString, domClass, i18nlang, MenuBarItem, Menu, MenuSeparator, CheckedMenuItem, MenuItem, PopupMenuItem, PopupMenuBarItem, MenuBar, BidiUtilsModule, dialogs, strings, uri, PageSetup, Menu, MenuTooltip, ListStyle, TableTemplatePane, TablePicker, ColorPalette, global) {

    global.createMenubar = function(node) {
        ...
        domClass.add(menubar.domNode, "lotusActionMenu");
        ...
        domClass.add(fileMenu.domNode, "lotusActionMenu");
        fileMenu.domNode.style.display = 'none';
        document.body.appendChild(fileMenu.domNode);

        var entitlements = pe.authenticatedUser && pe.authenticatedUser.getEntitlements();
        var enableAssignment = entitlements ? entitlements.assignment.booleanValue: false;
        var enableCoedit = entitlements ? entitlements.coedit.booleanValue : false;
        ...
        if (pe.scene.showNewMenu()) {
            ...
            /*
            	...
            	fromTemplateSubMenu.addChild( new MenuItem(
            	{
        ...
        if (pe.scene.showDiscardMenu()) {
            pe.discardMenuItem = new MenuItem({
                id: "D_i_Discard",
                label: nls.fileMenu_Discard,
                disabled: DOC_SCENE.hasTrack,
                onClick: function() {
                    dialogs.showDiscardDlg();
                },
                dir: dirAttr
            });
        ...
        if (DOC_SCENE.isPublishable && (isExternal || isECM)) {
            fileMenu.addChild(new MenuItem({
                id: "D_i_Save",
                label: ( window.gTPRepoName != "") ? dojo.string.substitute(nls.fileMenu_PublishVersion3,[window.gTPRepoName]): nls.fileMenu_PublishVersion2,
                onClick: function() {
                     pe.lotusEditor.execCommand('saveVersion');
                },
                dir: dirAttr
            }));
        }
        ...
        if (DOC_SCENE.isPublishable) {
            if (pe.scene.showSetAutoPublishMenu()) {
                var autopublishLabel = (isExternal ? (( window.gTPRepoName != "") ? dojo.string.substitute(nls.fileMenu_AutoPublish3,[window.gTPRepoName]): nls.fileMenu_AutoPublish2) : nls.fileMenu_AutoPublish);
                if(pe.scene.showCheckinMenu()) {
                  autopublishLabel = nls.fileMenu_AutoCheckIn;
                }
                pe.autoPublishMenuItem = new CheckedMenuItem({
                    id: "D_i_AutoPublish",
                    label: autopublishLabel,
                    checked: pe.scene.autoPublishSet(),
                    disabled: !pe.scene.autoPublishFeature(),
                    onClick: function() {
                        var checked = window["pe"].autoPublishMenuItem.get("checked");
            ...
            if (!uri.isExternalREST()) {
                var publishLable = pe.scene.showCheckinMenu() ? nls.fileMenu_CheckinVersion :nls.fileMenu_PublishVersion;
                pe.publishMenuItem = new MenuItem({
                    id: "D_i_Publish",
                    label: publishLable,
        ...
        if (bShowDownloadMenu !== false) {
            // Download ...
            var downloadSubMenu = new Menu({
                dir: dirAttr
            });
            domClass.add(downloadSubMenu.domNode, "lotusActionMenu");
            downloadSubMenu.domNode.style.display = 'none';
            document.body.appendChild(downloadSubMenu.domNode);

            downloadSubMenu.addChild(new MenuItem({
                id: "D_i_ExportToPDF",
                label: strings.getBidiRtlFormatStr(nls.fileMenu_ExportToPDF),
                onClick: function() {
            ...
            var labelExport = strings.getDefaultFileFormatStr(nls.fileMenu_ExportToMS, nls.fileMenu_ExportToODF);
            if (labelExport != "txt") {
                downloadSubMenu.addChild(new MenuItem({
            ...
            downloadSubMenu.addChild(new MenuItem({
                id: "D_i_ExportToMarkdown",
                label: strings.getBidiRtlFormatStr(nls.fileMenu_ExportToMarkdown),
                onClick: function() {
                	 pe.scene.exportToMarkdown();

                },
                dir: dirAttr
            }));            

            var fileDownload = new PopupMenuItem({
                id: "D_i_Download",
                label: nls.fileMenu_Download,
                popup: downloadSubMenu,
                dir: dirAttr
            });
            domStyle.set(fileDownload.arrowWrapper, 'visibility', '');
            fileMenu.addChild(fileDownload);
        }

        // Submit for Review ... always add this menu item and show/hide it in AbstractScene
        pe.reviewMenuItem = new MenuItem({
                id: 'P_i_SFR',
    	...
    	if (bShowDownloadMenu !== false) {
            ...
            fileMenu.addChild(new MenuItem({
                ...
                onClick: function() {
                    if (!pe.lotusEditor.pgSetDlg) {
                        var dlgNls = i18nlang;
                        pe.lotusEditor.pgSetDlg = new PageSetup(pe.lotusEditor, dlgNls.PAGE_SETUP);
                    }
           ...
           /* fileMenu.addChild(new MenuItem({
                ...
                onClick: function() {
                	pe.lotusEditor.execCommand('Print');//pe.scene.exportPdf(pe.lotusEditor);
                },
                dir: dirAttr
            }));*/
        	fileMenu.addChild( new MenuItem(
        ...
        domClass.add(editMenu.domNode, "lotusActionMenu");
        ...
        editMenu.addChild(new MenuSeparator());
        editMenu.addChild(new MenuItem({
            id: "D_i_Cut",
        ...
        domClass.add(viewMenu.domNode, "lotusActionMenu");
        viewMenu.domNode.style.display = 'none';
        document.body.appendChild(viewMenu.domNode);

        pe.toolbarMenuItem = new CheckedMenuItem({
            id: "D_i_Toolbar",
        ...
        /*pe.toolboxMenuItem = new CheckedMenuItem(
        		{
        			id: "D_i_Toolbox",
        ...
        //	pe.sidebarMenuItem = new CheckedMenuItem(
        //	{
        //		id: "D_i_Sidebar",
        ...
        if (enableAssignment) {
            pe.assignmentMenuItem = new CheckedMenuItem({
                id: "D_i_Assignment",
        ...
        pe.bookMarkMenuItem = new CheckedMenuItem({
            id: "D_i_ShowBookmark",
        ...
        pe.navigationMenuItem = new CheckedMenuItem({
            id: "D_i_ShowNaviPanel",
            label: nls.viewMenu_Navigation,
            checked: pe.scene.isShowNaviPanel && pe.scene.isShowNaviPanel(),
            onChange: function(isChecked) {
                pe.lotusEditor.execCommand('toggleNavigation', true);
            },
            dir: dirAttr
        });
        viewMenu.addChild(pe.navigationMenuItem);        

        var coEditChecked = pe.scene.isIndicatorAuthor();
        if (enableCoedit) {
            pe.coedtingMenuItem = new CheckedMenuItem({
                id: "D_i_Coediting",
                label: nls.viewMenu_Coediting,
                checked: coEditChecked,
                onChange: function(checked) {
        ...
        pe.showUnsupportMenuItem = new CheckedMenuItem({
        ...
        insertMenu.addChild(new MenuSeparator());
        insertMenu.addChild(new MenuItem({
            id: 'D_i_Image',
        ...
        pe.headerMenuItem = new MenuItem({
            id: "D_i_Header",
        ...
        pe.footerMenuItem = new MenuItem({
            id: "D_i_Footer",
        ...
        var pagenumber = new PopupMenuItem({
            id: 'D_i_PageNumber',
            label: nls.insertMenu_PageNumber,
            popup: pagenumberMenu,
            dir: dirAttr
        });
        domStyle.set(pagenumber.arrowWrapper, 'visibility', '');
        ...
        insertMenu.addChild(new MenuSeparator());
        var fieldsSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(fieldsSubMenu.domNode, "lotusActionMenu");
        fieldsSubMenu.domNode.style.display = 'none';
        document.body.appendChild(fieldsSubMenu.domNode);


        fieldsSubMenu.addChild(new MenuItem({
        ...
        var fields = new PopupMenuItem({
            id: 'D_i_Fields',
            label: nls.insertMenu_Fields,
            popup: fieldsSubMenu,
            dir: dirAttr
        });
        domStyle.set(fields.arrowWrapper, 'visibility', '');
        ...
        var formatMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(formatMenu.domNode, "lotusActionMenu");
        formatMenu.domNode.style.display = 'none';
        document.body.appendChild(formatMenu.domNode);

        //	formatMenu.addChild( new MenuItem(
        //	{
        //		id: 'D_i_Template',
        ...
        //			}
        //	}));
        //	formatMenu.addChild( new MenuSeparator() );
        var textSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(textSubMenu.domNode, "lotusActionMenu");
        textSubMenu.domNode.style.display = 'none';
        document.body.appendChild(textSubMenu.domNode);

        textSubMenu.addChild(new CheckedMenuItem({
            id: 'D_i_Bold',
        ...
        //		textSubMenu.addChild( new MenuSeparator() );
        //		
        //		
        //		textSubMenu.addChild( new MenuItem(
        //				{
        //					id: 'D_i_IncreaseSize',
        ...
        //			    	}
        //				}));
        var textProp = new PopupMenuItem({
            id: 'D_i_TextPro',
            label: nls.formatMenu_TextProperties,
            popup: textSubMenu,
            dir: dirAttr
        });
        domStyle.set(textProp.arrowWrapper, 'visibility', '');
        ...
        var paraSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(paraSubMenu.domNode, "lotusActionMenu");
        paraSubMenu.domNode.style.display = 'none';
        document.body.appendChild(paraSubMenu.domNode);

        paraSubMenu.addChild(new CheckedMenuItem({
        ...
        var paraProp = new PopupMenuItem({
            id: 'D_i_ParaProp',
            commandID: 'paraProp',
        ...
        domStyle.set(paraProp.arrowWrapper, 'visibility', '');
        formatMenu.addChild(paraProp);

        var imgPropItem;
        formatMenu.addChild(imgPropItem = new MenuItem({
            id: 'D_i_ImgPro',
        ...
        formatMenu.addChild(new MenuSeparator());
        //heading options
        var headingSubMenu = new Menu({
            commandID: 'headingFormat',
            dir: dirAttr
        });
        domClass.add(headingSubMenu.domNode, "lotusActionMenu");
        headingSubMenu.domNode.style.display = 'none';
        document.body.appendChild(headingSubMenu.domNode);

        headingSubMenu.addChild(new CheckedMenuItem({
            id: 'D_i_P',
            checked: false,
        ...
        var headingProp = new PopupMenuItem({
            commandID: 'headingFormat',
            id: 'D_i_Heading',
        ...
        domStyle.set(headingProp.arrowWrapper, 'visibility', '');
        ...
        */
        var muti = new ListStyle({
            type: "multilevelList"
        });
        var outline = new PopupMenuItem({
        ...
        var m_super = new CheckedMenuItem({
            id: 'D_i_Superscript',
        ...
        var m_subs = new CheckedMenuItem({
            id: 'D_i_subscript',
            checked: false,
            commandID: 'subscript',
        ...
        formatMenu.addChild(new MenuSeparator());
        var alignSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(alignSubMenu.domNode, "lotusActionMenu");
        alignSubMenu.domNode.style.display = 'none';
        document.body.appendChild(alignSubMenu.domNode);

        alignSubMenu.addChild(new CheckedMenuItem({
            id: 'D_i_AlignLeft',
        ...
        var formatAlign = new PopupMenuItem({
            id: 'D_i_Align',
            label: nls.formatMenu_Align,
            popup: alignSubMenu,
            dir: dirAttr
        });
        domStyle.set(formatAlign.arrowWrapper, 'visibility', '');
        formatMenu.addChild(formatAlign);
        if (window.BidiUtils.isBidiOn()) {
            formatMenu.addChild(new MenuSeparator());
            var directionSubMenu = new Menu({
                dir: dirAttr
            });
            domClass.add(alignSubMenu.domNode, "lotusActionMenu");
            ...
            directionSubMenu.addChild(new CheckedMenuItem({
                id: 'D_i_DirectionLtr',
            ...
            var formatDirection = new PopupMenuItem({
                id: 'D_i_Direction',
                label: nls.formatMenu_Direction,
                popup: directionSubMenu,
                dir: dirAttr
            });
            domStyle.set(formatDirection.arrowWrapper, 'visibility', '');
            formatMenu.addChild(formatDirection);
        }
        //////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////////////////////////////////////
        var lineSpacingSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(lineSpacingSubMenu.domNode, "lotusActionMenu");
        lineSpacingSubMenu.domNode.style.display = 'none';
        document.body.appendChild(lineSpacingSubMenu.domNode);

        lineSpacingSubMenu.addChild(new CheckedMenuItem({
            id: 'D_i_LineSpacing1',
        ...
        var formatLineSpacing = new PopupMenuItem({
            id: 'D_i_LineSpacing',
            label: nls.formatMenu_LineSpacing,
            popup: lineSpacingSubMenu,
            dir: dirAttr
        });
        domStyle.set(formatLineSpacing.arrowWrapper, 'visibility', '');
        ...
        formatMenu.addChild(new MenuItem({
            id: 'D_i_Columns',
            commandID: 'columns',
            label: nls.formatMenu_Columns,
            onClick: function() {
                pe.lotusEditor.execCommand('columns');
            },
            dir: dirAttr
        }));
        ...
        //	tableMenu.addChild( new MenuItem(
        //	{
        ...
        //	    	}
        //	}));
        //	tableMenu.addChild( new MenuSeparator() );
        tableMenu.addChild(new MenuItem({
        ...
        tableMenu.addChild(new MenuSeparator());
        var STRowSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(STRowSubMenu.domNode, "lotusActionMenu");
        STRowSubMenu.domNode.style.display = 'none';
        document.body.appendChild(STRowSubMenu.domNode);

        STRowSubMenu.addChild(new MenuItem({
        ...
        //		    	}
        //		}));
        STRowSubMenu.addChild(new MenuSeparator());
        STRowSubMenu.addChild(new MenuItem({
            commandID: 'deleteRow',
            id: 'D_i_DeleteRow',
        ...
        STRowSubMenu.addChild(new MenuSeparator());
        var STRowRepeat = new CheckedMenuItem({
        ...
        STRowSubMenu.addChild(STRowRepeat);
        STRowRepeat.domNode.style.display = "none";

        var STRow = new PopupMenuItem({
            commandID: 'row',
            id: 'D_i_Row',
        ...
        domStyle.set(STRow.arrowWrapper, 'visibility', '');
        tableMenu.addChild(STRow);

        var STColumnSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(STColumnSubMenu.domNode, "lotusActionMenu");
        STColumnSubMenu.domNode.style.display = 'none';
        document.body.appendChild(STColumnSubMenu.domNode);

        STColumnSubMenu.addChild(new MenuItem({
        ...
        //		}));
        //		
        STColumnSubMenu.addChild(new MenuSeparator());
        ...
        /*
			...
			*/
        var STColumn = new PopupMenuItem({
            commandID: 'column',
            id: 'D_i_Column',
        ...
        domStyle.set(STColumn.arrowWrapper, 'visibility', '');
        tableMenu.addChild(STColumn);
        var STCellSubMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(STCellSubMenu.domNode, "lotusActionMenu");
        STCellSubMenu.domNode.style.display = 'none';
        document.body.appendChild(STCellSubMenu.domNode);


        var palette = new ColorPalette({
            id: "D_m_BackgroundColor",
            colorType: "BackColor",
            palette: "7x10",
            onOpen: function(val) {
                if (pe.lotusEditor) {
        ...
        STCellSubMenu.addChild(new PopupMenuItem({
        ...
        STCellSubMenu.addChild(new MenuSeparator());
        STCellSubMenu.addChild(new MenuItem({
            id: 'D_i_CellMC',
        ...
        domStyle.set(STCell.arrowWrapper, 'visibility', '');
        ...
        var teamMenu = new Menu({
            dir: dirAttr
        });
        domClass.add(teamMenu.domNode, "lotusActionMenu");
	    ...
	    if(window.g_docTrackEnabled && (pe.authenticatedUser.getId() == window.DOC_SCENE.ownerId)){
	        pe.trackChangesMenuItem = new CheckedMenuItem({
	          id: 'D_i_TrackChange',
	        label: i18ntrack.menu_track_changes,
	        checked: false,
	        disabled: true,
	        onClick: function() {
	        	var checked = window["pe"].trackChangesMenuItem.get("checked");
	        		pe.scene.switchTrackChanges(checked);
	        	},
	        	dir: dirAttr
	        });
	
	        teamMenu.addChild(new MenuSeparator()); 
	        teamMenu.addChild(pe.trackChangesMenuItem);
	    }
        ...
        if (enableAssignment) {
            ...
            var teamNls = i18nAbstractTaskHandler;
            teamMenu.addChild(new MenuItem({
        ...
        domClass.add(toolsMenu.domNode, "lotusActionMenu");
		...
		*/
        ...
        //		taskToolsSubMenu.addChild( new MenuItem(
        //		{
        ...
        //	pe.spellcheckMenu = new MenuItem(
        //	{
        ...
        var scLangSubMenu = new Menu({
            dir: dirAttr,
            baseClass: "concordMenuStyle"
        });
        domClass.add(scLangSubMenu.domNode, "lotusActionMenu");
        ...
        if (spellcheckerManager) {
            for (var i = 0; i < spellcheckerManager.supportedLang.length; i++) {
                ...
                pe.dicMenus[dic] = new CheckedMenuItem({
                    id: id,
                    label: acc + label,
                    title: acc + label,
                    checked: false,
        ...
        var scLangDics = new PopupMenuItem({
            id: 'D_i_SelectDic',
            label: nls.toolsMenu_SelectDictionary,
            popup: scLangSubMenu,
            dir: dirAttr
        });
        domStyle.set(scLangDics.arrowWrapper, 'visibility', '');
        toolsMenu.addChild(scLangDics);

        pe.autoSpellCheckMenu = new CheckedMenuItem({
        ...
        domClass.add(helpMenu.domNode, "lotusActionMenu");
		...
		*/
        ...
        newFatureM.setDisabled(!g_hasNewFeature);
        ...
        if (pe.settings) {
            ...
            if (pe.settings.getToolbar() == pe.settings.TOOLBAR_NONE) {
                pe.toolbarMenuItem.set("checked", false);
            }

            //init assignment
            if (enableAssignment) {
                if (pe.settings.getAssignment() == false) {
                    pe.assignmentMenuItem.set("checked", false);
                }
            }
            if (enableCoedit) {
                //init co-editing
                if (pe.settings.getIndicator() == false) {
                    pe.coedtingMenuItem && pe.coedtingMenuItem.set("checked", false);
                }
            }
            ...
            for (var dic_locale in pe.dicMenus) {
                if (spellcheckerManager && spellcheckerManager.lang == dic_locale)
                    pe.dicMenus[dic_locale].set("checked", true);
                else
                    pe.dicMenus[dic_locale].set("checked", false);
            }
