...
define([
    ...
    "dijit/registry",
    "dojox/uuid"
], function(lang, dom, domGeometry, has, on, topic, events, RangeIterator, constants, Range, msgCenter, ModelTools, ViewTools, registry, uuid) {
     ...
     */
    var CommentService = lang.mixin(lang.getObject("writer.model.comments.CommentService", true), {
        instantiated: null,
        init: function() {
            var clz = {
                ...
                constructor: function() {
                    ...
                    topic.subscribe(constants.EVENT.PAGESCROLLED, lang.hitch(this, this.selectionChangeHandler));
                    topic.subscribe(constants.EVENT.DOMCREATED, lang.hitch(this, this.nodeDomCreated));
                    topic.subscribe(constants.EVENT.EDITAREACHANGED, lang.hitch(this, this.editModeChangeHandler));
                    ...
                    topic.subscribe(constants.EVENT.FIRSTTIME_RENDERED, lang.hitch(this, openCommentBar));

                    events.subscribe(events.commenting_popupDlg_click, this, '_handlePopupCommentsBoxClick');
                    events.subscribe(events.commenting_popupDlg_mouseOut, this, '_handlePopupCommentsBoxMouseOut');
                    events.subscribe(events.commenting_popupDlg_mouseOver, this, '_handlePopupCommentsBoxMouseOver');
                    events.subscribe(events.commenting_addCommentPopupDlg, this, '_handleaddCommentPopup');
                },
                ...
                selectionChangeHandler: function() {
                    ...
                    setTimeout(lang.hitch(this, function() {
                        ...
                        var comment_btn = registry.byId("concord_comment_btn");
                        var tCommentsCmd = pe.lotusEditor.getCommand('toggleCommentsCmd');
                        if (bAdd) {
                        	tCommentsCmd && tCommentsCmd.enable();
                            if (comment_btn) comment_btn.setAttribute('disabled', false);
                        } else {
                        	tCommentsCmd && tCommentsCmd.disable();
                        ...
                        if (lastcmtsview && lastcmtsview.obj && lastcmteview && lastcmteview.obj) {
                            var startview = ranges[0].getStartView(),
                                endview = ranges[0].getEndView();
                        ...
                        if (this.select_comment_id.length == 0) {
                            ...
                            this.laststate.scrbasept = null;
                            this.laststate.enablechksel = false;
                            this.laststate.sticky = false;
                            events.publish(events.comments_deselected, []);
                            return;
                        }
                ...
                editModeChangeHandler: function() {
                    if (pe.lotusEditor.isContentEditing()) {
                        topic.publish(concord.util.events.disable_commentsPanel, false);
                    } else {
                        topic.publish(concord.util.events.disable_commentsPanel, true);
                    }
                },
                ...
                nodeDomCreated: function(model, domNode, view, args) {
                    ...
                    if (!ModelTools.hasComments(model) || !domNode) return;
                    ...
                    var _onHoverInterval = function(model, domNode, view) {
                        ...
                        if (!domNode || !model || !ModelTools.hasComments(model)) return;
                    ...
                    var compare = function() {
                        ...
                        if (Math.sqrt((pX - cX) * (pX - cX) + (pY - cY) * (pY - cY)) < cfg.sensity &&
                            ...
                            hoverIntent_s = true;
                            m_moveevent && m_moveevent.remove();
                            m_moveevent = null;
                            ...
                            cmtsrv.hoverIntent_t = setTimeout(function() {
                                compare();
                            }, cfg.interval);
                        }
                    };
                    ...
                    var _onMouseOver = function(ev) {
                        ...
                        m_moveevent = on(domNode, "mousemove", _onMouseMove);
                        if (!hoverIntent_s) {
                            cmtsrv.hoverIntent_t = setTimeout(function() {
                                compare();
                            }, cfg.interval);
                    ...
                    var _onMouseOut = function() {
                        ...
                        if (cmtsrv.hoverinterval)
                            cmtsrv.hoverinterval = clearInterval(cmtsrv.hoverinterval);
                        if (m_moveevent) m_moveevent.remove();
                    };

                    on(domNode, "mouseenter", _onMouseOver);
                    on(domNode, "mouseleave", _onMouseOut);
                    //dojo.connect(domNode, 'onmousemove', null, _onMouseMove);
                    on(domNode, "mousedown", _onMouseOut); // same behavior as out
                },
                ...
                findSameCommentID: function(run_pre, run_next) {
                    ...
                    if (!run_pre || run_pre === run_next) // try to find prev run
                        run_pre = ModelTools.getPrev(run_next, ModelTools.isRun);
                    if (!run_next)
                        run_next = ModelTools.getNext(run_pre, ModelTools.isRun);
                ...
                handleCommentByView: function(runview) {
                    if (runview.model && runview.model.modelType === constants.MODELTYPE.IMAGE)
                        this._trackImageCommentRange(runview);
                    else if (runview.model && runview.model.modelType === constants.MODELTYPE.TEXT)
                ...
                _trackImageCommentRange: function(runview) {
                    ...
                    var next_run = ModelTools.getNext(runview.model, ModelTools.isRun);
                    var pre_run = ModelTools.getPrev(runview.model, ModelTools.isRun);
                    while (next_run != null) {
                        if (next_run.length == 0) {
                            var nrun = ModelTools.getNext(next_run, ModelTools.isRun);
                    ...
                    while (pre_run != null) {
                        if (pre_run.length == 0) {
                            var prun = ModelTools.getPrev(pre_run, ModelTools.isRun);
                ...
                _trackTextCommentRange: function(runview) {
                    ...
                    if (next = iterator.next()) {
                        ...
                        {
                            next_run = ModelTools.getNext(next, ModelTools.isRun);
                            pre_run = ModelTools.getPrev(next, ModelTools.isRun);
                            while (next_run != null) {
                                if (next_run.length == 0) {
                                    var nrun = ModelTools.getNext(next_run, ModelTools.isRun);
                                    next_run = (nrun === next_run ? null : nrun);
                            ...
                            if (pre_run.clist && pre_run.clist.length > 0) {
                                var tbl = ModelTools.getTable(runview.model);
                                var nexttbl = ModelTools.getTable(next_run);
                                if (tbl && (tbl != nexttbl || !next_run)) {
                                    var cell = ModelTools.getCell(runview.model);
                                    var prevcell = ModelTools.getCell(pre_run);
                ...
                isCommentContinued: function(obj, index) {
                    ...
                    var tbl = ModelTools.getTable(obj);
                    var next_run = ModelTools.getNext(obj, ModelTools.isRun);
                    if (next_run == null) {
                        if (tbl) return true;
                        else return false;
                    } else {
                        var ntbl = ModelTools.getTable(next_run);
                ...
                trySetCommentOnInsertHint: function(hint, pre_run, next_run) {
                    ...
                    if (hint.modelType === constants.MODELTYPE.IMAGE || hint.modelType === constants.MODELTYPE.TEXT) {
                ...
                getCommentListOnPara: function(para, idx, len) {
                    ...
                    while (run) {
                        ...
                        if (run.length > 0 && (run.start + run.length) >= end) break;
                    ...
                    if (run.clist && run.clist.length > 0) {
                        ...
                        if (commentsIds.length >= 0) {
                            ...
                            if (commentsIds.length == 1) {
                                events.publish(events.comments_selected, [commentsIds[0], true]);
                            } else {
                                events.publish(events.comments_selected, [commentsIds, true]);
                            }
                ...
                insertCmtTextRun: function(textrun) {
                    ...
                    for (var n = 0; n < textrun.clist.length; n++) {
                    	var cid = textrun.clist[n]; 
                        var comment = this.comments[cid];
                        if (comment) {
                    ...
                    for (var n = 0; n < cCmt.length; n++) {
                    	var cid = cCmt[n]; 
                        var comment = this.comments_children[cid];
                        if(!comment)
                        	continue;
                        var pid = (comment.pid ? "'" + comment.pid + "'" : "''");
                        if (pCmt.indexOf(pid) < 0) {
                            comment.runs = comment.runs || new Array();
                            comment.runs.push(textrun);
                        }
                    }
                ...
                selectComment: function(comment_id, bscroll, noFocus) {
                    ...
                    if (comment_id == null) {
                        ...
                        if (selranges.length > 0) {
                            if (selranges[0].isCollapsed())
                                ...
                                selection.scrollIntoView(false, false, new Range(sview, sview));
                            }
                        }
                    ...
                    var cmtid = null;
                    if (lang.isArray(comment_id)) { // temp code
                        isidarray = true;
                        cmtid = comment_id[0];
                        var bsame = true;
                    ...
                    for (var it = this.select_comment_id.length - 1; it >= 0; it--) {
                        ...
                        events.publish(events.comments_deselected, [cmtid_]);
                    };
                ...
                unselectComment: function(comment_id, noUpdate) {
                    ...
                    events.publish(events.comments_deselected, [comment_id]);
                ...
                insertComment: function(commentId, commentContent) {
                    ...
                    var eventData = [{
                        eventName: events.commenting_undoDelete,
                        widgetId: commentId
                    }];
                    events.publish(events.commentingEvents, eventData);
                },
                deleteComment: function(comment_id, coedit) {
                    ...
                    if (!pe.scene.localEdit || !coedit) {
                        ...
                        for (var n = 0; n < rcids.length; n++) {
                            ...
                            for (var i = 0; i < del_msg_list.length; i++) {
                                ...
                                tx_msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.delCommentAct(del_msg.pid, target_cmt.id, del_msg.start, del_msg.end)], constants.MSGCATEGORY.Content);
                                msgs.push(tx_msg.msg);
                            }
                            //2. update the relation json
                            msg = msgCenter.createMsg(constants.MSGTYPE.KeyMessage, [msgCenter.createDeleteArrayAct(target_cmt.id, target_cmt, "comments")], constants.MSGCATEGORY.Relation);
                            msgs.push(msg.msg);
                        }
                    ...
                    if (coedit) {
                        var deleventData = [{
                            eventName: events.commenting_delete,
                            widgetId: comment_id
                        }];
                        events.publish(events.commentingEvents, deleventData);
                    }
                    pe.lotusEditor.focus();
                },
                ...
                appendComment: function(pid, item) {
                    ...
                    json_comment.id = uuid.generateRandomUuid();
                    ...
                    this.comments_children.push(lang.clone(json_comment));

                    msg = msgCenter.createMsg(constants.MSGTYPE.KeyMessage, [msgCenter.createInsertArrayAct(json_comment.id, json_comment, "comments")], constants.MSGCATEGORY.Relation);
                    msgs.push(msg);
                    ...
                    for (var i = 0; i < runs.length; i++) {
                        ...
                        if (therun.clist)
                        {
                            for (var it = 0; it < therun.clist.length; it++) {
                                if (therun.clist[it] == pid) {
                                    therun.clist.splice(it + 1, 0, json_comment.id);
                                    break;
                                }
                            }
                        }
                        ...
                        var append_msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.addCommentAct(para.id, json_comment.id, 0, 0, pid, rcmtid)], constants.MSGCATEGORY.Content);
                ...
                addComment2Paragraph: function(para, c_id, start, end, msgs, runid) {
                    if (!this.checkParaType(para))
                        return;
                    //		if(para.parent.modelType == writer.constants.MODELTYPE.CELL) {
                    //			start = 0;
                    //			end = para.getLength();
                    //		}
                    ...
                    for (var i = 0; i < runs.length; i++) {
                        ...
                        var runid;
                        if (ModelTools.isImage(runs[i]))
                            runid = runs[i].id;
                        var tx_msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.addCommentAct(para.id, c_id, start, end, null, null, runid)], constants.MSGCATEGORY.Content);
                        msgs.push(tx_msg);
                    }
                },
                addComment2ParaModel: function(para, c_id, start, end, runid) {
                    ...
                    runs.forEach(function(run) {
                        //		if(run.modelType===writer.constants.MODELTYPE.IMAGE)
                        //			return;
                ...
                addComment2Run: function(run, clist) {
                    ...
                    for (var i = 0; i < run.clist.length; i++) {
                        ...
                        var tx_msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.addCommentAct(para.id, c_id, start, end)], constants.MSGCATEGORY.Content);
                        msgs.push(tx_msg);
                    }
                    //	writer.msg.msgCenter.sendMessage( msgs );
                },
                checkParaType: function(para) {
                    //if(para.getLength()==0)
                    //	return false;
                    if (ModelTools.getParent(para, constants.MODELTYPE.TOC))
                        return false;
                    if (ModelTools.isTextBox(para.parent))
                        return false;
                ...
                checkAddComment: function(noselection, async) {
                    ...
                    if (bAdd) {
                        for (var i = 0; i < ranges.length; i++) {
                            ...
                            var it = new RangeIterator(ranges[i], maxParagraphCount);
                            var para = null;
                ...
                addComment: function(comments) {
                    ...
                    this.comments[json_comment.id] = (lang.clone(json_comment));
                    msg = msgCenter.createMsg(constants.MSGTYPE.KeyMessage, [msgCenter.createInsertArrayAct(comments.id, json_comment, "comments")], constants.MSGCATEGORY.Relation);
                    msgs.push(msg);
                    ...
                    for (var i = 0; i < ranges.length; i++) {
                        ...
                        if (paras.length == 0)
                            continue;
                        para = paras[0];
                        var start = range.getStartParaPos().index,
                            end = range.getEndParaPos().index;
                        if (paras.length == 1) {
                ...
                updateComment: function(cmtid, index, item) {
                    ...
                    var msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.createUpdateCommentAct(cmtid, index, updateitem)],
                            constants.MSGCATEGORY.Relation),
                        msgs = [];

                    msgs.push(msg.msg);

                    pe.scene.session && pe.scene.session.sendMessage(msgs);
                ...
                removeCommentRef: function(textrun) {
                    ...
                    for (var n = 0; n < textrun.clist.length; n++) {
                        ...
                        if (comment) {
                        	var j = 0, len = comment.runs.length;
                                for (var i = 0; i < len; i++) {
                                    var run = comment.runs[j];
                                    if (textrun == run ) {
                                        //remove the run from comment ref
                                    	comment.runs.splice(j, 1);
                                    } else
                                    	j++;
                                }
                        }
                        if (comment) {
                        	if(comment.runs.length <= 0) {
                            	...
                            	nClist.push(comment.id);
                            }
                        }
                    }
                    textrun.clist = nClist;
                    return nClist;
                },
                checkCommentsDelete: function() {
                    ...
                    for (var id in this.comments) {
                        ...
                        for (var i = comment.runs.length - 1; i >= 0; i--) {
                            if (comment.runs[i].deleted || comment.runs[i].isTrackDeleted() || comment.runs[i].paragraph.deleted
                                || ModelTools.isInTrackDeletedTable(comment.runs[i]))
                ...
                genDelCommentMsg: function(cmt_id) {
                    ...
                    var j = 0,len = this.comments_children.length;
                    for (var i = 0; i < len; i++) {
                    	var cm = this.comments_children[j];
                        if (cm.pid == cmt_id || cm.id == cmt_id) {
                            cmts.push(cm);
                            this.comments_children.splice(j, 1);
                        } else
                        	j++;
                    }
                    ...
                    for (var idx = cmts.length - 1; idx >= 0; idx--) {
                        ...
                        for (var i = 0; i < del_msg_list.length; i++) {
                            var del_msg = del_msg_list[i];
                            tx_msg = msgCenter.createMsg(constants.MSGTYPE.TextComment, [msgCenter.delCommentAct(del_msg.pid, comment_id, del_msg.start, del_msg.end)], constants.MSGCATEGORY.Content);
                            msgs.push(tx_msg);
                        }
                        ...
                        if (comment.date == "1999-06-07T00:00:00.000Z")
                            delete comment.date;
                        //2. update the relation json
                        msg = msgCenter.createMsg(constants.MSGTYPE.KeyMessage, [msgCenter.createDeleteArrayAct(comment_id, comment, "comments")], constants.MSGCATEGORY.Relation);
                        msgs.push(msg);
                ...
                scroll2Comment: function(comment_id) {
                    ...
                    if (comment && comment.runs) {
                        if (comment.runs.length > 0) {
                            ...
                            var end = {
                                "obj": comment.runs[comment.runs.length - 1],
                                "index": 0
                            };
                            var range = null;
                            if (!ModelTools.isImage(start.obj)) {
                ...
                _getPosition: function(pos) {
                    ...
                    var view_height = editorFrame.offsetHeight;
                    if (concord.util.browser.isIOSBrowser()) {
                        // on ios browser editorFrame height is the real height, chang to use mainNode.
                        var mainNode = dom.byId("mainNode");
                        var height = mainNode.style.height;
                        view_height = height.substring(0, height.length -2) -2;
                    }
                    ...
                    pos.y += headerheight;
                    if (concord.util.browser.isIOSBrowser()) {
                    // on ios browser the pos.y is the real height of element not screen pos, need to remove the scrollTop
                        var mobileIframeDiv = dojo.byId("mobileIframeDiv");
                        if (mobileIframeDiv)
                            pos.y -= mobileIframeDiv.scrollTop;
                    }
                ...
                _handlePopupCommentsBoxMouseOut: function() {
                    if (this.select_comment_id.length == 0) {
                        ...
                        var mvout_handler = function() {
                            ...
                            cmtsvr.laststate.len = 0;
                            cmtsvr.laststate.startview = null;
                            cmtsvr.laststate.endview = null;
                            cmtsvr.laststate.scrbasept = null;
                        };
