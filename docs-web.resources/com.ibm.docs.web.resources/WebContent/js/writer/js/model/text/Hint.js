...
define([
...
], function(topic, lang, array, Container, constants, ModelTools, trackChange, msgCenter) {
    ...
    Hint.prototype = {
        ...
        removeTextLength: function(index, len, container) {
            ...
            if (this.length == 0 && this.modelType == constants.MODELTYPE.TEXT) {
                ...
                if (!prev || 
                    ((!this.rParagraph || this.rParagraph == prev.rParagraph) && 
                        (!prev.br || prev.br.type == "page"))) {
                    if (!inTrack) {
                ...
                if (delta > 0) {
                    ...
                    if (delta >= this.length) {
                        ...
                        if (!inTrack) {
                            ...
                            var right = this.split(index + len);
                            return this.markDeleteChange();
                        }
                    }
                } else {
                    // -----(---)-start-------------------end-------
                    // select NOT pass my head, but i need to move my head ahead, because something in my header would be cut
                    if (!inTrack) {
                        this.start -= len;
                        this.markDirty();
                    }
                ...
                if (this.start + this.length > index + len) {
                    // ---------start----(--------------)---end--------
                    if (!inTrack) {
                        // only change my length, start remain unchanged.
                        this.length -= len;
                        this.markDirty();
                    } else {
                        this.markDirty();
                    ...
                    // ---------start------(-----------end---)-----
                    // after my head, but passed my tail, leave some text in the header.
                    if (!inTrack) {
        ...
        insertHint: function(index, hint) {
            if (this.canMerge(hint)) {
                this.merge(hint);
                if (trackChange.sum && hint.modelType == "run.text" && this.ch)
                {
                    // because of merge, the hint's track id got lost.
                    array.forEach(this.ch, function(ch){
                       if (ch.id)
                       {
                           delete trackChange.sum.seen[ch.id];
                       }
                    });
                }
                this.markDirty();
        ...
        insertText: function(text, index) {
            ...
            while (parent && parent.hints) {
                ...
                if (parent.modelType == constants.MODELTYPE.PARAGRAPH) {
                    var para = parent;
                    para._insertText(text, index);
                    para.markDirty();
                    topic.publish(constants.EVENT.REQUESTSPELLCHECKONPARA, para, text, index);
                    break;
                } else {
        ...
        markDeleteChange: function() {
            if (!this.ch)
                this.ch = [];

            if (this.isTrackDeleted()) {
                // already deleted;
                console.warn("already deleted");
                return null;
            }
            ...
            var msgData = {
                start: this.start,
                length: this.length,
                oldChanges: lang.clone(this.ch)
            }

            if (this.isTrackInsertedByMe()) {
                // console.warn("delete my insertion");
                msgData.deleted = true;
                msgData.obj = this;
                delete msgData.oldChanges;
                return msgData;
            }

            var tc = trackChange.createChange("del");
            this.ch.push(tc);
            msgData.newChanges = this.ch;
            return msgData;
        },
        ...
        deleteInTrack: function() {
            ...
            if (this.container) {
                this.container.forEach(function(child) {
                    if (child && child.deleteInTrack) {
                        var _msgs = child.deleteInTrack();
                        if (_msgs) {
                ...
                if (!this.isTrackDeleted()) {
                    if (this.isTrackInsertedByMe()) {
                        // TODO, seems no need, the parent level handled.
                    } else {
                        this.ch.push(trackChange.createChange("del"));
                        msgs.push(msgCenter.createMsg(constants.MSGTYPE.Text, [msgCenter.createSetTextAttribute(this.start, this.length, this.paragraph, null, null, {
                            "ch": this.ch
                        }, {
                            "ch": oldChanges
                        })]));
