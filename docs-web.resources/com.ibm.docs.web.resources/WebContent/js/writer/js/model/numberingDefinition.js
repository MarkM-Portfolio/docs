...
], function(constants, ParagraphProperty, Property, TextProperty, FontTools, msgCenter) {

    var numberingDefinition = function(source) {
        this.init(source);
    };

    numberingDefinition.prototype = {
        init: function(source) {
            this.isNumDef = true;
            this.start = source.start && source.start.val;
            this.paraProperty = source.pPr && new ParagraphProperty(source.pPr, null, true);
            ...
            var txtUnType = (this.lvlText ? FontTools.getFontTypeByCode(this.lvlText) : "ascii" );
            this.textProperty = source.rPr && new TextProperty(source.rPr, txtUnType && {"txtUnType": txtUnType, "owner": this});
        ...
        changeListType: function(newDefinition, numId, level) {
            ...
            if (newDefinition.rPr && newDefinition.rPr instanceof Object) {
                ...
                var txtUnType = (newDefinition.lvlText ? FontTools.getFontTypeByCode(newDefinition.lvlText) : "ascii" );
                this.textProperty = new TextProperty(newDefinition.rPr, txtUnType && {"txtUnType": txtUnType, "owner": this});
            } else
            ...
            var msg = msgCenter.createMsg(constants.MSGTYPE.List, [msgCenter.createSetListAttributeAct(numId, newJson, oldJson, constants.ACTTYPE.ChangeType)], constants.MSGCATEGORY.List);
