...
define([
...
], function(tools, constants, listPrototype, msgCenter, ModelTools) {
    ...
    list.prototype = {
        modelType: constants.MODELTYPE.LIST,
        init: function() {
        ...
        addPara: function(para) {
            if (para.isInTCGroup && para.isInTCGroup())
                return;
            if (this.paras.length == 0)
                this.paras.push(para);
            else {
                // TODO Special case for list in text box
                var curPath = ModelTools.getParagraphPath(para);
                ...
                for (var i = this.paras.length - 1; i >= 0; i--) {
                    ...
                    var existPath = ModelTools.getParagraphPath(this.paras[i]);
                    var compareResult = tools.arrayCompare(curPath, existPath);
                    if (compareResult > 0) {
        ...
        getFirstParagraphInSameLevel: function(para) {
            if (!this.paras)
                return null;
            var parasLen = this.paras.length;
            if (!parasLen)
            	return null;
            var foundCnt = -1;
            var refIdx = this.paras.indexOf(para);
            var listLevel = para.getListLevel();
            if (refIdx == -1)
                return null;
            var foundPara;
            for (var i = refIdx; i >= 0; i--) {
                var currentLevel = this.paras[i].getListLevel();
                if (currentLevel == listLevel)
                    foundPara = this.paras[i];
        ...
        getParagraphsInSameOrLargerLevel: function(startPara) {
        	if (!this.paras || !this.paras.length)
        	    return [];
            var startIdx = this.paras.indexOf(startPara);
            if (startIdx == -1)
                return [];
            var foundParas = [];
            var startListLevel = startPara.getListLevel();
            var parasLen = this.paras.length;
            for (var i = startIdx; i < parasLen; i++) {
                if (this.paras[i].getListLevel() >= startListLevel)
                    foundParas.push(this.paras[i]);
                else
                    break;
            }
            return foundParas;
            ...
            if (!this.listSymbolProperties[level]) {
                ...
                if (lvlInfo) {
                    ...
                    if (styleTextProp) {
                    	...
                    	var txtStyle = textProp && textProp.style;
                    	if( txtStyle.style["rFonts"]) {
                    		if(!txtStyle.style["font-family"])
                    			txtStyle.style["font-family"] = txtStyle.style["rFonts"]["ascii"];
                    		delete txtStyle.style["rFonts"];
                    	}
                    }
        ...
        _getSymbolText: function(lvlText, lvl, listValues) {
            var type = 1, startNum = 1; // Default type
            ...
            var symbol = type ? list.prototype.getNextValue(type,
             startNum, (listValues[lvl] - 1)) : "";
        ...
        getNextValue: function(type, currentValue, offset) {
            ...
            if (base.getHindiValue) {
                ret = base.getHindiValue(1, offset);
            } else if (offset == 0) {
        ...
        setNumberingStart: function(startVal, level) {
            ...
            var msg = msgCenter.createMsg(constants.MSGTYPE.List, [msgCenter.createSetListAttributeAct(this.id, newJson, oldJson, constants.ACTTYPE.ChangeStart)], constants.MSGCATEGORY.List);
    ...
    tools.extend(list.prototype, new listPrototype());
    return list;
});
