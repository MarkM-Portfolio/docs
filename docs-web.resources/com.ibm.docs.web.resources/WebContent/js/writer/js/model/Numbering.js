...
], function(constants, Image, Run, abstractNum) {

    var Numbering = function(source) {
        this.init(source);
    };
    ...
    Numbering.prototype = {
        init: function(source) {
            ...
            for (var data in source) {
                if (data.indexOf('abstractNum') != -1)
                    this.abstracts[data.substr(11)] = new abstractNum(source[data]);
                else if (data.indexOf('numPicBullet') != -1)
                    this.imgs[data.substr(12)] = new Image(source[data], this);
        ...
        getImg: function(imageId, withOwner) {
            ...
            if (withOwner) {
                ...
                retImg = new Image(jsonData, withOwner);
            }
        ...
        addImg: function(imageId, img) {
            if (img.modelType == constants.MODELTYPE.IMAGE)
                this.imgs[imageId] = img;
            else if (img.rt == constants.RUNMODEL.IMAGE || img.t == 'numPicBullet')
                this.imgs[imageId] = new Image(img, this);
            else
