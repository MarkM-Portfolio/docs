...
function release(){
	...
	for(i = 0; i < result.length; i++){
		...
		if(kwArgs.internStrings){
			...
			if(kwArgs.layerOptimize.indexOf("uglifyjs") == -1){
		...
		var compressedResult = buildUtil.optimizeJs(fileName, fileContents, layerLegalText, kwArgs.layerOptimize, kwArgs.stripConsole, uncompressedFileName);
		if(kwArgs.layerOptimize.indexOf("uglifyjs") == -1){
			fileUtil.saveUtf8File(fileName, compressedResult);
		} else {
			compressProcesses[i] = compressedResult;
			compressFiles[i] = fileName;
		}
	}
	// Waiting for compression result ready. 
	if(kwArgs.layerOptimize.indexOf("uglifyjs") == 0){
		for(i = 0; i < result.length; i++){
			compressProcesses[i].waitFor();
			if(compressProcesses[i].exitValue() != 0){
			   throw "Compress file: " + compressFiles[i] + "failed.";
			}
		}
	}
	...
	//clear dependency js file.
	/*logger.info("Clear Dependency Files: " + kwArgs.releaseDir);
	for(var i = 0; i < result.length; i++){
		for(var j=0; j< result[i].depList.length; j++){
			var layerName = result[i].layerName.substring(2, result[i].layerName.length);
		    if((result[i].depList[j].indexOf(layerName)==-1) && (result[i].depList[j].indexOf("dojoGuardEnd.jsfrag")==-1) 
		       && (result[i].depList[j].indexOf("dojoGuardStart.jsfrag")==-1) && (result[i].depList[j].indexOf("concord/scenes/ErrorScene")==-1) && (result[i].depList[j].indexOf("dojox/gfx/path")==-1)&& (result[i].depList[j].indexOf("dojox/gfx/svg")==-1)&& (result[i].depList[j].indexOf("dojox/gfx/shape")==-1)){
			  fileUtil.deleteFile(result[i].depList[j]);
			  logger.info("remove js file: " + result[i].depList[j]);
			  var index = result[i].depList[j].lastIndexOf("/");
			  if(index!=-1){		  	
			    var pathName = result[i].depList[j].substring(0, index);
			    var jsName = result[i].depList[j].substring(index, result[i].depList[j].length);
			    fileUtil.deleteFile(pathName + "/nls" + jsName);
			    logger.info("remove nls file: " + pathName + "/nls" + jsName);
			  }
			}
		}
	}*/
