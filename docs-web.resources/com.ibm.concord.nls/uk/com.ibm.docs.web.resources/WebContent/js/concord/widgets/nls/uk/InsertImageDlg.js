/* ***************************************************************** */
/*                                                                   */
/* IBM Confidential                                                  */
/*                                                                   */
/* IBM Docs Source Materials                                         */
/*                                                                   */
/* (c) Copyright IBM Corporation 2012. All Rights Reserved.          */
/*                                                                   */
/* U.S. Government Users Restricted Rights: Use, duplication or      */
/* disclosure restricted by GSA ADP Schedule Contract with IBM Corp. */
/*                                                                   */
/* ***************************************************************** */

({
	titleInsert :"Insert Image",
	insertImageBtn :"Insert",
	upload	: "From File System",
	insertFromUrl: "From Web",
	insertFromUrlWithColon: "URL:",
	urlIsNull: "The address of the image is not correct.  Select a valid image and try again.",
	loading: "The image is loading...",
	maxSize: "The image is too large to upload.The Maxinum size of a image that can be upload is ${0} KB",
	insertImageErrorP1: "The image cannot be inserted into the document.",
	insertImageErrorP2: "There is a problem on the server, such as not enough disk space.",
	insertImageErrorP3: "Ask your administrator to check the server log to identify the problem.",
	dlgIFrameTitle: "Frame for inserted image"
})
