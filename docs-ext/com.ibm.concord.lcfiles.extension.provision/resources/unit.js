 ...
 * @return in
 */
concord.unit.InToCm = function (inch){
	return inch*2.54;
};
/**
 * Convert em to cm
 ...
 *  @return float
 */
concord.unit.toCmValue = function( string ){
 ...
 *  @return float
 */
concord.unit.toPtValue = function( string ){
	var cm = concord.unit.toCmValue( string );
 ...
 */
concord.unit.toLocalizedValue = function( string ){
	var value = concord.unit.toCmValue( string );
	if ( !isNaN(value) && !concord.unit.isMeticUnit() )
		value = value/2.54;
	return value;
};
/**
 ...
 */
concord.unit.formatNumber = function(showString){
	if(showString == null)
		return NaN;
	var g_locale = dojo.locale;	
 ...
 */
concord.unit.parseNumber = function(showString){
	if(showString == null)
		return NaN;
	var g_locale = dojo.locale;	
	dojo["requireLocalization"]("dojo.cldr",'number', g_locale);
...
concord.unit.fullWidthNum = {
	"０":"0","１":"1","２":"2","３":"3","４":"4","５":"5",
	"６":"6","７":"7","８":"8","９":"9"
},
	
concord.unit._normalizeChar = function(value){
...
concord.unit.parseValue = function( string, allowedUnit, isLocalied)
{
	var unitIndex = string.length+1;
	var unit = concord.unit.isMeticUnit() ? 'cm' : 'in';
	string = concord.unit.trim(concord.unit._normalizeChar(string));
	...
	string = concord.unit.trim(string.substring(0,unitIndex));
		
	var numberLocale = concord.unit.parseNumber(string);
	if(!isNaN(numberLocale))
		return isLocalied ? concord.unit.CmToPx(concord.unit.toCmValue(numberLocale + unit)) : numberLocale;
	else
		return NaN;
};