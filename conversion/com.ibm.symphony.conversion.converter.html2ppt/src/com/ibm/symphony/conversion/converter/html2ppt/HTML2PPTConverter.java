...
package com.ibm.symphony.conversion.converter.html2ppt;
...
public class HTML2PPTConverter extends AbstractFormatConverter
{
  private static String CONVERTOR = ODPCommonUtil.createMessage(ODPCommonUtil.LOG_CONVERTOR, "HTML", "PPT");

  @SuppressWarnings("unused")
  private static Logger log = Logger.getLogger(HTML2PPTConverter.class.getName());
  ...
  public ConversionResult convert(File sourceFile, File targetFolder, Map parameters) throws ConversionException
  {
    ...
    try
    {
      ...
      ConversionResult tempResult = ConversionService.getInstance()
          .getConverter(ConversionConstants.HTML_MIMETYPE, ConversionConstants.ODP_MIMETYPE).convert(sourceFile, tmpFolder, parameters);
      // odp->ppt
      symResult = SymphonyConverterImpl.getInstance().convert(tempResult.getConvertedFilePath(), targetFolder.getAbsolutePath(),
          ConversionConstants.ODP_MIMETYPE, ConversionConstants.PPT_MIMETYPE, parameters == null ? null : new HashMap(parameters));
  ...
  public ConversionResult convert(File sourceFile, Map parameters) throws ConversionException
  {
    ...
    File targetFolder = new File(conversionService.getRepositoryPath() + File.separator + "output" + File.separator + "html2ppt"
  ...
  private File getTempFolder()
  {
    ...
    File tempFolder = new File(conversionService.getRepositoryPath() + File.separator + "output" + File.separator + "html2ppt"
