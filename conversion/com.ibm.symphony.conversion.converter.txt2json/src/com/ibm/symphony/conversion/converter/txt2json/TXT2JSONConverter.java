...
package com.ibm.symphony.conversion.converter.txt2json;
...
public class TXT2JSONConverter extends AbstractFormatConverter
{
  private static Logger log = Logger.getLogger(TXT2JSONConverter.class.getName());
  ...
  private ConversionResult convert(File sourceFile, File targetFolder, Map parameters, boolean isZip) throws ConversionException
  {
    ...
    try
    {
      ...
      if(result.isSucceed())
      {
        ...
        result = ConversionService.getInstance().getConverter(ConversionConstants.ODT_MIMETYPE,
            ConversionConstants.JSON_MIMETYPE).convert(odtFile, targetFolder, parameters);
    ...
    {
      log.log(Level.SEVERE, "Failed to convert TXT to JSON:", e);       
  ...
  public ConversionResult convert(File sourceFile, Map parameters) throws ConversionException
  {
    ...
    File targetFolder = new File(conversionService.getRepositoryPath() + File.separator + "output" + File.separator + "txt2json"
  ...
  {
    IConversionService conversionService = ConversionService.getInstance();
    File tempFolder = new File(conversionService.getRepositoryPath() + File.separator + "output" + File.separator + "txt2json"
