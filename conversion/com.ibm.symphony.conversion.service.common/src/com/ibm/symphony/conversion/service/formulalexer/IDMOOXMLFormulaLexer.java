...
public class IDMOOXMLFormulaLexer
{
  String formula;
    ...
    while ((curToken = nextToken()) != null && ferror == null)
    {
      ...
      else
      {
        ...
        else if (type == IDMFormulaToken.LexTokenType.NAME_TYPE)
        {
          ...
          else if (!curToken.text.isEmpty() && curToken.text.charAt(0) == '#')
          {
      ...
      switch (state)
        {
            ...
            switch (current_char)
              {
                ...
                case 0x3000 :
                {
                  ...
                  while (nextChar == ' ' || nextChar == 0xA0 || nextChar == 0x3000 || nextChar == '\n' || nextChar == '\r'
                      || nextChar == '\t')
                  {
                    sb.append(nextChar);
                  ...
                  token = generateTokenFromText(sb.toString(), IDMFormulaToken.LexTokenType.OPERATOR_TYPE,
          ...
          case STATE_NAME :
          {
            ...
            if (text.startsWith("\'"))
            {
              ...
              if (pos > 0)
              {
                // 3d reference with quote sheetname such as 'Sheet 1:Sheet 2'!A1:D3
                // split it to 3 token as 'Sheet 1' , : , 'Sheet 2'!A1:D3
                String firstsheetname = text.substring(0,pos)+"'";
                token = generateTokenFromText(firstsheetname, IDMFormulaToken.LexTokenType.NAME_TYPE, IDMFormulaToken.TokenSubtype.NAME_SHEETNAME);
                token.setOffset(offsetsave);
                pushToList(token);
                token = generateToken(':', IDMFormulaToken.LexTokenType.OPERATOR_TYPE, IDMFormulaToken.TokenSubtype.OPERATOR_COLON);
                token.setOffset(offsetsave + pos);
                pushToList(token);
                text = "'" + text.substring(pos+1);
                offsetsave = offsetsave + pos + 1;
              }
  ...
  void mergeAndAdaptToken(IDMFormulaToken token)
  {
    ...
    if (preToken != null)
    {
      ...
      if (preType == IDMFormulaToken.LexTokenType.NAME_TYPE)
      {
        ...
        IDMFormulaParsedRef ref = IDMFormulaRefParser.parseRef(preToken.text, IDMFormulaLexer.LexFormulaType.FORMAT_OOXML);
        if (ref != null)
        {
      ...
      else if (preType == IDMFormulaToken.LexTokenType.OPERATOR_TYPE)
      {
        ...
        while (preToken2 != null
            && (preToken2.type == IDMFormulaToken.LexTokenType.WHITESPACE_IGNORE || (preToken2.type == IDMFormulaToken.LexTokenType.OPERATOR_TYPE && preToken2.subType == IDMFormulaToken.TokenSubtype.OPERATOR_INTERSECTION)))
        {
        ...
        if (preToken.ch == ':' && token != null)
        {
          ...
          || preToken2.type == IDMFormulaToken.LexTokenType.NUMBER_TYPE)) // 1:1
          {
            ...
            if (text.indexOf(":") < 0 || text.indexOf(_separatorSheetname) < 0)
            {
              ...
              IDMFormulaParsedRef ref = IDMFormulaRefParser.parseRef(text, IDMFormulaLexer.LexFormulaType.FORMAT_OOXML);
              if (ref != null)
              {
