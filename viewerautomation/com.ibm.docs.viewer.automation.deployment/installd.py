...
def map2webserver():
  if map_webserver.lower() == "true":
    logging.info("Start to map web server.")
    
    ear_path = build_dir + "/" + "com.ibm.docs.viewer.automation.ear.ear"
    args = []
    args.extend([ear_path, app_name])
    succ, ws_out = call_task("map2webserver.py", args)
    return succ
  else:
...
def install():
  r = install_ear()
  if not r:
    logging.info("Failed to install the application, and exit installation.")
    sys.exit(-1)
  
  data_dir = get_data_dir()
  if data_dir:
    logging.info("Found Viewer data dir %s.", data_dir)
    copy_config_file("../proxy.json", data_dir + "/FileViewer/config/proxy.json")
  else:
    logging.info("Failed to get Viewre data dir, and exit installation.")
    sys.exit(-1)
    
  r = map2webserver()
  if not r:
    logging.info("Failed to map the application to web server.")
    
  r = start_app()
  if not r:
    logging.info("Failed to start the application.")
    sys.exit(-1)
