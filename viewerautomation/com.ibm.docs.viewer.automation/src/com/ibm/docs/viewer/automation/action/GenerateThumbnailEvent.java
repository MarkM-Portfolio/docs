...
public class GenerateThumbnailEvent extends AbstractAction
{
  ...
  public GenerateThumbnailEvent(HttpClient client, Server server, EventInfo event)
  {
  ...
  protected void initRequest(HttpUriRequest request)
  {
    ...
    if (eventInfo.docId == null || eventInfo.modified == null)
    {
      throw new IllegalArgumentException("DocId, lastModified and repository cannot be null.");
    }
    else
    {
      params.add(new BasicNameValuePair(EventInfo.DOCUMENT, eventInfo.docId));
      params.add(new BasicNameValuePair(EventInfo.MODIFIED, eventInfo.modified));
      params.add(new BasicNameValuePair(EventInfo.REPOSITORY, eventInfo.repoId));
    }
    ...
    params.add(new BasicNameValuePair("dynamicToken", generateToken(eventInfo.docId, eventInfo.modified)));
  ...
  {
    return new StringBuffer(server.getHost()).append(server.getCtxRoot()).append("/upload").toString();
  }
