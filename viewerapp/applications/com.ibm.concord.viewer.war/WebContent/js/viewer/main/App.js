...
/*                                                                   */
/* ***************************************************************** */
dojo.provide("viewer.main.App");
dojo.require("dojo.cookie");
dojo.require("viewer.util.uri");
dojo.require("viewer.util.BidiUtils");

dojo.declare("viewer.main.App", null, {

	TEXTDOC_SCENE: 			"viewer.scenes.TextDocScene",
	SHEETDOC_SCENE: 		"viewer.scenes.SheetDocScene",
	PRESDOC_SCENE:			"viewer.scenes.PresDocScene",
	PDFDOC_SCENE:			"viewer.scenes.PdfDocScene",
	PDFJSDOC_SCENE:         "viewer.scenes.PdfJsDocScene",
//	PRESDOC_MOBILE_SCENE:	"viewer.scenes.PresDocMobileScene",
...
//		{
//			viewer.scenes.ErrorScene.renderError(3002);
//			return;
//		}
...
//		this.authenticatedUser = this.loadAuthentication();
	...
	decodeState: function(uri) {
		...
		else if (DOC_SCENE.type == "pdf") {
			if(DOC_SCENE.isPDFJsViewMode == "true")
				DOC_SCENE.id = this.PDFJSDOC_SCENE;
			else
				DOC_SCENE.id = this.PDFDOC_SCENE;
		}
		else {
			throw "unsupported";
		}
...
viewer.main.App.onLoad = function(window, app) {
...
viewer.main.App.onUnload = function(window) {
	try {
		var app = window.pe;
...
viewer.main.App.onWindowUnload = function(window) {
...
viewer.main.App.executeRegex = function(regex, path, results) {
