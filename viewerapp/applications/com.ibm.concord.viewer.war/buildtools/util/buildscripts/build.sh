...
if [ "$use_node" = "0" ]; then
    ...
    case "$OSTYPE" in
#      darwin*) cmd="`dirname $0`/../../nodejs/node-v10.16.0-macos-x86/bin/node $debug_node" ;;
#      linux*)  cmd="`dirname $0`/../../nodejs/node-v0.10.33-linux-x86/bin/node $debug_node" ;;
       linux*)  cmd="${NODE_HOME}/bin/node $debug_node" ;;
      *) cmd="node $debug_node" ;;
    esac
    cmdflags="`dirname $0`/../../dojo/dojo.js"
else
    cmd="java"
    cmdflags="-Xms256m -Xmx1024m -cp `dirname $0`/../shrinksafe/js.jar:`dirname $0`/../closureCompiler/compiler.jar:`dirname $0`/../shrinksafe/shrinksafe.jar org.mozilla.javascript.tools.shell.Main  `dirname $0`/../../dojo/dojo.js baseUrl=`dirname $0`/../../dojo"
fi

echo "${0}: dojo build wrapper exec: [${cmd} ${cmdflags} load=build ${ba}]"
