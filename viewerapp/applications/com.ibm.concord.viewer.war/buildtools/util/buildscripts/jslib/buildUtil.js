...
buildUtil.DojoBuildOptions = {
	...
	"localeList": {
		defaultValue: "ar,bg,ca,cs,da,de-de,el,en-gb,en-us,es-es,fi-fi,fr-fr,fr-ca,fr-ch,he,he-il,hi-in,hr,hu,it-it,ja-jp,kk,ko-kr,nl-nl,no,nb,nb-no,nn-no,nn,pl,pt-br,pt-pt,ro,ru,sk,sl,sv,th,tr,zh-tw,zh-cn",
	...
	"optimize": {
		...
		helpText: "Specifies how to optimize module files. If \"comments\" is specified, "
			...
			+ "UglifyJS can be used by specifying \"uglifyjs\" as the value. "
	...
	"nodejs": {
		defaultValue: "",
		helpText: "The location of NodeJS command. Only need when set layerOptimize or optimize as uglifyjs."
	},
	"uglifyjs": {
		defaultValue: "",
		helpText: "The location of UglifyJS command. Only need when set layerOptimize or optimize as uglifyjs."
	}
};
...
buildUtil.optimizeJs = function(/*String fileName*/fileName, /*String*/fileContents, /*String*/copyright, /*String*/optimizeType, /*String*/stripConsole, /*String*/uncompressedFileName){
	...
	if(optimizeType == "uglifyjs"){
		return  java.lang.Runtime.getRuntime().exec([kwArgs.nodejs, kwArgs.uglifyjs, "-c", "--unsafe", "-mt", "-nc", "--copyright", copyright, "-o", fileName, uncompressedFileName]);		
	}
...
buildUtil.optimizeJsDir = function(/*String*/startDir, /*RegeExp*/optimizeIgnoreRegExp, /*String?*/copyrightText, /*String?*/optimizeType, /*String?*/stripConsole){
	...
	var compressProcesses = [];
	var compressFiles = [];
	var compressProcessNum = 0;
	if(fileList){
		for(var i = 0; i < fileList.length; i++){
			...
			if(!fileList[i].match(optimizeIgnoreRegExp)
				...
				var fileContents = "";
				if(optimizeType && optimizeType.indexOf("uglifyjs") == 0){
					//do nothing for uglifyjs
				} else {
					//Read in the file. Make sure we have a JS string.
					fileContents = fileUtil.readFile(fileList[i]);
				}
				//Do comment removal.
				if(optimizeType){
					try{
						fileContents = buildUtil.optimizeJs(fileList[i], fileContents, copyright, optimizeType, stripConsole, fileList[i]);
					}catch(e){
				...
				if(optimizeType && optimizeType.indexOf("uglifyjs") == 0){
					compressProcesses[compressProcessNum] = fileContents;
					compressFiles[compressProcessNum++] = fileList[i];
					if(compressProcessNum > 9){
					  for(var j = 0; j < compressProcesses.length; j++){
					    compressProcesses[j].waitFor();
					    if(compressProcesses[j].exitValue() != 0){
						  throw "Compress file: " + compressFiles[j] + "failed.";
						}
					  }
					  compressProcesses = [];
					  compressFiles = [];
					  compressProcessNum = 0;
				    }							
				} else {
		...
		if(optimizeType.indexOf("uglifyjs") == 0){
			for(var j = 0; j < compressProcesses.length; j++){
			   compressProcesses[j].waitFor();
			   if(compressProcesses[j].exitValue() != 0){
				   throw "Compress file: " + compressFiles[j] + "failed.";
