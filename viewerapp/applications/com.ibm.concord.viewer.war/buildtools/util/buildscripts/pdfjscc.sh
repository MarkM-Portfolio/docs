...
if [ "$use_node" != "0" ]; then
    if [ -z "$JAVA7_HOME" -o ! -d "$JAVA7_HOME" ]; then
    	cmd="java"
    else
    	cmd="$JAVA7_HOME/bin/java"
    fi
    cmdflags="-Xms256m -Xmx1024m -jar `dirname $0`/../closureCompiler/compiler.jar --language_in ECMASCRIPT5 --warning_level QUIET --compilation_level SIMPLE_OPTIMIZATIONS"
    $cmd $cmdflags $ba
fi