...
package com.ibm.docs.viewer.journal.subscriber.sample;
...
public class PackageReceivedBean implements MessageListener, MessageDrivenBean
{
  ...
  {
    // this.journalDir = WASConfigHelper.getCellVariable("VIEWER_INSTALL_ROOT");
    String installRoot = ViewerConfig.getInstance().getInstallRoot();
    if (installRoot == null || installRoot.length() == 0)
    {
      logger.log(Level.WARNING, "VIEWER_INSTALL_ROOT WAS variable NOT SETUP, JOURNLAL IS DISABLED.");
      return;
    }

    this.journalDir = ViewerConfig.getInstance().getInstallRoot() + File.separator + "journal";
  ...
  public void onMessage(javax.jms.Message msg)
  {
    ...
    {
      logger.info("IBM Viewer Journal get illeagal message");
