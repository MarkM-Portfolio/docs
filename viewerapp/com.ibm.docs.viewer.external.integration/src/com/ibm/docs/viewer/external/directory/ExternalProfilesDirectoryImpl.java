...
import com.ibm.docs.viewer.external.members.ExternalProfilesMembersModel;
import com.ibm.docs.viewer.external.members.ExternalProfilesUserImpl;
...
{   
  ...
  {
    configHelper.setRequestHeaders(method, requester, null, null);
  }
  ...
  public UserBean getById(UserBean caller, String id)
  {
    ...
    try{
      ...
      if (jsonObj != null)
      {
        ...
        LOG.log(Level.FINEST, "External Profiles UserBean is: ", bean.getId());      
      }
    }
    catch(Exception e)
    {      
    ...
    if (bean == null)
    {
      LOG.log(Level.SEVERE, "Did not get UserBean from the URL: {0} for ID {1}", new Object[] {profilesUrl, id});
    }
    return bean;
  }
