...
public class CMISDocumentEntry extends AbstractDocumentEntry implements IDocumentEntry
{
  ...
  private void init(Document document)
  {
    ...
    try
    {
      lastMajorDoc = document.getObjectOfLatestVersion(false);
    }
    catch(Throwable e)
    {
    ...
    {
      versionSeriesUri = versionSeriesId + Constants.REPO_ECM_URI_POSTFIX + docUris[1];
    }
    else
    {
      versionSeriesUri = versionSeriesId;
    }
    Set<Action> actions = workDoc.getAllowableActions().getAllowableActions();
