...
public final class TopologyConfigHelper
{
  ...
  private static final String TOPOLOGY_SERVER_VIEWER_KEY = "viewer_be";

  private static final String TOPOLOGY_SERVER_VIEWERNEXT_KEY = "viewernext_be";
      ...
      if (configFS != null)
      {
        ...
        if (configFile.exists() && configFile.isFile())
        {
          ...
          if (root != null)
          {
            ...
            if (serverJson != null)
            {
              Object vnUrl = serverJson.get(TOPOLOGY_SERVER_VIEWERNEXT_KEY);
              if (vnUrl != null)
              {
                viewerServerUrl = (String) vnUrl;
              }
              else
              {
                viewerServerUrl = (String) serverJson.get(TOPOLOGY_SERVER_VIEWER_KEY);
              }
            }
            else
            {
              viewerServerUrl = null;
            }

            if (s2sToken != null && viewerServerUrl != null)
            {
              isCloud = true;
            }

            LOGGER.log(Level.INFO, "Configuration items: viewerServerUrl=" + viewerServerUrl);
          }
        }
        else
        {
